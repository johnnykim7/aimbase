...
| CR-124 | MCP SDK 0.17.0 → 2.0.0 업그레이드 (프로토콜 `2025-11-25` 협상 실패 해소) — **발단**: bp-wes 원격 도구 CLI 노출 작업(CR-121/122/123)을 전부 해소했는데도 CLI 가 도구를 못 받는 증상이 남아 추적. **근본원인(운영 로그 실측 확정)**: `McpAsyncServer : Client requested unsupported protocol version: 2025-11-25, so the server will suggest the 2024-11-05 version instead`. Claude Code CLI 2.1.197 이 `2025-11-25` 를 요구하는데 aimbase 의 MCP Java SDK 0.17.0 은 `2024-11-05/2025-03-26/2025-06-18` 만 지원(`McpSchema.LATEST_PROTOCOL_VERSION="2025-06-18"` jar 실측). 서버가 `2024-11-05` 를 역제안 → CLI 미수용 → 핸드셰이크 미완료 → `mcp_servers=[{name=aimbase-server, status=pending}]` 고정, 도구 0개. CLI 요청 5회 전부 재현되는 **결정적 실패**(간헐 아님). **서버 정상 대조 실험**: 러너 컨테이너 안에서 러너와 동일 키·헤더로 수동 프로브(프로토콜만 `2024-11-05` 명시) → SSE endpoint 정상 → initialize 200 → **tools/list 110개 반환**(`get_progress`/`get_bottleneck`/`get_current_time` 포함). 즉 SSE·인증·도구노출·CR-121 수정 전부 정상이고 실패 지점은 버전 협상 단 하나. **소비앱 문제 아님** — AIMBASE 모드를 쓰는 순간 어느 소비앱이든 동일 발생. 지금까지 안 드러난 이유는 다른 러너(bidding/axopm/workmap)가 전부 NATIVE 라 MCP 연결 자체를 시도하지 않았기 때문(`mcp_servers=[]`). **현재 aimbase 는 AIMBASE 모드 사실상 동작 불가 상태**. **해결경로**: SDK **2.0.0 이 유일** — 1.x 대는 1.1.3 도 미지원([issue #715](https://github.com/modelcontextprotocol/java-sdk/issues/715)), 2.0.0 이 `2025-11-25` 를 모든 transport 에 추가. **2.0.0 Breaking Change**: ①**서버측 SSE transport deprecated**(Streamable HTTP 권장) — `WebMvcSseServerTransportProvider` 현재 사용 중, **최대 리스크** ②`JsonSchema`→`Map`(inputSchema) ③sealed interface 제거 ④lenient deserialization. **영향범위(실측)**: 의존성 선언 2곳(`platform-core/build.gradle.kts:32`, `sdk/tool-sdk-mcp/build.gradle.kts:9` — `mcp-bom:0.17.0`), `io.modelcontextprotocol` import 소스 **8개**(★=SSE transport 사용처: `ServerMcpConfig`★/`AimbaseAdminMcpConfig`★/`AgentMcpServer`★ + `ServerMcpToolDispatcher`/`MCPServerClient`/`McpToolConversion` + 테스트 2). **주의**: `ServerMcpConfig` 는 CR-121 로 방금 수정한 파일 — 2.0.0 전환 시 주기 동기화(`addTool`/`removeTool`/`notifyToolsListChanged`)와 기동 순서 가드가 새 API 로 살아남아야 함. **선행 확인 5**: ①2.0.0 에서 서버측 SSE 가 제거인지 deprecated 인지(제거면 Streamable HTTP 전환 필수, 규모 급증) ②Claude CLI 가 Streamable HTTP 지원하는지(`--mcp-config` 의 `"type":"sse"` 대체값) ③`addTool`/`removeTool`/`notifyToolsListChanged` 잔존 여부 ④Spring AI 버전 정합성 ⑤agent 측·BE 측 동시 배포 필요 여부. **당장 우회**: 소비앱은 NATIVE + push 방식 운영(bp-wes 세션에 push 폴백 구현 완료), AIMBASE(능동조회/pull)는 본 CR 완료 후 재개. **선례**: 0.10.0→0.17.0 업그레이드(첫 호출 180초 hang 해소)와 같은 계열이나 major 점프라 규모 큼. origin=docs/origins/원본_요구사항_CR124_MCP_SDK_2_0_0_프로토콜_20260721.md | 신규 | High | (미착수) | 📝 발번+원인확정+영향범위 조사 완료, 다른 세션 작업 |
