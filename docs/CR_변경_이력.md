...
- **요청자**: 사용자 | **승인자**: (대기) | **적용 버전**: (미정)
- **변경 일자**: 2026-06-04 (발번)
- **구현 상태**: 📝 발번 완료 (설계 확정 — 코드 수정은 사용자 승인 후, 빌드/배포 별도 승인)
- **마이그레이션**: 불필요 — 상수는 application.yml, ToolResult.newMessages 는 코드 필드 (DB 스키마 무변경)
- **설계 결정 (열린 질문)**:
  - 미지원 어댑터(supportsPdf=false) PDF: 페이지 이미지화 강제(이미지는 대부분 지원) vs 기존 PdfTextExtractor 텍스트 폴백 — 구현 시 확정
  - 로컬 Ollama 비전 모델 선택(한국어 문서 품질) — 별도 결정
