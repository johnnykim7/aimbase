...
| CR-090 | 워크플로우 실행 가시성 — `workflow_run_events` 테이블 + 조회 API. V64 마이그레이션(id BIGSERIAL, run_id UUID, step_id, iteration INT nullable, event_type[STEP_START/TOOL_USE/TOOL_RESULT/LLM_RESPONSE/STEP_END/STEP_FAILED], tool_name, duration_ms, payload JSONB[메타만], trace_id, subagent_run_id, created_at) + 인덱스 `(run_id, created_at)` + 부분인덱스 `(trace_id WHERE NOT NULL)`. **얇은 버전 원칙**: prompt/response 원본 저장 안 함, `trace_id`/`subagent_run_id` 참조 키로만 join. payload 는 메타만(input_keys, preview ≤100자, finish_reason 등). **신규 컴포넌트**: `WorkflowRunEventEntity` + `WorkflowRunEventRepository` + `WorkflowRunEventRecorder`(VT fire-and-forget, 예외 격리, TenantContext 캡처 재주입). **Hook in 3곳(DAG/cyclic 양경로)**: `WorkflowEngine.doExecuteAsync/executeCyclic` (STEP_START/END/FAILED + cyclic step_budget_exceeded), `ToolCallStepExecutor.execute` (TOOL_USE + TOOL_RESULT), `LlmCallStepExecutor.execute` (LLM_RESPONSE Phase1+Phase2). **ToolCallHandler.executeLoop hook 은 후속 보류** — AGENT_CALL → SubagentRunner → OrchestratorEngine.chat 경로에서 `ToolContext.workflowRunId` 가 null 로 들어와 이벤트 발행 안 됨, 전파 보강 후속 CR. **모두 async fire-and-forget** — 워크플로우 hot path 지연 0. **API**: `GET /api/v1/workflows/runs/{runId}/events` (시간순). 단위 테스트 Recorder 9 + Controller 3 PASS, 회귀 PASS. 운영 V64 적용·실사용 검증 대기 | 신규 | Medium | v8.14.0 | ✅ 구현 |
| CR-092 | 전통적 OCR 통합 (Tesseract) — 스캔 PDF·이미지 텍스트 추출 완성. 현재 `_read_pdf_pdfplumber(ocr_enabled)` 가 인자만 받고 본문 미사용(반쪽 구현). Vision 모델(CR-061)과 분리된 결정론·오프라인 OCR 경로 정식 신설. **Sidecar**: 신규 `rag_pipeline/tools/ocr.py`(`ocr_image_bytes`, `ocr_pdf_bytes`, 화이트리스트 검증) + `_read_pdf_pdfplumber` ocr 실연결(상위 `read_pdf` 에서 분기 — OCR 경로 vs 텍스트 추출 경로) + 신규 MCP 툴 `ocr_image(file_base64, languages)` + `read_pdf` 시그니처 확장(`ocr_languages`, `ocr_max_pages`). **BE**: `PdfTextExtractor` 1차 parse_document 결과 < BIZ-107 임계(50자) 시 `read_pdf(ocr_enabled=true)` fallback + `MCPRagClient.readPdf` + `MCPRagClient.ocrImage` + `aimbase.ocr.*` 설정 4키 (`@Value` 주입). 신규 Built-in Tool `OcrImageTool` (READ_ONLY, MCPRagClient 위임) + `McpExposurePolicy.CLI_EXPOSED` 27개로 확장 (Claude CLI 자율 호출 가능). **인프라**: Dockerfile `tesseract-ocr-kor` + `tesseract-ocr-eng` 명시 언어팩 추가 + `pyproject.toml` `pytesseract`/`pdf2image`/`Pillow` 명시. **마이그레이션**: V65 master `global_config` 4 row(`aimbase.ocr.enabled/languages/max-pages/fallback-threshold-chars`). **신규 BIZ**: BIZ-105(페이지 상한 50) / BIZ-106(언어 화이트리스트: eng/kor/jpn/chi_sim/chi_tra/fra/deu/spa) / BIZ-107(BE fallback 50자 임계). **테스트**: sidecar 26 PASS (validate 7 + ocr_image 5 + ocr_pdf 10 + read_pdf 라우팅 4) + BE 19 PASS (PdfTextExtractor 11 + OcrImageTool 8) + 통합 IT 골격 `test_ocr_integration.py`(게이트 `OCR_IT=true`). **가이드**: API v3.4.0 § 20 + Ops v3.4.0 시나리오 O. **Non-Goals**: 수식/표/손글씨 OCR, 다국어 자동감지, OCR 진행률 SSE | 신규 | Medium | v8.15.0 | ✅ 구현 |
| CR-093 | 서브에이전트 갭 정공 해소 (잔여 3건) — 3자 벤치마크(Aimbase vs OpenClaude vs Claude CLI)로 비용 1/15~1/50 우위가 입증된 상태에서, 코드 실측으로 드러난 잔여 갭만 묶어 정공 처리. **남는 진짜 갭 3건 모두 해소**: (갭 ①, 화이트리스트 코드 레벨 강제) `SubagentRunner.executeAgent` 가 `AgentTypeRegistry.getConfig().allowedTools()` 를 `ToolFilterContext` 로 변환해 `ChatRequest.toolFilter` 에 주입 — V57 프롬프트 부탁이 아닌 `ToolRegistry.getToolDefs(filter)` 단계에서 강제. readOnly 타입은 `readOnlyMode=true` 이중 방어. (갭 ②, Plan FSM 자동 진입 — **A안 채택**) `agentType == PLAN` 진입 시 부모 세션 기준 `PlanService.hasActivePlan` 확인 후 `PlanService.createPlan()` 자동 호출 → PLANNING 상태. 시스템 프롬프트에 `plan_id` + `exit_plan_mode` 안내 자동 주입. 부모 세션 없거나 PlanService 실패 시 안전 폴백(B안 동작). (갭 ③, 재귀 차단) `subagent_runs.depth` 컬럼 신설(V65 tenant 마이그레이션) + `child_session` 인덱스 + `SubagentRunner.resolveDepth(parentSessionId)` 가 `findFirstByChildSessionId` 로 부모 row 조회해 depth+1. `MAX_SUBAGENT_DEPTH=3` 초과 시 DB row/훅/Worktree 모두 생략하고 즉시 FAILED 반환. **신규 BIZ**: BIZ-108(AgentType별 도구 화이트리스트 코드 레벨 강제) / BIZ-109(서브에이전트 재귀 깊이 최대 3) / BIZ-110(Plan FSM 자동 진입 조건 — PLAN 진입 + active plan 부재 + 부모 세션 존재). **영향 범위**: SubagentRunner / SubagentRunEntity / SubagentRunRepository / V65 tenant 마이그레이션. **테스트**: 신규 SubagentRunnerCr093Test 9 PASS (화이트리스트 전파 2 + Plan 자동 진입 3 + depth 카운팅·차단 3 + GENERAL 무영향 1) + 기존 SubagentRunnerTest/AgentOrchestratorTest/AgentCallStepExecutorTest 생성자 PlanService 주입 회귀 PASS + agent 패키지 + tool.builtin + workflow.step 회귀 PASS. **운영 배포 + 5 테넌트 e2e PASS**(2026-06-03 19:18~19:21 KST): `TENANT_AUTO_MIGRATE_ON_STARTUP=true` 1회 ON → ProdTenantMigrationInitializer 가 5개 테넌트(flowguard_dev/shopai-store-a/bidding_system/shopai-store-b/axopm_companyA) V65 자동 적용 성공 5 / 실패 0 → flag OFF 복원. DB 전수 확인: flyway v65 success=true + `subagent_runs.depth INT DEFAULT 0` + `idx_subagent_runs_child_session` 5개 테넌트 모두 존재. BE 재기동 후 ToolRegistry 47 tools / Server MCP 27 CLI-exposed 회귀 없음. **Non-Goals**: 이미 구현된 ToolResultStorage/ContextWindowManager/MemoryAutoExtract 재손질, 도구 루프 횟수 변경, AgentType 추가, 사용자 정의 agent 마크다운 등록(별도 CR) | 변경 | Medium | v8.16.0 | ✅ 운영 라이브 |
| CR-094 | ParseDocumentTool 신설 — AGENT 자율 호출용 8종 문서 파싱(PDF/DOCX/PPTX/XLSX/CSV/HTML/TXT/MD). 사이드카 `parse_document` MCP 도구를 BE Built-in Tool(READ_ONLY)로 노출, Native Read 가 binary 로 거부하는 DOCX/XLSX/PPTX 등을 LLM 이 직접 파싱. url/file_path 입력 + 화이트리스트 검증 + 50MB 상한 + 32K 응답 절삭 | 신규 | Low | v8.16.0 | ✅ 구현 (커밋 b42a244) |
| CR-095 | PDF Native 비전 파싱 (openclaude 1:1 포팅) — PDF 를 사이드카 텍스트 추출(unstructured/OCR ~38초)이 아니라 LLM API 안에서 모델 비전으로 파싱. ≤3MB & 지원 어댑터 → base64 `document` block 통째 / >3MB or 미지원 → poppler(pdf2image, CR-092 기설치)로 페이지→JPEG 이미지화 → `image` block 배열. 상수(`PDF_EXTRACT_SIZE_THRESHOLD`=3MB / `PDF_MAX_PAGES_PER_READ`=20 / `PDF_AT_MENTION_INLINE_THRESHOLD`=10) application.yml 외부화. **사이드카** `pdf_to_images(base64, pages?)` MCP 도구 신설. **도구 결과 멀티모달 주입(핵심 구조 확장)**: openclaude `newMessages`(isMeta user 메시지로 document/image block 별도 주입) 대응 — `ToolResult.newMessages` 필드 + `ToolCallHandler` 도구 루프 주입 + `UnifiedMessage.ofUserContent`. **두 진입점**: ChatController.resolveDocumentBlock 3MB 게이트 + ParseDocumentTool PDF 비전 경로. 비전 주체=Anthropic 기본 + 로컬 Ollama 비전 옵션(IMAGE_ONLY+images 구조 준비됨, 비전 모델 pull 전제조건). **후속(06-05)**: 13MB/29p turn timeout = openclaude 10p 분할 가드(`PDF_AT_MENTION_INLINE_THRESHOLD`) 누락 → `pdf_page_count` 도구 + `PdfVisionResolver` NEEDS_SPLIT 가드(임계 초과 시 모델이 `pages` 로 분할 호출) + `ParseDocumentTool` `pages` 파라미터. 원본=`docs/origins/원본_요구사항_PDF_비전파싱_20260604.md` | 신규 | High | v8.18.0 | ✅ 운영 라이브 |
| CR-096 | Super Admin 테넌트 임퍼소네이션 — 플랫폼 관리자가 테넌트를 골라 '들어가서' 해당 테넌트 화면(워크플로우/RAG/정책/세션 등) 전체를 보는 운영 UI. **현 구조 실측**: ①로그인은 테넌트 DB `users` 만 조회(이메일로 tenant 자동 resolve, 1유저=1테넌트) ②Master `tenant_admins` 테이블/엔티티/리포지토리는 존재하나 `findByEmail` 호출처 0건 = 죽은 인프라 ③`JwtAuthenticationFilter:118` 가 access 토큰 `tenant_id` claim ≠ `X-Tenant-Id` 헤더면 403(CR-086 사칭 방어, 이미 구현됨) → super admin 이라도 헤더만 바꿔선 타 테넌트 접근 불가. **해소 방식(정공)**: 헤더 가드 유지한 채, super admin 만 합법적으로 대상 테넌트 토큰을 발급받게 함. **BE**: (1) `AuthController.login` 에 Master `tenant_admins` 로그인 분기 추가 — 이메일이 tenant_admins 에 있으면 `tenant_id=null` + role=`super_admin` 토큰 발급(기존 users 경로 하위호환 100%) (2) `JwtProvider.generateImpersonationToken` — 대상 `tenant_id` + role=super_admin + `impersonating=true` + `actor=관리자 email` claim, 단기(30분) (3) `POST /api/v1/platform/tenants/{id}/impersonate`(ROLE_SUPER_ADMIN 전용) — tenant active 검증 후 임퍼소네이션 토큰 발급 + 감사 로깅(BIZ-020). 403 헤더 가드는 무변경(impersonation 토큰의 tenant_id claim = 대상 테넌트라 일치 통과). **FE**: (4) super admin 로그인 시 테넌트 선택 화면(`GET /platform/tenants` 목록, ID 비노출·name 표시) (5) 선택→impersonate 호출→토큰+tenant_id localStorage 교체→전체 화면 동작 (6) 상단 임퍼소네이션 배너 + '나가기'(super admin 토큰 복원). **마이그레이션**: 없음(tenant_admins·admin@platform.local 계정 기존재). **Non-Goals**: 테넌트 간 데이터 이동/복사, super admin 셀프 가입, impersonation 중 쓰기 작업 제한(읽기·쓰기 동일 권한 — 후속 검토) | 신규 | Medium | (미정) | 🚧 구현 중 |
| CR-097 | SUB_WORKFLOW 비주얼 에디터 노출 (FE 마감) — BE `SUB_WORKFLOW` StepType + `SubWorkflowStepExecutor`(2026-04-04 v4.0 병합 시 완성, 변수치환·위상정렬·재시도까지 동작)는 살아있으나 **FE 비주얼 에디터에 통째로 미노출**되어 화면으로 서브워크플로우 노드를 만들 수 없음. **실측 누락 4곳**: ①`components/workflow/NodePalette.tsx:1` PALETTE_ITEMS 8종에 sub_workflow 없음 ②`components/workflow/ConfigPanel.tsx:411` getConfigFields() switch 에 SUB_WORKFLOW case 없음(설정 UI 부재) ③`components/workflow/WorkflowNode.tsx:4` TYPE_STYLES 에 sub_workflow 스타일 없음 ④`utils/flowToWorkflow.ts:5` TYPE_MAP 에 sub_workflow→SUB_WORKFLOW 매핑 없음. **마감 작업(FE only)**: (1) PALETTE_ITEMS 에 `{type:"sub_workflow", label:"서브 워크플로우", icon, color}` 추가 (2) ConfigPanel 에 SUB_WORKFLOW case — `workflow_id`(플랫폼 공용 워크플로우 드롭다운, ID 비노출·name 표시 — feedback_id_display 준수) + `input`(변수치환 JSON) 필드 (3) TYPE_STYLES 에 sub_workflow + SUB_WORKFLOW 스타일 (4) TYPE_MAP 매핑 (5) `types/workflow.ts` StepType union 에 sub_workflow 명시. **데이터원**: 플랫폼 공용 워크플로우 목록 = `GET /api/v1/platform/workflows`(또는 기존 platform workflows 조회 API 확인 후 사용). **BE 무변경 / 마이그레이션 없음**. **범위 경계**: 부를 수 있는 대상은 현행대로 플랫폼 공용 워크플로우(`platform_workflows`)만 — 테넌트 임의 워크플로우 호출은 본 CR 제외. **자식 run 트리 가시화(실행이력에서 서브워크플로우가 흡수되지 않고 트리로 보이게)는 본 CR 제외 → 기존 CR-065 에 위임**. 원본=`docs/origins/원본_요구사항_CR097_SubWorkflowFE노출_20260605.md`. **구현 완료(2026-06-05)**: FE 5파일 수정 — NodePalette(sub_workflow 팔레트 추가) / WorkflowNode TYPE_STYLES(sub_workflow+SUB_WORKFLOW) / flowToWorkflow TYPE_MAP(sub_workflow→SUB_WORKFLOW) / types/workflow.ts StepType union / ConfigPanel(usePlatformWorkflows hook 으로 workflow_id 드롭다운 name 표시·ID 비노출 + input JSON, JSON_CONFIG_KEYS 에 input 기존 포함 → handleSave 자동 파싱). 데이터원=기존 `usePlatformWorkflows`(GET /api/v1/platform/workflows). BE 무변경. **추가 보강**: SubWorkflowStepExecutor 단위 테스트 0→10개 신설(supports/workflow_id 누락·공백/미존재/비활성/빈 steps/input 변수치환/위상정렬 순차실행/input 미지정/executor 미존재) 전부 PASS, FE tsc 통과 | 변경 | Low | (미정) | ✅ 구현 |
| CR-098 | Claude CLI 어댑터 FE 노출 + 운영 Runner docker 상주 — BE `ClaudeCliAdapter`(CR-071)는 정식 구현됐으나 연결 추가/수정 화면 LLM 어댑터 선택지에서 누락. **(1) FE Connections.tsx**: ADAPTERS.llm 에 "Claude CLI" 추가 + 선택 시 API Key 숨김(OAuth 기반) + CLI 전용 필드(tool_mode 셀렉트 AIMBASE/NATIVE/HYBRID, config_dir, runner_api_key, system_prompt_override) 노출 + handleSave/openEditModal config 키 매핑(BE 키명 일치) + 모델 placeholder. **(2) BE ConnectionAdapterFactory.normalizeAdapterType**: 공백형 "claude cli"/"anthropic cli" 매칭 추가 — FE 표시명 "Claude CLI"(공백)가 `contains("claude")` 로 일반 anthropic 어댑터에 오분류되는 함정 차단. **(3) 피처 플래그** `llm.anthropic-cli.enabled-tenants` flowguard_dev→`*` (BIZ-099, 전체 허용). **(4) 런타임 설정 화면 LLM 카테고리 편집 가능화**: PlatformSettingsService.CATEGORIES + FE PlatformSettings.CATEGORY_LABELS 에 `llm` 추가(조회 카테고리는 하드코딩, 수정 경로는 카테고리 무관). 테넌트 화이트리스트 전용 입력 — ID 직접 입력 대신 테넌트 멀티셀렉트(name 표시/id 저장, feedback_id_display 준수) + 전체허용(*) 토글, 네이티브 체크박스→프로젝트 UI Checkbox 교체. **(5) 운영 Runner docker 상주**: aimbase-agent(같은 jar 에 SERVLET=Runner HTTP /v1/chat + --mcp-stdio=Tool agent MCP 두 역할, BE platform-core 엔 RunnerController 없어 별도 프로세스 필수)를 59번 운영 서버 docker-compose 서비스로 기본 상주. 같은 compose 네트워크라 TURN 불필요(BE→Runner localhost). claude-accounts/local-agent-1 OAuth 계정 공유(실측 유효). aimbase-agent Dockerfile 신규(claude CLI 설치) + docker-compose.prod.yml agent 서비스 + deploy.sh agent 타겟. anthropic-cli connection 신규 생성(config_dir/runner_api_key). 원본=`docs/origins/원본_요구사항_CR098_ClaudeCLI어댑터FE노출+운영Runner상주_20260605.md`. **Non-Goals**: Gemini/기타 프로바이더 항목 추가(기존 OpenAI Compatible 로 수동, 사용자 결정), 로컬 SDK 도구(file/bash) chat 경로 기본 노출(AIMBASE=aimbase MCP / NATIVE=CLI 내장), TURN 기반 사용자 PC agent. **실측 정정 2건**: (a) SecurityConfig `/api/v1/**` .authenticated() 가 agent 자가등록(`/api/v1/agents/register,*/heartbeat,DELETE`)도 막아 403 → permitAll 추가(토큰 없는 부트스트랩, 테넌트 격리는 TenantResolver X-Tenant-Id 가 보장). (b) AgentLifecycle 이 runnerEndpoint 를 TURN-TCP 활성 경로에만 metadata 등록 → TURN 비활성+같은 네트워크 경로가 비어 runner_capability=false 로 라우팅 실패. **보강**: AgentProperties.runnerEndpoint + AgentLifecycle.setExplicitRunnerEndpoint + start() 에서 !turnEnabled && explicitRunnerEndpoint 시 metadata.runnerEndpoint 등록. **운영 배포+e2e PASS(2026-06-05)**: docker-compose aimbase-agent 서비스 기동(claude CLI 2.1.143 내장, claude-accounts 볼륨 공유) → bidding_system 에 ACTIVE 등록 runner_capability=t runner_endpoint=http://aimbase-agent:8290 확인 → Runner /v1/chat 직접 호출 e2e: local-agent-1 OAuth 계정으로 Claude CLI spawn → "AIMBASE CLI E2E OK" 정상 응답(input 10/output 64 tokens, finish=END). anthropic-cli connection(cli-runner-bidding-001, config_dir/runner_api_key/tool_mode=AIMBASE) 생성 | 신규 | High | v8.17.0 | ✅ 운영 라이브 |
| CR-099 | CLI 어댑터 경로 멀티모달(PDF/이미지) 전달 — API 경로(AnthropicAdapter)는 document/image block 으로 PDF/이미지를 LLM 비전에 전달하나, CLI 경로(ClaudeCliAdapter→Runner→claude CLI)는 toRawMessages 가 content 를 텍스트로 평탄화하며 Document/Image 블록을 버려 멀티모달 불가였음. **실측 확정(openclaude=claude CLI 동등코드 + 운영 직접 호출)**: claude CLI 2.1.143 stream-json input 이 Anthropic content block 배열을 수용 — `{type:"user",message:{role:"user",content:[{type:"text"},{type:"document",source:{type:"base64",media_type:"application/pdf",data}}]}}`. 우리는 base64 **전달만** 하고 비전 읽기는 CLI/LLM 이 함(사용자 정정: 비전화≠데이터 사전정제). **구현 3곳**: (1) ClaudeCliRunnerClient.toRawMessages — Document/Image 있으면 content 를 Anthropic content block 배열로 보존(toAnthropicBlock), 없으면 텍스트 String 하위호환 (2) RunnerService.toUnified — content 가 블록 배열이면 ContentBlock(Text/Image/Document) 복원(toContentBlock), 문자열이면 텍스트 (3) ClaudeCliWorker — writeUserContentBlocks 신설, 멀티모달 USER 메시지를 stream-json content 배열로 stdin 전송(text/document/image, base64 source 만, systemPrefix 는 첫 text 블록 prepend). URL source·기타 블록 미지원(생략). tool_mode 무관(메시지 입력 채널). **운영 배포(agent+be)+e2e PASS(2026-06-05)**: 유효 PDF(SECRET CODE: PURPLE-7392) → Runner /v1/chat document 블록 → RunnerService 복원 → Worker stdin → claude CLI 비전 → "PURPLE-7392" 완독(손PDF 는 PURP 만, 정상PDF 전체). **Non-Goals**: URL source(base64 만), 파일경로+Read 방식(base64 블록 채택), BE chat/completions 풀경로 토큰 e2e(Runner 직접 호출로 입증) | 신규 | Medium | v8.18.0 | ✅ 운영 라이브 |
| CR-100 | 로컬 PC 문서 찾아 분석 — 파싱 다리 (로컬 agent ↔ 서버 사이드카) — **배경**: "내 PC /Users/sykim/aaa 에서 PDF 찾아 분석해줘" 발화를 끝까지 추적. **라우팅**(user_ref 매핑, CR-075 와 같은 매핑)✅ + **찾기**(로컬 agent SdkToolKit Glob/Grep/FileRead, 서버→agent MCP CR-072/074)✅ 는 있으나, **파싱**(PDF/오피스→텍스트, unstructured) 다리가 ❌ 빈 구멍. **실측 확정(2026-06-06)**: 로컬 agent(aimbase-agent + tool-sdk-core 14개)에 파싱도구·MCPRagClient·byte 업로드 경로 전무 — ParseDocumentTool 은 platform-core(서버BE)에만 존재. agent 가 로컬 PDF 를 byte 로 읽기까지만 되고 서버 사이드카 파싱으로 보낼 다리 없음. DocumentSectionReadTool 은 Markdown heading/line range 읽기일 뿐 파싱 아님. **왜 기존엔 문제 안 됐나**: Runner(Claude CLI)+도구 한 PC 동거 시 CLI 와 파일이 같은 PC라 "로컬/서버"가 문제 안 됨. Runner(서버)/로컬 agent(도구) 분리하며 "파일은 로컬, LLM/파싱은 서버" 간극 노출. **선행 완료(이번 세션, CR 무발번 — 사용자 '작은 수정' 결정)**: parse_document file_path 입력 추가(서버사이드 base64 왕복 제거) — parser.py(file_path 분기 + _validate_local_path 화이트리스트 + _parse_from_path/_elements_to_result/_resolve_partition_fn) + server.py(MCP 노출) + MCPRagClient.parseDocumentByPath + ParseDocumentTool(비-PDF path 패스, PDF 비전 유지) + compose(rag-pipeline workspace 마운트 + PARSE_ALLOWED_ROOTS). 운영 라이브+e2e PASS(TXT/DOCX/거부). 이 file_path 는 서버사이드용, 로컬 시나리오는 base64 업로드 경로. **결정사항**: 사이드카 로컬 설치 폐기(실제 사용자 Docker 없음, 파싱은 서버 사이드카), 로컬엔 가벼운 agent(Java)만, 패키징도 agent만(기존 jpackage 재사용). **다음 세션 설계 대상**: (1) 파싱 다리 — 로컬 agent 가 찾은 파일 byte → 서버 ParseDocumentTool/사이드카로 base64 업로드. 갈래: 로컬 agent 에 '파싱요청' 도구 추가 vs 서버 LLM 이 agent FileRead 로 byte 받아 서버 파싱(통신 방향 실측 후 결정) (2) agent 로컬 application.yml(TURN off/Runner off/workspace) (3) agent-only 인스톨러. 원본=`docs/origins/원본_요구사항_CR100_로컬문서파싱다리_20260606.md`. **구현 완료(2026-06-08, 갈래 1-a)**: 실측으로 사이드카(Python `parser.py:82-136` file_content base64)+`MCPRagClient.parseDocument(base64)` 가 이미 base64 입력을 받음을 확인 → **Python/Java RAG 클라이언트 무수정**, Java 도구 2개만 수정. (1) `FileReadTool` `as_base64` 옵션 신설 — 바이너리(PDF/DOCX)도 base64 반환(일반 모드는 메타만), 10MB 가드(stdio/relay 부담). (2) `ParseDocumentTool` `content`(base64) 입력 신설 — url/file_path/content 3종 택1 분기, PDF면 비전(BE 디코드→document/image 블록 주입) 그 외 사이드카 텍스트 추출, 50MB 가드+base64 유효성 검증. **완성 흐름**: CLI→`builtin_file_read(as_base64=true)`(로컬 byte)→`parse_document(content=base64)`(서버 MCP, CLI-level 기존 노출)→`MCPRagClient.parseDocument`→사이드카 `parse_document(file_content)`→unstructured. **도중 수정**: 선행 작업(file_path→`parseDocumentByPath` 미커밋)에 테스트 mock 미갱신으로 깨져있던 `execute_filePathReadsBytes` 정정. **테스트**: 신규 `FileReadToolTest` 4 + `ParseDocumentToolTest` content 4 추가, tool 패키지 회귀 PASS. 남은: agent 로컬 yml/인스톨러(패키징, 별개) | 신규 | tool-sdk-core/platform-core | 4파일 | ✅ 구현+테스트 완료, 운영 배포 진행 |
| CR-102 | 워크플로우 실행 본문 전문 적재 + 한눈에 보는 UI — **배경**: CR-090(workflow_run_events 얇은 메타) 운영 라이브 후, "흐름은 시간순으로 보이는데 LLM 이 무엇을 받아 무엇을 답했나" / "도구 input/output 이 다음 단계에 제대로 쓰였나" 정독 불가. 메타만으로는 **품질 분석/리그레션 추적/소비앱 디버깅**이 어려움 (payload preview ≤100자, output_size 만 보임). **세션 사이 사용자 선반영(미커밋)**: `WorkflowRunEventEntity` 에 `prompt_text/response_text/input_json/output_text` 컬럼 4종 + `WorkflowRunEventRecorder` 에 본문 전문 받는 오버로드(`stepEnd(...,outputBody)` / `toolUse(...,input jsonb)` / `toolResult(...,outputBody)` / `llmResponse(...,promptBody,responseBody)`) 추가됨 — 코드 주석은 "CR-099" 였으나 changelog CR-099(CLI 멀티모달)와 번호 충돌하여 **CR-102 로 정식 발번**. **남은 작업**: (1) V65 tenant 마이그레이션 — `workflow_run_events` ALTER TABLE 로 4개 컬럼 추가(prompt_text/response_text TEXT, input_json JSONB, output_text TEXT) (2) Hook 호출부 갱신 — `WorkflowEngine.doExecuteAsync/executeCyclic` 의 stepEnd 호출에 result 본문 전달, `ToolCallStepExecutor.execute` 의 toolUse/toolResult 에 input map/output 전달, `LlmCallStepExecutor.execute` 의 LLM_RESPONSE 발행 시 prompt(system+user 결합)+response textContent 전달 (3) 코드 주석 정정 — `CR-099` → `CR-102`. **UI 가시화(범위 결정 필요)**: 별도 메뉴 vs 기존 `WorkflowDetail.tsx` 탭 추가. 시간순 한 줄 흐름(Claude Code 스타일) + 이벤트 클릭 시 본문 패널(prompt/response/input/output) 펼치기. **얇은-두꺼운 결합 원칙**: 흐름 재구성은 메타 컬럼(payload), 정독은 본문 컬럼 — 같은 row 에 공존. 본문 컬럼 nullable, 기존 데이터(메타만) 하위호환. **Non-Goals**: 본문 압축/외부 저장(tool_result_storage 와는 별개 — 본 CR 은 인라인 적재), 본문 절단(절단 없는 원문, 추후 운영 시 디스크 부담 측정 후 보완), 로그 익스포트(JSON 다운로드 등 별도). **구현 완료(2026-06-12)**: 실측 결과 선반영분이 발번 메모보다 진척 — V66 마이그레이션(`V66__cr102_workflow_run_event_bodies.sql`, V65 는 CR-093 이 선점)·Recorder 오버로드·Hook 3곳(WorkflowEngine stepEnd DAG/cyclic + ToolCallStepExecutor 성공/에러 + LlmCallStepExecutor Phase1/2 `[SYSTEM]+[PROMPT]` 합성)·`?include_body=true` 까지 이미 작업트리에 존재 → 이번 세션은 마감 작업. (1) CR-099→CR-102 표기 정정(주석 11곳 + V66 파일명 rename) (2) **신규 API 3종** — `GET /workflows/runs`(횡단 run 목록, workflow_id/status 필터 + 페이지네이션, `WorkflowRunRepository.searchRuns`) + `GET /workflows/runs/{runId}`(run 단건) + `GET /workflows/runs/{runId}/events/{eventId}`(이벤트 단건 본문 — 타임라인은 메타로 가볍게, 행 펼침 시에만 본문 fetch. include_body 일괄은 수 MB 위험이라 화면은 단건 패턴), 이벤트 직렬화 `toEventMap` 공통화 (3) **FE "실행 내역" 신설**(사용자 결정: 특정 워크플로우에 얽매이지 않는 전체 횡단 뷰) — 사이드바 메뉴 + `WorkflowRuns.tsx`(워크플로우/상태 필터 + 페이지네이션) + `WorkflowRunDetail.tsx`(run 메타 카드 + 이벤트 타임라인 6종 시간순 + 행 클릭 본문 전문 펼침 prompt/response/input/output, `useWorkflowRunEvent` enabled-on-expand + staleTime Infinity) + 라우트 `/workflow-runs(/:runId)` + AppShell 헤더 (4) **용량 정책(사용자 결정)**: 무조건 적재, TTL 후속 CR. **테스트**: Recorder CR-102 오버로드 5 신규(전문 무절단/하위호환 null) → 14 PASS, Controller 신규 4(단건 본문/타 run 404/횡단 필터 위임/run 단건 404) → 8 PASS, workflow.* 회귀 30클래스 무실패, FE tsc PASS. **가이드**: API v3.8.0 § 17-10 + Ops v3.5.0 § 3-5 실행 내역 화면. **운영 배포+e2e(2026-06-12)**: 구 cr099 파일 삭제 후 BE+FE 배포. V66 은 선반영 세션에서 이미 5테넌트 적용돼 있었고 `TenantMigrationRunner.repair()`(CR-066) 가 cr099→cr102 파일명/체크섬 변경을 히스토리에 자동 정렬(전 테넌트 description=cr102, cols=4 확인). e2e: bidding_system 임시 워크플로우(LLM_CALL 1스텝, haiku) 실행 → prompt_text `[SYSTEM]+[PROMPT]` 전문 + response_text + STEP_END output_text 적재 + 횡단 목록/단건 API 200 + 타 run 경계 404 + 타임라인 메타 응답엔 본문 미포함(가볍게) 전부 확인 후 임시 데이터 정리. 커밋 d3c73a7 (origin/dev). **2차 — 내부 도구 루프 가시화(2026-06-12)**: 1차 라이브 후 사용자 지적 "모든 액티비티(LLM 이 툴 호출한 것까지) 전부 보이게" + "API/CLI 단일 인터페이스이니 이미 구현돼 있을 것" → 실측: CLI Worker 가 stream-json tool_use 를 파싱·관찰까지 구현(`[CLI-OBS]` 로그)하나 LLMResponse 에 안 실어 한 구간만 끊김(사용자 판단 절반 적중). 3갈래 구현 — **(①)** 구조화 출력(response_schema) 본문 폴백: textContent 빈 응답이면 structured_data JSON 을 response_text/output_text 에 적재(스크린샷 "0 chars" 원인 해소, LlmCallStepExecutor+WorkflowEngine.resultBody 2곳) **(②)** AGENT_CALL 서브에이전트 BE 도구 루프 가시화: workflowRunId/stepId/subagentRunId 를 `AgentCallStepExecutor→SubagentRequest→ChatRequest→OrchestratorEngine→ToolContext`(레코드 3개 필드 추가, 호환 생성자) 로 전파 + `ToolCallHandler` 에 eventRecorder 훅 — 회차별 LLM_RESPONSE(응답만, prompt 는 누적 중복이라 미적재 — 사용자 결정) + executeAndRecord TOOL_USE(input 전문)/recordLineage 인접 TOOL_RESULT(output 전문), subagent_run_id 완전 연결(사용자 결정: ToolContext 필드 추가) **(③)** CLI 내부 루프 관찰 운반: `LLMResponse.observedToolEvents` 신설(관찰 전용 — toolCalls 재사용 시 외부 루프 재진입 충돌, CR-050 결정 보존) + Worker tool_use(assistant)↔tool_result(user 이벤트) tool_use_id 페어링(duration 근사) + RunnerController/RunnerChatResponse/ClaudeCliRunnerClient HTTP 직렬화 + Recorder.observedTools 일괄 적재(LLM_CALL 은 LlmCallStepExecutor, AGENT_CALL 은 ToolCallHandler 에서). **테스트**: 신규 ToolCallHandlerCr102Test 3(훅/무적재/관찰적재) + Recorder observedTools 1 → 회귀 platform-core tool/workflow/agent/orchestrator 54클래스 + cli-runner 4 + aimbase-agent 1 전부 무실패. **한계**: 스트리밍 도구 루프(executeLoopStream)는 LLM_RESPONSE 훅 미적용(서브에이전트는 비스트리밍이라 공백 없음), CLI 관찰 운반은 비스트리밍 /v1/chat 경로만, **LLM_CALL/AGENT_CALL 의 CLI 연결은 워크플로우 경로 라우팅 미지원**(X-Aimbase-Agent-Id/user_ref 가 HTTP 컨텍스트 기반 — `cli_routing_missing`, CR-075 가 chat 경로용인 기존 제약 — 워크플로우 CLI 라우팅은 후속 CR 후보). **2차 운영 배포+e2e(2026-06-13, BE+agent 재배포)**: ① translate-opportunity-title 재실행 → response_text 에 structured_data JSON + STEP_END 폴백 확인 ② AGENT_CALL(haiku) 임시 워크플로우 → 회차별 LLM_RESPONSE(iter 0/1) + TOOL_USE/TOOL_RESULT(에이전트가 run_python 자율 선택, input_json/output_text 전문, subagent_run_id 연결) 전부 적재 ③ 상주 Runner /v1/chat 직접 호출 → CLI 내부 bash 실패→calculate 성공 관찰이 observed_tool_events 로 운반 확인. **e2e 가 또 운영 버그 발견**: TOOL_USE insert 가 CR-090 이래 전멸(운영 전 역사 0행 vs TOOL_RESULT 78행) — payload 의 `Set`(input.keySet()) 이 Hypersistence jsonb 깊은복사에서 "byte array cannot be transformed to Json" 으로 깨짐 → List 변환 핫픽스 + estimateResultSize "0 chars" 보정. 커밋 d9b36fe (CR-098/099/065 미커밋 헌크는 분리 보존) | 신규 | High | (미정) | ✅ 2차 운영 라이브 + e2e PASS (d9b36fe) |
| CR-101 | CLI(MCP) 경로 PDF 비전 — 도구 결과 멀티모달 운반 — **배경**: CR-100 어댑터 풀경로 운영 e2e(2026-06-12)에서 발견. 비-PDF(DOCX)는 자율 도구 선택까지 PASS(자연어만으로 CLI 가 `builtin_file_read(as_base64)`→`parse_document(content)` 체인 선택, 마커 회수)했으나 **PDF 케이스 FAIL** — CLI 가 "no separate message visible" 응답. **원인(코드 실측)**: `parse_document` 의 PDF 비전 결과는 쪽지+소포 구조 — `ParseDocumentTool.buildVisionResult`(ParseDocumentTool.java:227) 가 `ToolResult` 에 `output`(`{"mode":"inline_pdf",...}` 쪽지) + `newMessages`(PDF base64 document 블록 = 소포) 동봉. **API 경로**는 도구 루프가 ToolResult 객체를 받아 `ToolCallHandler`(ToolCallHandler.java:866,972) 가 newMessages 를 user 메시지로 주입 → 정상. **CLI 경로**는 `ServerMcpToolDispatcher.dispatch`(ServerMcpToolDispatcher.java:96-116) 가 `tool.execute()` **String 반환만** `McpSchema.CallToolResult` 로 직렬화 — newMessages 운반 채널 부재로 dispatch 단계에서 소포 유실. 루프 주체 문제가 아니라 **전송 채널(결과 직렬화) 문제**. CR-095(비전 메커니즘)·CR-099(유저 메시지 멀티모달)는 각자 범위에서 정상 — "CR-095 메커니즘 × CR-072 채널" 통합 갭. **해소안 2갈래**: (1) 정공 — dispatcher 가 newMessages 를 MCP tool result 의 멀티모달 content 블록(image/document)으로 변환. 선행 실측 필수: claude CLI 가 tool result 내 document/image 블록을 실제 수용하는지(MCP 표준은 image content 지원, document 는 비표준 가능성) (2) 폴백 — CLI 수용 불가 시 MCP 채널 한정으로 PDF 도 사이드카 텍스트추출(+스캔본 OCR, CR-092 자산 재사용)로 글자 반환. **검증 기준**: CR-100 어댑터 풀경로 e2e PDF 케이스(마커 PDF→verbatim 회수) PASS. **영향 범위**: ServerMcpToolDispatcher(+필요시 ParseDocumentTool MCP 컨텍스트 분기), 마이그레이션 없음. 원본=`docs/origins/원본_요구사항_CR101_CLI경로_PDF비전_도구결과_멀티모달운반_20260612.md`. **구현(2026-06-12)**: ① 선행 실측 — 미니 MCP stdio 서버(마커 PNG/PDF를 tool result 에 실어 반환)로 claude CLI 직접 검증. **image 블록은 인라인으로 모델 비전 전달**(2.1.109 로컬 + 2.1.143 운영 컨테이너 둘 다 마커 회수). **PDF blob(EmbeddedResource)은 2.1.143 이 인라인하지 않고 파일 저장 + 텍스트 포인터로 치환** — 프로브에선 CLI 가 native Read(비전)로 저장 파일을 읽어 회수했으나, 운영 Runner 는 `--tools ""`(native 도구 잠금)라 저장 파일을 읽을 수 없음(1차 운영 e2e 실측: dispatcher 블록 운반은 정상 도달, CLI 가 포인터만 보고 우회 시도하다 turn timeout). ② 확정 구현 — `ServerMcpToolDispatcher.dispatch` 가 `EnhancedToolExecutor` 면 String bridge 대신 ToolResult 직접 수신, 텍스트는 기존대로 `ToolResultRenderer.render`+truncate, `newMessages` 는 MCP content 변환 동봉: image→ImageContent, **document(PDF)→`PdfVisionResolver.resolve(bytes,false,true)` 페이지 이미지화→ImageContent 배열**(안내 TextContent 동반, 렌더 실패 시 EmbeddedResource blob 폴백 — NATIVE/HYBRID tool-mode 용), 비-PDF document→EmbeddedResource(`aimbase://tool-result/` 가상 URI), text→TextContent. base64 블록은 truncate 제외. ParseDocumentTool inline_pdf note 채널 중립화. **테스트**: ServerMcpToolDispatcherTest 신규 4(PDF 페이지 이미지화/렌더실패 폴백/단일 텍스트 유지/Enhanced 실패 훅) + mcp/tool 회귀 PASS. **운영 e2e PASS(2026-06-12)**: FlateDecode 압축 마커 PDF(raw 바이트에 마커 부재 — 1차 시도에서 무압축 PDF 를 CLI 가 raw 읽기로 우회 회수한 교란 발견·차단) → 어댑터 풀경로(chat/completions→Runner→CLI→builtin_file_read(as_base64)→parse_document(content)) → tool result 에 page image(jpeg 30KB) 도착(세션 jsonl 실측) → 마커 SAFFRON-3318-CR101 verbatim 회수. 관찰된 별건: agent self-heartbeat 400(STALE 전락, CR-072 후속 기지), analyze_image JDBC 예외 | 신규 | Medium | (미정) | ✅ 운영 라이브 + e2e PASS |
| CR-105 | 워크플로우 실행 중지 API (협조적) — **배경**: 소비앱에서 "워크플로우를 실행시키고, 중지시키고 싶다"는 요구. **실측(2026-06-14)**: 실행 API(`POST /workflows/{id}/run`)·승인(`/runs/{runId}/approve`)·조회는 있으나 **중지 API 부재** — `WorkflowController` 에 cancel/stop/abort 매핑 0건. `WorkflowRun` enum 에 `CANCELLED` 값은 정의돼 있고 조회 필터(`status=cancelled`)도 있으나 **그 상태로 전이시키는 코드가 워크플로우 패키지 전무**(빈 껍데기). 중지 API 가 있는 곳은 워크플로우가 아닌 `chat/{sessionId}/abort`(CR-046)·`agents/{runId}/cancel`(서브에이전트) 둘뿐. **실행 구조**: `WorkflowEngine.execute` 가 Virtual Thread fire-and-forget(runId→Thread 핸들 미보관)이라 외부 강제 interrupt 불가. **사용자 결정**: 협조적 중지(방식 A, 스텝 경계에서만 끊어 DB/CLI 워커 일관성 유지) + 바로 구현(CR 사후 발번). **구현(2026-06-14)**: (1) `WorkflowEngine` 에 `ConcurrentHashMap.newKeySet()` 중지 표식 + `cancelRun(UUID)` 공개 진입점 — running 은 표식만(백그라운드 루프 위임) / pending_approval 은 백그라운드 스레드 없으므로 즉시 cancelled 전이 + 대기 승인 엔티티 cancelled 정리 / terminal 은 무변경 멱등 / 미존재 IllegalArgumentException. (2) DAG 루프(`doExecuteAsync` step 순회) + cyclic 루프(`executeCyclic` worklist poll) 양쪽 스텝 경계에 `isCancelRequested` 체크포인트 → 감지 시 `finishCancelled`(stepResults 보존 + status=cancelled 저장 + `runCompleted("cancelled")` 이벤트 + 메트릭). (3) VT finally 3곳(execute/executePlatform/resume)에 `clearCancel`(표식 누수 방지). (4) `WorkflowController` `POST /workflows/runs/{runId}/cancel`(404 on 미존재). **한계**: 협조적이라 진행 중 한 스텝(긴 LLM 호출 등)은 끝까지 수행 후 멈춤(즉시성 없음). 메모리 플래그라 멀티노드 시 running 은 인스턴스 로컬, pending_approval 은 DB 기반이라 노드 무관. **테스트**: 신규 `WorkflowEngineCancelTest` 9 PASS(running 표식만/pending_approval 즉시중지+승인정리/terminal 3종 멱등/미존재 예외/requestCancel 멱등) + workflow.*·WorkflowController.* 회귀 GREEN + platform-core compileJava PASS. **마이그레이션 없음**(CANCELLED enum 기존 존재). 원본=`docs/origins/원본_요구사항_CR105_워크플로우_실행중지_20260614.md`. **재기동/배포 보류**(사용자 지시) | 신규 | Medium | v3.10.0 | ⏸️ 구현+테스트 완료, 배포·재기동 보류 |
| CR-104 | CLI 어댑터 도구 노출 목록을 API 경로와 동일화 — **결함**: 워크플로우를 CLI 커넥터로 돌릴 때 CLI 가 받는 도구 목록 ≠ API 커넥터일 때. 같은 스텝이 API 면 동작, CLI 면 `No such tool`. **실측(2026-06-14)**: 도구 흐름은 어댑터 무관 동일(`OrchestratorEngine.executeLoop → request.tools() → adapter.chat`). API 어댑터는 `transformToolDefs` 가 tools 파라미터로 변환해 전달, **CLI 어댑터는 `ClaudeCliAdapter.transformToolDefs()` 가 항상 null** — 도구를 버리고 오직 `--mcp-config` 통로로만 받음. 두 목록 출처 불일치 = **A) API tools = `ToolCallHandler.getToolDefs(toolFilter)`(전체 builtin 48개 중 허용분)** vs **B) CLI MCP 노출 = `McpExposurePolicy` CLI 화이트리스트 28개** → 미분류 14개(bash/file_write/builtin_grep/file_read/glob/safe_edit/patch_apply/structured_search/document_section_read/path_info/workspace_snapshot, zip_extract, calculate, get_current_time)가 NONE 폴백으로 CLI 에 안 보임. **핵심 불변식(목표)**: aimbase 가 API tools 에 넣는 도구 목록 == CLI 에 MCP 로 노출하는 목록. **사용자 결정**: 노출 정책=**B안**(화이트리스트 유지 + 누락분 추가) / 범위=**정공 A 풀구현**(호출별 동적 mcpConfig·도구 전달) + 운영 env 보강 / 메커니즘=**①** endpoint 는 B 전체 노출 + CLI `--allowedTools` 로 스텝별 제한(endpoint 노출목록은 기동 시 고정=상한, 호출별로는 allowedTools 로 좁힘). **구현(2026-06-14)**: (Phase 1) `McpExposurePolicy.CLI_EXPOSED` 28→**42**(미분류 14개 추가, EXPLICITLY_NONE 6개=team_create/delete·enter/exit_plan_mode·verify_plan_execution·temp_cleanup 유지 → 42+6=48 전수 분류). (Phase 2 정공 A) `ClaudeCliRunnerClient.buildBody` 가 `request.tools()`(이미 어댑터까지 도달하나 버려지던 것) 도구명을 `allowed_tools` 로 전송 → `RunnerChatRequest.allowedTools`(DTO 신규 필드) → `RunnerController.chat/chatStream` → `RunnerService.chat/chatStream`(5인자 오버로드) → `runTurn`(allowedTools) → `ClaudeCliWorkerPool.getOrCreateMain`(6인자 오버로드) → `ClaudeCliWorker.setAllowedTools` → `buildCommand` 가 원본 도구명 → `mcp__aimbase-server__<tool>` prefix 변환 → `ClaudeCliCommandBuilder.allowedTools` → `--allowedTools`. Worker 는 **run 단위 spawn 시 고정**(재사용 정책상 스텝별 교체 불가 — 별도 CR). (Phase 3) `docker-compose.prod.yml` aimbase-agent: `AIMBASE_RUNNER_AIMBASE_MCP_JAR` 제거(local stdio MCP 중복 + prefix 깨짐 방지) + `AIMBASE_RUNNER_SERVER_MCP_BASE_URL=http://api:8080` / `SERVER_MCP_API_KEY` / `SERVER_MCP_AGENT_ID=cli-runner-bidding` 추가 → resolveMcpConfigJson 이 `aimbase-server` SSE 만 합성. `/mcp/**` 인증용 bidding_system 바인딩 평문 API key `plat-771…` 신규 생성(aimbase_master.api_keys, 평문 복원불가라 신규 필요). **테스트**: 신규 6 PASS(McpExposurePolicyTest 2 차집합/내부전용 + ClaudeCliCommandBuilderTest 2 allowedTools 플래그 + ClaudeCliWorkerTest 2 stub 프로세스로 prefix 변환 실검증) + RunnerControllerTest 회귀(service.chat 4→5인자 mock) 수정 + 전체 빌드 SUCCESSFUL + tool/mcp/workflow 패키지 회귀 PASS. **운영 배포+e2e(2026-06-14, BE+agent)**: BE 로그 `exposing 42 CLI-level tool(s)` + CLI-INIT `tools=[mcp__aimbase-server__... 42개]`(이전 `{"mcpServers":{}}` 0개) + `--allowedTools` 107회 = 정공 A 작동 + 도구 실호출 parse_document 25/file_write 8/bash 8/tool_search 3/builtin_workspace_snapshot 1 + **`No such tool` 0건** = 인계서 실패 증상 전부 해소. **★ 별개 이슈 노출(범위 밖 → CR-106)**: run 최종 failed 사유는 `No such tool` 이 아니라 **`turn timeout after 300s`**(`cli_runner_unreachable:AGENT_TIMEOUT`). 원인=다수 PDF 작업이 Worker turnTimeout 300s + HTTP timeout 300s 초과 + **AGENT_CALL retry 3회가 [SubagentRunner](backend) 가 매 호출 새 `subagent-<UUID>` runId 발급 → Worker 재사용 키 불일치 → 매번 새 spawn + 30KB 프롬프트 처음부터 → 진행 버리고 같은 일 3번 반복**. 도구 자체는 정상. **미커밋**(작업트리 CR-098/099/065 등 타 헌크 혼재 — 선별 커밋 필요) | 신규 | High | (미정) | ✅ 구현+배포+e2e PASS (도구 불변식 충족), 커밋 대기 |
| CR-106 | AGENT_CALL 장기작업 timeout + retry 멱등화 (Worker 세션 재사용) — **배경**: CR-104 e2e 중 발견. CLI 가 도구를 정상 호출하게 되자(이전엔 즉시 `No such tool` 실패) 비로소 "다수 PDF 적재 같은 긴 AGENT_CALL 이 300초를 넘긴다"는 다음 단계 문제가 드러남. **실측(2026-06-14, run 9a8704a8 / opportunity-analysis / build_workspace)**: 3겹 문제 — (1) **근본**: build_workspace 가 CLI 에게 첨부 PDF 다수를 parse_document→file_write 로 작업장 적재 지시 → 단일 turn 300초 초과(첫 turn 22:46:41~22:48:23 에 parse_document/file_write/bash 활발 호출 후 침묵→timeout). (2) **타임아웃 2곳 동시**: `ClaudeCliWorker.turnTimeout` 기본 **300s**(`ClaudeCliTimeoutException: turn timeout after 300s`) + `ClaudeCliRunnerClient` HTTP `.timeout(Duration.ofSeconds(300))`(`HttpTimeoutException: request timed out`) 거의 동시 발동. (3) **retry 가 멱등 아님**: `WorkflowEngine.executeWithRetry` 가 AGENT_CALL 3회 retry 하는데 매번 [SubagentRunner.java:113-114](backend/platform-core/src/main/java/com/platform/agent/SubagentRunner.java#L113) 가 **무조건 새 `runId = UUID.randomUUID()` + `childSessionId = subagent-<UUID>`** 발급 → CLI Runner `run_id`(=Worker 재사용 키) 가 매번 달라 `getOrCreateMain` 이 살아있는 Worker 못 찾고 새 spawn(운영 로그 3개 다른 `subagent-<UUID>` 확인) + CLI `--resume` 미사용이라 이전 작업장 맥락(이미 적재한 파일들) 모른 채 30KB 프롬프트부터 다시 → 같은 일 3번 반복, 세 번째 turn 은 5분간 도구 0건 후 timeout. **재사용/이어하기 인프라는 이미 존재**(getOrCreateMain runId 재사용 + ClaudeCliWorker `--resume <sessionId> --fork-session`) — 호출 측이 매번 새 ID 주는 게 미사용 원인. **방향 확정(2026-06-14 사용자 결정)**: **A(retry 멱등화/이어하기) + B(timeout 설정화)** 채택, **C(FOREACH 분할) 제외**(소비앱 워크플로우 영역). 이어하기 범위=**timeout 류 실패만**(그 외 새 세션 깨끗이 재시도), 적용 경로=**워크플로우 AGENT_CALL 한정**(메인 대화 서브에이전트 격리 유지). **구현(2026-06-14)**: (A) `SubagentRequest.resumeSessionId`(nullable) 신설 + [SubagentRunner.java:113](backend/platform-core/src/main/java/com/platform/agent/SubagentRunner.java#L113) 분기(있으면 childSessionId 로 사용, 없으면 기존 새 UUID) + `AgentCallStepExecutor.buildRequest(…, enableResume)` 가 직전 attempt 가 turn timeout 류일 때만 `deterministicSessionId(runId, stepId)`(=`subagent-`+`UUID.nameUUIDFromBytes(run:step)`)를 resumeSessionId 로 전달 → 같은 step retry 는 같은 CLI run_id → Pool Worker 재사용 + `--resume`. 단일 AGENT_CALL 한정(멀티는 enableResume=false, 같은 stepId 충돌 회피). (A-채널) `StepContext.previousAttemptFailure` + `withRetryFailure()` 신설 + `WorkflowEngine.executeWithRetry` 가 catch 에서 `attemptContext = context.withRetryFailure(e.getMessage())` 로 다음 attempt 에 직전 실패 주입. (B) `ClaudeCliRunnerClient` HTTP timeout 하드코딩 300s → `@Value platform.llm.anthropic-cli.http-timeout-seconds`(application.yml, ENV `ANTHROPIC_CLI_HTTP_TIMEOUT_SECONDS`, 기본 300) + `RunnerAutoConfiguration` Worker turnTimeout 하드코딩 300s → `RunnerProperties.turnTimeoutSeconds`(`aimbase.runner.turn-timeout-seconds`, 기본 300). step별 동적 turnTimeout 전파는 미채택(BE→Runner HTTP body 무변경, 양 모듈 걸친 작업이라 범위 초과 — 정적 설정화로 갈음). **테스트**: 신규 `AgentCallStepExecutorCr106Test` 6 PASS(첫시도 noResume / timeout→결정적 resume / 비timeout→noResume / 결정적키 안정·구분 / isTurnTimeoutFailure 변형) + 기존 AgentCallStepExecutorTest 7 + workflow/agent/adapter/cli-runner/aimbase-agent 회귀 전부 PASS, 전체 BUILD SUCCESSFUL. 설계서 `docs/CR106_설계_장기AGENT_CALL_retry_멱등화.md`. **빌드/배포·BIZ 번호 부여·커밋 대기** | 신규 | High | (미정) | ✅ 구현+단위테스트 완료, 빌드/배포 대기 |
| CR-109 | Claude CLI Worker 좀비 누수 해소 (turn timeout 시 Worker 정리 + BE cancel 신호) — turn timeout 시 claude CLI 프로세스가 종료되지 않고 누적(운영 12개 관측). 누수 2경로 정공 해소: **A(Runner 내부)** `RunnerService.runTurn` catch 가 timeout 예외(`ClaudeCliTimeoutException`) 시 worker.close() 미호출 → catch 에서 `workerPool.shutdownForRun(runId)` 호출(firstTurnDone 회수+모든 worker close → 재초기화/재시도 가능). **C(BE→Runner)** `ClaudeCliAdapter` 가 HTTP timeout 시 `runnerClient.cancel()` 미호출(메서드 정의됐으나 호출처 0건) → chat/chatStream catch 에서 `cancelIfTimeout()` 추가(`classifyRunnerFailure`=AGENT_TIMEOUT/TURN_BROKEN_PIPE 만 전송, AGENT_OFFLINE 은 Runner 에 닿지도 못해 제외). **C 보조** cancel 전용 10s timeout(`ClaudeCliRunnerClient.newJsonRequest` timeout override 오버로드 — chat 용 300s 와 분리). 부품(close/destroyForcibly·shutdownForRun·/v1/cancel·RunnerService.cancel)은 모두 정상 존재했고 timeout 실패경로에서 호출만 안 된 게 원인 — 신규개발 아닌 연결작업. **테스트**: 신규 `RunnerServiceTest` 3(timeout→shutdown / 재진입 / 정상→no-shutdown) + `ClaudeCliAdapterTest` 3(chat timeout→cancel / offline→no-cancel / stream timeout→cancel) PASS + cli-runner/aimbase-agent/기존 ClaudeCliAdapterTest 회귀 PASS. **운영 배포(2026-06-15, deploy.sh be+agent)**: agent jar 바이트코드에 shutdownForRun 호출 박힘 검증, 두 컨테이너 Up(healthy), 재기동으로 기존 좀비 12→0 리셋. 잔여=운영 e2e(긴 AGENT_CALL timeout 유발 후 `ps -e\|grep -c [c]laude` 누적 안 됨 확인). 번호: CR-108 은 observeConsumer(실시간 도구 관찰) 사용 중이라 좀비건은 CR-109. | 신규 | High | (운영 라이브) | ✅ 운영 배포 완료, 운영 e2e 대기 |
| CR-110 | CLI/API 도구 노출 목록 하드코딩 → 런타임 설정 단일 소스 외부화 — **배경**: 소비앱(bidding) AGENT_CALL(CLI 자율주행)에서 CLI 가 `parse_document` 를 ToolSearch 로 헛검색하는 화면을 추적하다 더 근본 문제 발견. 사용자 3질문이 같은 뿌리: (1) "노출 툴 리스트 왜 하드코딩?" — `McpExposurePolicy.CLI_EXPOSED = Set.of(43개)` 코드 박힘 → 추가/제거마다 빌드+배포. (2) "API/CLI 어댑터 동일 인터페이스가 정석인데 왜 다른가?" — 인터페이스(`LLMAdapter`)는 통일됐으나 **노출 목록 출처가 둘로 갈림**: API=`ToolRegistry.getToolDefs(toolFilter)`(McpExposurePolicy 안 봄) vs CLI=`McpExposurePolicy.resolve()==CLI`. CR-104 가 "두 목록 동일집합"을 주석 불변식으로만 강제 중. (3) "도구 목록을 설정으로 빼야" — 런타임 설정값이어야 정석. **실측 전제**: `getMcpExposedExecutors`=죽은 코드(호출 0건); `resolve()` 1순위(`ToolContractMeta.mcpExposureLevel` 자기선언)=실질 미사용(선언 도구 0개)→100% 하드코딩 의존; `parse_document` 사이드카 직접 호출(`MCPRagClient.parseDocument`, 첨부 자동파싱)=도구레지스트리 무관(무영향); `NAME_TO_LEVEL` static 필드라 클래스로드 시점 완성→설정 기반 전환 시 빈 미존재 함정→**static→빈 전환 강제**; API 8개 호출처 모두 `getToolDefs(toolFilter)` 단일 통과. **Phase 0(게이트)**: 전 활성 테넌트(bidding/flowguard/shopai/axopm 등) `workflows.steps::text LIKE '%parse_document%'` 조회 → **0건** 확인 후 진행. **사용자 결정**: parse_document CLI+API 양쪽 제거 / 단일소스화=메타선언+블랙리스트(정석) / **API까지 하나로 수렴**(Phase 분리 안 함) / parse_document=`@ConditionalOnProperty` 빈 비활성화. **구현(2026-06-15)**: (P1) `McpExposurePolicy` static 유틸→`@Component` 빈, `PlatformSettingsService` 주입, `Set.of` 삭제→`resolve()` 가 `getStringList("mcp.cli-exposed-tools", DEFAULT_FALLBACK)` 참조(메타 1순위 메커니즘 보존, 데이터 미충전). `ServerMcpConfig`/`ServerMcpToolDispatcher` 빈 주입+인스턴스 호출. `getMcpExposedExecutors` 삭제. (P2 API 수렴) `ToolRegistry.getToolDefs(filter)` 가 노출 정책 코드 참조→API 모델 목록==CLI 노출 집합(CR-104 불변식 코드 강제). **회귀 가드(구현 중 발견)**: 외부 MCP 도구(`MCPServerManager`/`RemoteToolDiscovery` 동적 register, 화이트리스트 없어 NONE)가 API 목록서 전멸하는 위험→`builtinNames` 집합으로 **builtin 만 정책 적용**, 외부 MCP 는 항상 통과. CR-068 환각 회귀 가드(정책 NONE 만 제외, 세션/권한 축소 금지). (P3) `ParseDocumentTool` `@ConditionalOnProperty(aimbase.parse-document.tool-enabled, matchIfMissing=false)` 기본 비활성화. **V66 master seed** `mcp.cli-exposed-tools`=42개(parse_document+내부전용 6개 제외), `DEFAULT_FALLBACK` 코드상수와 동일집합(fail-safe·환경 갈림 방지). **테스트**: `McpExposurePolicyTest` 재작성(목 주입+parse_document NONE 가드+blank→fallback+42개 고정) + 신규 `ToolRegistryExposureFilterTest`(builtin NONE 제외/외부MCP 통과/null filter) + `ServerMcpToolDispatcherTest` 5인자 수정. 단위+회귀(ToolCallHandler/SessionToolRegistry/ServerMcp/ContextAssembly) 전부 PASS. **운영 배포+검증(2026-06-15, deploy.sh be)**: Flyway V66 master 적용 성공 + global_config seed(597자, parse_document 미포함) + 기동 로그 `Server MCP endpoint exposing 42 CLI-level tool(s)`(parse_document 부재) + `ToolRegistry 48 builtin`(parse_document 부재=조건부 빈 작동) + impersonate→CLI 어댑터 chat 2회 워커 spawn 정상(cost_usd=0.0, parse_document dispatch 0건=CLI 미도달). 잔여=실 워크플로우 run(첨부 PDF HYBRID allowedTools 경로) e2e(코드+단위로 확정, 다음 워크플로우 실행 시 자연 확인). 커밋 8a67683. | 신규 | High | (운영 라이브) | ✅ 구현+배포+기동/chat e2e 완료, 워크플로우 run e2e 대기 |
| CR-111 | AGENT_CALL turn 30초 조기 절단 해소 (agent async timeout 상향 + BE stream 미완료 종료 감지) — **배경**: CR-110 검증 중 bidding opportunity-analysis 워크플로우(13MB/29p 이미지 PDF 첨부)의 extract_facts(AGENT_CALL)가 PDF 읽던 중 **31초에 끊겨** "~하겠습니다" 도입부만 남기고 종료 → COMPLETED 오판 → 다음 스텝(verify/structure)에 빈 입력 연쇄 실패("어설픔"). **근본원인(운영 로그 실측)**: aimbase-agent `/v1/chat/stream` 은 `StreamingResponseBody`(Spring MVC async)인데 `spring.mvc.async.request-timeout` 미설정 → 컨테이너 기본 **30초**에 `interrupted while awaiting result`/`AsyncRequestTimeoutException`. BE `timeoutMs`(120s)·turn timeout(300s)보다 훨씬 짧은 이 30초가 가장 먼저 끊는 진짜 병목이었다. BE 측은 stream 이 `result`(정상 종료) 이벤트 없이 끊겨도 그때까지 받은 부분 delta 로 응답 조립 → 예외 없이 COMPLETED 처리(finish_reason 없음). **수정 2지점**: (1) `aimbase-agent/application.yml` `spring.mvc.async.request-timeout=600000`(ENV `AIMBASE_AGENT_ASYNC_TIMEOUT_MS` override) — turn timeout(300s)+여유 포괄. (2) `ClaudeCliRunnerClient.chatStream` `sawResult` 플래그 — `result` 이벤트 없이 스트림 종료 시 `RuntimeException("...turn truncated — likely agent async timeout")` 승격. 메시지에 "timeout" 포함 → CR-106 `isTurnTimeoutFailure` 매칭 → 같은 세션 이어하기 retry. 부분 delta 를 COMPLETED 로 흘려보내 다음 스텝에 빈 결과 넘기던 것 차단. **전파**: chat() 예외 → SubagentRunner `future.get` catch → `SubagentResult.failed` → AgentCallStepExecutor → WorkflowEngine retry(최대 3회, CR-106 멱등 이어하기). **테스트**: ClaudeCli/Subagent/AgentCall/aimbase-agent 회귀 PASS. **운영 배포+e2e(2026-06-15, deploy.sh be+agent)**: 동일 13MB PDF 워크플로우 재실행(run dc02c628) → 이전 31초 절단이 **300초까지 생존** → timeout 시 FAILED→3차 이어하기 재시도 → **extract_facts→verify_and_gapcheck→structure_output→save 전 스텝 실행, run completed**(이전엔 extract 에서 끊겨 후속 미실행). 커밋 0265e20. **★남은 과제(별개, 후속)**: ① **A 효율** — 300초×3회 retry 로 완주는 하나 느림. 실측 결과 모델·claude cli 정상(클로드코드 37초/claude cli 71초로 같은 PDF 분석), 운영만 마지막 도구 호출 후 **4분 15초 침묵→timeout** 반복(원인 미확정 — 여러 대용량 PDF 이미지 컨텍스트 누적 ttft 의심, 미검증). timeout 상향은 증상 가리기라 부적절 — 한 turn 작업량(4 PDF 정독+11항목+FACTOR계층+근거부착+contentJson 거대 출력) 축소가 근본(소비앱 워크플로우 프롬프트/스텝 분할 영역). ② **B 값유실** — structure_output 이 `{{verify_and_gapcheck.output}}` 만 받는데 verify 출력이 검증 마크("✓정확")만 남기고 실제 값 누락 → "값을 모르겠다" 변환 실패 → save status=FAILED(run 은 completed). bidding 워크플로우 변수 전달 설계 문제(소비앱 영역). | 신규 | High | (운영 라이브) | ✅ 구현+배포+run e2e 완주, A 효율/B 값유실 후속 |
| CR-112 | CLI 스트림 단절을 성공으로 둔갑하는 4층 진범 해소 — **배경**: CR-111 후속 운영 추적에서 bidding extract_facts(AGENT_CALL)의 진짜 실패 메커니즘 확정. CLI 도구 타임라인(aimbase-agent 로그): 21:47:16 Read→Bash→ToolSearch→Read×4→21:48:00 Grep 이후 **3분 11초 공백**(다음 turn 스트리밍 응답 없음) → 21:51:19 STEP 종료(output=`socket connection was closed`). 운영 서버↔api.anthropic.com 긴 turn(sonnet-4-5 대용량 분석)에서 스트리밍 응답이 ~3분 끊기는데 turnTimeout(300초) 미달이라 안 잡힘. **핵심 버그**: CLI 가 socket closed 를 result 텍스트로 뱉으면 `ClaudeCliWorker.awaitResult` 가 exit_code 0 / COMPLETED 로 처리 → 다음 STEP 에 쓰레기 입력 → run 실패. **실측 확정(openclaude QueryEngine.ts 원본 대조)**: claude code stream-json `result` 이벤트는 `subtype`(success/error_max_turns/error_max_budget_usd/error_during_execution/error_max_structured_output_retries) + `is_error` boolean. 정상 success 는 `is_error:false`, **socket closed/API 에러는 subtype=success 라도 `is_error:true`(isApiError), 모든 error_* subtype 도 `is_error:true`** → **`is_error` 가 success/error 전 subtype 공통 단일 진실 소스**(subtype 파싱 불필요, 오탐 없음). 기존 `awaitResult` case "result"(549-586) 는 `is_error` 미검사 + result 텍스트를 finalText 로 받아 무조건 `FinishReason.END` 반환. `socket connection was closed`/`API Error:`/`is_error` 처리 BE 전체 0건(grep 확정). CR-111 `sawResult` 가드는 *result 이벤트가 아예 없을 때*만 동작 → CLI 가 에러를 `is_error:true` result 로 정상 발행하면 sawResult=true 로 통과(별개 경로). **수정**: `ClaudeCliWorker.awaitResult` case "result" 에서 `Boolean.TRUE.equals(json.get("is_error"))` 감지 시 `ClaudeCliException("CLI result reported is_error=true (subtype=...): ...")` 승격. **전파 체인(실측 검증)**: awaitResult throw → `RunnerService:125` catch(worker 정리+재throw) → `RunnerController:128` stream 람다 catch → NDJSON `{type:"error"}` → BE `ClaudeCliRunnerClient.handleStreamEvent:412` 재throw → `AgentCallStepExecutor:81`(FAILED/TIMEOUT→Exception 승격) → `WorkflowEngine.executeWithRetry:919`(retryMaxAttempts 재시도, CLI `--resume` 이어하기). 단발 `/v1/chat`(비스트림)도 같은 awaitResult 통과라 동시 보호. **테스트**: `ClaudeCliWorkerTest` +3(is_error+success subtype→예외 / is_error+error_during_execution→예외 / is_error:false→정상 반환 회귀) 전체 11 PASS + ClaudeCliAdapterTest/AgentCallStepExecutorTest 회귀 PASS. **운영 배포(2026-06-16, deploy.sh agent)**: aimbase-agent jar(cli-runner 의존 포함) 재빌드+컨테이너 재기동, Runner health UP(active_runs=0/max_workers=5), MCP 14 tools/STUN/Registry 정상 기동. 커밋 f67076d. **CR-111 과의 경계**: CR-111=result 없이 스트림 단절(sawResult), CR-112=result 가 is_error:true 로 발행되는 둔갑 — 두 경로 모두 차단. 잔여=긴 turn 유발 후 is_error 승격→retry 이어하기 운영 e2e. | 신규 | High | (운영 라이브) | ✅ 구현+배포+단위 e2e, 운영 e2e 대기 |
| CR-113 | 구조화 출력(response_schema) 단일 인터페이스 — CLI 입구 강제(JSON-only 지시) + 어댑터 출구 정규화(String→Map 객체화) — **배경**: 소비앱(bidding)이 LLM_CALL+response_schema 결과에서 ```json 펜스를 직접 벗기는 코드를 매번 복붙하던 문제 추적. **실측 진단(2단계)**: ①`LlmCallStepExecutor.extractStructuredData()`(586-591)는 `ContentBlock.Structured` 블록만 줍는데, **7개 어댑터 중 `Structured` 를 만드는 건 AnthropicAdapter 하나뿐**(tool_use `structured_output`→Structured 변환, 573-577). 나머지 6개(OpenAI/OpenAICompatible/Bedrock·Vertex 위임/Ollama/ClaudeCli)는 응답을 `ContentBlock.Text`(JSON String)로만 내놓아 extractStructuredData→null→`structured_data` 미기재. **핵심 개념 정리(사용자 토론)**: structured_data 는 "JSON→Map 변환"이 아니라 **"LLM 이 준 JSON String 을 객체화"**(같은 내용, .get(key) 가능한 형태). AnthropicAdapter 만 객체화하고 나머지는 String 그대로 흘려보낸 것. ②**더 근본 결함 발견(운영+단독 CLI 실측)**: CLI 는 Anthropic tool_choice / OpenAI json_schema strict / Ollama json_object 같은 **API 레벨 JSON 강제 장치가 전혀 없다**(CLI 어댑터/워커 responseSchema 처리 grep 0건). 운영 bidding subagent_runs 6건 전수 확인 → **structured_data 6/6 전부 null**, output 은 MD 표/한글 산문/영문/```json 펜스/`API Error: socket closed` 로 **형식이 완전히 흔들림**. 즉 CLI 는 펜스 JSON 조차 안 줄 때가 태반 → 출구 펜스제거만으론 해결 불가. **단독 CLI 명령 실측(claude 2.1.109, sonnet-4-5)**: "정리해줘"(형식 미지정)→MD 표 / `--append-system-prompt` 로 "이 schema 의 JSON only, no fence, no preamble" 강제→**순수 JSON**(펜스도 없음) / "HTML 로"→HTML. **즉 CLI 는 형식을 지시하면 그대로 따른다 — 못 지키는 게 아니라 안 시켰던 것**. **수정(입구 강제 + 출구 정규화, 2026-06-17)**: (A 출구) 신규 `StructuredOutputNormalizer.normalize(content, responseSchema)` — schema 있고 Structured 없으면 Text(JSON String)에서 펜스 제거(```언어태그…``` 정규식)+첫`{`~마지막`}` 추출→JSON 객체화→`ContentBlock.Structured` 부착. 이미 Structured 면 no-op, JSON 아니면 원본 유지. 텍스트 어댑터 4곳 응답 조립부에 1줄 호출(OpenAI/OpenAICompatible→Bedrock·Vertex/Ollama/ClaudeCliRunnerClient). (B 입구) `ClaudeCliRunnerClient.appendSchemaDirectiveToLastUser` — response_schema 있으면 **마지막 user 메시지 끝**에 "ONLY 1 valid JSON object, raw JSON only, no fence/preamble + schema JSON" 지시 덧붙임(멀티모달 배열이면 text 블록에). **system prompt 가 아니라 user 메시지에 붙인 이유**: `--system-prompt` 는 CLI 기본 프롬프트 완전 교체라 systemPromptOverride 가 null 인 커넥터(`conn.config.system_prompt_override` 선택필드, 대부분 미설정)에서 CLI 기본 동작(도구 가이드 등) 소실 위험 → user 메시지 덧붙이기가 안전. **YAML/key:value 폴백은 추가했다 제거**(실측상 Anthropic/OpenAI/Ollama 는 이미 JSON 강제 통일, 흔들리는 건 CLi 뿐 → "JSON 으로 통일"이 본질이지 "느슨한 형식 흡수"가 아니라는 사용자 결론). extractStructuredData/EvaluatorLoop 자체폴백은 미수정. 효과=CLI 도 JSON 으로 통일→객체화→structured_data 채워짐, response_schema 쓰는 모든 소비앱(WMS/OMS/MALL/bidding)이 모델 무관 수신, 펜스-벗기기 복붙 불필요. **이중 방어**: 입구(CLI JSON 강제)+출구(펜스 제거 안전망). **주의**: structured_data Map 계약상 최상위 객체만(배열 미지원). **테스트**: `StructuredOutputNormalizerTest` 8 PASS + `ClaudeCliSchemaDirectiveTest` 4 PASS(String/멀티모달 text블록/schema없음/user없음) + 회귀(ClaudeCliAdapterTest 18 / SubagentRunner 20) PASS. 컴파일 PASS. 미배포·미커밋. | 신규 | High | (미배포) | ✅ 구현+단위 12 PASS+회귀 PASS, 배포·커밋 대기 |
| CR-114 | 정상 완료(success) 경로 CLI worker 결정적 정리 — run 종료 시 좀비 누수 차단 — **배경**: CR-109(timeout 시 worker 정리) 후속 운영 추적에서 agent 컨테이너에 8시간 생존한 좀비 worker(0cb2ae03)의 정체 확정. **사용자 진단**: `cancelIfTimeout`(CR-109, ClaudeCliAdapter:100/121)은 timeout/실패 catch 경로에서만 cancel 송신, **정상 완료(예외 없이 return) 경로엔 worker 정리 호출 0건**. CLI 자율주행 AGENT_CALL 이 completed 로 끝나면 worker 가 아무도 안 닫혀 pool 에 영구 생존(firstTurnDone+pool 잔류, completed run 은 재호출 안 됨 → 영구 좀비). **CR-109 와의 경계(사용자 정리)**: CR-109=실패/timeout 분기의 catch→shutdownForRun(닫힌 CR, 커밋 d76ed57). CR-114=정상 완료 분기 — CR-109 가 손대지 않은 다른 경로. 증상(좀비)은 같으나 원인 경로 다름(timeout vs completed). **실측 확정(2026-06-18 소스 직접 확인)**: ①worker pool key=`runId`(=`request.sessionId()`=`childSessionId`), `ClaudeCliWorkerPool.runs` 맵 등록(ClaudeCliWorkerPool.java:107), 정상 정리 메서드는 `shutdownForRun(runId)` 1개(:257). ②`RunnerService.runTurn` 은 catch(예외)에서만 shutdownForRun(:114-137), 정상 완료 정리 0건+TTL/idle timeout 없음. ③WorkflowEngine finally 의 `shutdownCliWorkers(workflowRunId)` 는 **no-op**(WorkflowEngine.java:95-97, CR-071) 껍데기, 게다가 WorkflowEngine 의 runId=workflowRunId ≠ worker key=childSessionId(불일치)라 거기선 worker key 를 모름. ④`SubagentRunner.executeAgent` 는 orchestratorEngine.chat() **1회 호출 후 반환**(:335) — 한 AGENT_CALL=한 chat=한 worker, 멀티턴 재사용 없음(재사용은 timeout retry 시 deterministic childSessionId `--resume` 경로뿐, 정상완료 정리와 무관) → 정상완료 직후 childSessionId cancel 충돌 없음. ⑤cancel 부품 이미 존재: `ClaudeCliRunnerClient.cancel(endpoint,runId,runnerApiKey)`(:163)→RunnerService.cancel→shutdownForRun(:76-86)→POST /v1/cancel(RunnerController:140-153). **사용자 결정**: 정리 레이어=**SubagentRunner 스텝 단위 정리**(worker key=childSessionId 를 정확히 아는 유일한 지점). CR-109 와 동일 "부품 다 있고 호출만 누락" 패턴(연결 작업 1건). **구현(2026-06-18)**: 3파일. ①`LLMAdapter` default `cleanupSession(sessionId)` no-op 추가(worker pool 없는 어댑터는 자동 무시, 분기문 불필요). ②`ClaudeCliAdapter.cleanupSession` 오버라이드 — `resolveEndpoint()`+`runnerClient.cancel(endpoint,sessionId,runnerApiKey)`, CR-109 `cancelIfTimeout` 이 쓰던 cancel 신호를 정상완료 경로에서 공유(best-effort). ③`SubagentRunner` 에 `ConnectionAdapterFactory` 주입 + `executeAgent` completed 반환 직전 `cleanupCliWorker(req, childSessionId)` 호출. **안전장치 3겹**: (a) `resumeSessionId` 있으면 정리 skip(timeout retry `--resume` 이어하기 워커 보호) (b) `connectionId` null/blank 면 skip(modelRouter 라우팅=CLI 아님, worker pool 자체 없음 — getAdapter computeIfAbsent NPE 도 회피) (c) CLI 외 어댑터는 default no-op. best-effort try/catch 로 정리 실패가 결과 반환 막지 않음. **테스트**: 신규 `SubagentRunnerCr114Test` 4 PASS(정상완료+connectionId→cleanup 호출 / resume→미호출 / connectionId 없음→미호출 / CLI 외 어댑터 no-op 통과) + 기존 4테스트 생성자 ConnectionAdapterFactory 주입 회귀(SubagentRunner 12 / AgentOrchestrator 4 / AgentCallStepExecutor 9 / CR093 8 / ClaudeCliAdapter 18) = 51 PASS, failures/errors 0. 컴파일 PASS. **운영 배포(2026-06-18, deploy.sh be)**: platform-core bootJar→운영 jar 교체→api 컨테이너 재빌드/재기동. api Up(healthy), BE health UP(masterDataSource UP), 기동 로그 정상(Server MCP 42 tools / ToolRegistry 48 builtin). agent 는 cancel 엔드포인트(POST /v1/cancel) 이미 보유(CR-109)라 미배포. **운영 위생**: agent 컨테이너(aimbase-aimbase-agent-1) claude/node 프로세스 확인 → 0건(좀비 이미 소거 상태, 추가 kill 불요). 잔여=긴 turn 워크플로우 정상완료 후 worker 0건 유지 운영 e2e(다음 워크플로우 실행 시 자연 확인), 미커밋. | 신규 | Medium | (운영 라이브) | ✅ 구현+단위 4+회귀 51 PASS+배포+기동 검증, 커밋 대기 |
| CR-115 | run 단건 조회에 진행 단계 사람이 읽는 이름·진행 목록 추가 — **배경**: 소비앱(bidding)이 공고분석 워크플로우 진행 중 "어느 단계인지"를 화면에 표시하려 하나, run 조회 응답의 `currentStep` 은 step **id**(개발용 식별자, 예 `verify_and_gapcheck`)라 그대로 못 씀. WF 정의의 사람이 읽는 step `name`("분석 결과 검증 중")이 필요. **실측(2026-06-18 소스 직접 확인)**: run 조회 3개 엔드포인트(`WorkflowController` getRun/getRunById/runs)가 `WorkflowRunEntity` 를 raw 직렬화 → `currentStep`(id)만 노출. step `name` 은 WF 정의 `WorkflowEntity.getSteps()`(`List<Map>` 각 원소에 `id`/`name`/`type` 키, 운영 DB `aimbase_bidding_system.workflows` 실측 확인)에 존재. **구현(단건 조회 2종만)**: 신규 `WorkflowRunDetail` record(기존 run 필드 미러링 + `currentStepName` + `steps[]`) + `StepProgress{id,name,status}` 중첩 record. `toRunDetail()` 헬퍼 — WF 정의에서 `currentStep` id 로 `name` lookup, status 도출(`stepResults` 키 존재→completed / `currentStep` 일치→run terminal 이면 종료 상태(completed/failed/cancelled) 아니면 running / 그 외 pending). WF 정의 미존재 시 `currentStepName=null`·`steps=[]` graceful. 적용 엔드포인트=`GET /workflows/{id}/runs/{runId}`·`GET /workflows/runs/{runId}` 2종만 반환타입 교체(목록 `/runs`·SSE·approve·cancel·run 무변경). **하위호환**: 기존 필드(`currentStep`/`stepResults`/`status`/`startedAt` …) 그대로 유지, 신규 필드만 추가. 전체 step 목록은 소비앱이 `GET /workflows/{id}`(WF 정의) 1회 캐싱으로도 처리 가능하나 진행바 편의 위해 steps[] 동봉. **DB 스키마/마이그레이션 무변경**(인메모리 lookup, WF 정의 1회 조회). 가이드 API v3.12.0(§ 4-5 진행 단계 표시 + 응답 예시). | 신규 | Low | (미배포) | 📝 구현+가이드 완료, 빌드 검증·커밋·배포 대기 |
| CR-116 | 워크플로우 run 강제 취소 + 재실행 (좀비 worker hang 자력 복구) — **배경**: 운영 중 bidding_system run a614183f extract_facts(AGENT_CALL) 가 8분+ "진행 중" 멈춤. **실측(2026-06-18)**: worker 가 `[CLI-STDIN] textLen=31544` 전송 후 `[CLI-INIT]` 한 번도 못 찍고 정확히 600s 침묵→"turn timeout after 600s"→shutdown→즉시 재spawn **무한 반복**. 정상 worker 는 STDIN 후 1초 내 CLI-INIT(mcp connected). **같은 환경 CLI 직접 실행은 say hi/31KB 둘 다 3~5초 성공**(MCP connected, rate_limit allowed, CLI v2.1.143 정상) → 환경/입력크기/rate limit/resume/락 전부 반증. claude 프로세스 9개 잔존(2개 9시간 좀비, timeout shutdown 로그는 찍히나 프로세스 안 죽음). 직전 3 run 모두 completed → **간헐적**(환경 영구손상 아님). **근본원인**: 미완전 확정 — worker vs 직접 경로 유일 차이=여러 worker 동시생존(좀비 누적), project_opportunity_timeout_env_pollution 결론과 일치하나 메커니즘 미확정. **간헐 1% 근본차단 못할 수 있음을 전제로 "걸렸을 때 자력 복구" 장치가 본 CR 목표** (사용자 발화: "100번중 1번 발생해도 취소하고 다시할 수 있는 메커니즘이 있어야"). **현재 cancel 한계(실측)**: `POST /workflows/runs/{runId}/cancel`(WorkflowController:150, 소비앱 이미 사용) → cancelRun running 분기=`requestCancel` 표식만 → 백그라운드 DAG/cyclic 루프가 **스텝 경계(WorkflowEngine:630/1033)에서만 isCancelRequested 검사** → hang worker(600s) 대기로 스텝 안에 막히면 표식 확인 기회 없어 cancel 즉효 안 됨. worker kill 부품 존재(ClaudeCliWorkerPool.shutdownForRun:257, SubagentRunner future.cancel(true):210)하나 cancel 신호 전파 안 됨. **요구**: ①**강제취소(force) — 기존 cancel API 강화**(소비앱 쓰는 같은 엔드포인트에 force 경로, 별도 엔드포인트 신설 금지): 협조적 안 먹는 hang 시 해당 run CLI worker 즉시 kill+run terminal 처리. ②**재실행**: cancelled/failed run 같은 입력 재실행(FE 버튼+소비앱 연동 API). ③(선택)**hang 자동 손절**: worker N초간 CLI-INIT/토큰 0이면 600s 안 기다리고 자동 kill+재시도. 우선순위 1→2→3. **즉시 복구 조치(본 대화)**: run a614183f→DB cancelled 표식, claude 좀비 9개→0(컨테이너 pkill TERM→KILL), 정리 후 CLI 2.9초 정상 검증. origin=docs/origins/원본_요구사항_CR116_강제취소_재실행_좀비복구_20260618.md | 신규 | Medium | (미착수) | 📝 발번+origin 저장 완료, 설계 대기 |
| CR-117 | AGENT_CALL 빈 응답 자식 감지 + 자식 재시도 + **대용량 PDF 32MB 회피(FOREACH 자동 페이지 분할)** — **★진범 확정(2026-06-27 후속 실측)**: 빈 응답은 hang/지연이 아니라 **API 단일요청 32MB 한계 초과를 CLI 가 `is_error:true(subtype=success)` 로 둔갑**한 것. run 39ccf645 body[3]=`As_built__B949__1.pdf`(169p·18.8MB) 가 한 turn 에 1-20…81-100 = 100p 이미지 누적(개별 20p≈3.6MB 안전, 5청크 누적이 32MB 초과). 로그 `Request too large (max 32MB)`. **인과**: 로컬 CLI 직접 재현 시 청킹 지시 0 이면 CLI 가 **스스로 40p 에서 멈춰 정상 응답**(62초) — 자율은 적정선 정지. 그런데 우리 프롬프트가 *"마지막까지 20p 씩, 건너뛰지 말 것"* 완주 강제 → CLI 자율 정지 깨고 100p 적재. 부분 누락(자율 정지 시 뒷부분=환각) 막으려다 100p 32MB 로 통째 빈 응답. **본질**: 한 에이전트는 32MB 안에 169p 못 담음 → **유일 출구=169p 를 여러 자식이 분담 완주**. **근본 해결(사용자=aimbase 플랫폼화)**: (B) FOREACH 가 파일 받으면 `pdf_page_count`+페이지별 용량추정→**용량균등 청크(자식당 ~15MB)로 자동 분할**→청크당 자식 1개 완주→파일 단위 병합(verify 가 파일별 fact 수신). 분할 기준=페이지수(X)→용량(O). (A) `PdfVisionResolver` 한 turn 누적 바이트 가드(안전벨트, 초대형 도면 재발 차단). **부품 존재**: `PdfVisionResolver`(CR-095) pdfPageCount/pdfToImages/NEEDS_SPLIT 다 있음, 막힌 곳=누적 끊기+용량균등 청크 산출. **on_item_error 추가발견**: FOREACH config `continue` → 실패 자식 status=failed 마킹만 step 은 completed → **169p fact 누락된 채 "완료" 둔갑**(사용자 우려 현실화), continue→fail/실패율 임계치 필요. 은폐층 `--resume` 재시도는 32MB 엔 무효(같은 100p 재전송) → 근본층(B) 선행 필수. **배경(원)**: bidding_system FOREACH(extract_facts, body.type=AGENT_CALL, parallel) 실행 가시화 작업 중, 도구(PDF 분할 적재)는 전부 성공했으나 그 뒤 **LLM_RESPONSE(추출 fact 텍스트)가 영영 안 오는 자식**이 **빈 결과로 성공 완료** 처리되는 현상 발견. 사용자 요구: "응답이 없는 경우 재개/재시도하게 해야 한다"(빈 결과를 후속 스텝에 흘리지 말 것). **실측(2026-06-27, psql+소스)**: 완료 run 413bb470 의 body[3]/body[5] 가 마지막 TOOL_RESULT(ok:true) 후 LLM_RESPONSE 0건, 부모 STEP_END(17:15:54)까지 ~6분 무응답 종료. 응답 있는 body[4] 는 마지막 도구 6분 뒤 LLM_RESPONSE 정상 도착 → **"응답 지연" ≠ "응답 부재"**. BE 가 빈 자식을 실패로 인지 못 해 run completed. **근본원인 3지점(소스 실측)**: ① `SubagentResult.isSuccess()`=`status==COMPLETED && exitCode==0` 만 검사 → output 비어도 성공(SubagentResult.java:49). ② `AgentCallStepExecutor.executeSingleAgent`(AgentCallStepExecutor.java:83-86) 가 FAILED/TIMEOUT 만 예외 승격, COMPLETED+빈 output 은 검사 없이 toResultMap 정상 반환. ③ `ForeachStepExecutor.runOne`(ForeachStepExecutor.java:251) 이 `bodyExecutor.execute()` 직접 호출 → WorkflowEngine.executeWithRetry(retryMaxAttempts) 우회 → **자식 단위 재시도 구조적 불가**(부모 재시도 시 12개 전체 재실행 낭비). **부품 존재(배선만)**: CLI `--resume` 이어하기(SubagentRunner.java:122-126 resumeSessionId→childSessionId 재사용, 현재 단일 AGENT_CALL CR-106 만 enableResume=true) + executeWithRetry(WorkflowEngine.java:975). **요구(범위)**: ①빈 응답 감지(COMPLETED 여도 output+structuredData 비면 실패 판정, `require_text_output` 기본 true 로 도구-only 에이전트 오탐 방지) ②자식 단위 재시도(빈 자식만 `--resume` 이어하기, 부모 전체 재실행 금지, resumeSessionId 에 자식 step_id `extract_facts.body[3]` 결정적 키) ③재시도 상한 후 FAILED 확정(CR-111 "B 값유실" 차단) ④FE "모델 응답 대기 중…"/"도구 실행 중…" 문구+빈 응답 경고 배지. 우선순위 1→2→3→4. **CR-116 과 경계**: CR-116=worker hang(600s 무응답)→강제취소+run 재실행(run/worker 레벨), CR-117=자식 정상 종료했으나 텍스트 0→자식 재시도(step 레벨). 보완 관계, 합치지 않음. **설계 시 추가 실측 필요(미해결 2)**: "빈 응답=실패" 판정 기준(output 0 만? structuredData 도?) / 빈 응답이 hang 인지 정상 종료인지(ClaudeCliWorker 레벨 "도구 후 텍스트 0 turn 종료" 수신 방식, CR-112 is_error 둔갑 인접). origin=docs/origins/원본_요구사항_CR117_빈응답_자식_재시도_20260627.md | 신규 | Medium | (구현+배포) | ✅ 구현·배포·운영 e2e(2026-06-27). **착지 변경**: 근본 32MB 회피는 FOREACH 정밀 페이지분할(B)이 아니라 **"에러 기반 더 잘게 재시도"** 로 확정 — 분할은 ① 페이지당 용량 추정이 5배 빗나감(샘플 페이지 무거움) ② 청크당 CLI spawn 으로 느림(169p→22 CLI) ③ Read 1회 20p 한계와 청크크기 불일치로 재분할 중복 → **과잉설계 철회, PdfPageSplitter 삭제**. 대신 32MB 실패(`Request too large`, is_error 둔갑)를 `isPayloadTooLargeFailure` 로 감지 → 재시도 프롬프트에 "한 turn 5~10p 만, 몰아읽기 금지" 힌트 주입(fresh session, resume 안 함=100p 누적 컨텍스트 회피) + FOREACH `item_retry_max`. **유지된 것**: 은폐층(require_text_output 기본 true / 자식 item_retry_max / max_failed_ratio) + FE("모델 응답 대기 중…"/"도구 실행 중…"+빈응답 배지, TOOL_CALL 자식 오탐 가드=isAgent 자식만). **곁다리 정리**: builtin_file_read CLI 노출 제거(V67). **잔여→CR-118**: builtin_* 8개 CLI 노이즈 근본 정리(워크플로우별 도구 필터). 커밋 대기 |
| CR-118 | AGENT_CALL 스텝 워크플로우별 도구 필터(ToolFilterContext) 배선 — **발단**: CR-117 작업 중 CLI 어댑터가 `builtin_file_read` 를 "No such tool available" 로 반복 호출(allowedTools 에 없는데 builtin_* 시리즈 추론) → Read 폴백 중복 노이즈. **실측(2026-06-27 소스)**: 워크플로우 AGENT_CALL 도구 노출=2단계 — ①전역 `mcp.cli-exposed-tools`(master global_config) ②요청별 `ToolFilterContext`(allow/exclude+readOnly/permission/capability, ToolRegistry.getToolDefs:91-140). **②메커니즘은 이미 존재하나 AGENT_CALL 이 안 씀**: SubagentRunner.buildToolFilter(:574)가 AgentType(GENERAL→null=무제한)만 보고 **AGENT_CALL config 의 도구 목록을 안 읽음**. 워크플로우가 "이 에이전트는 Read/parse_document/pdf_* 만" 처럼 좁힐 방법 없음. **요구**: ①AGENT_CALL config 에 `tools`/`exclude_tools`/`read_only` 추가→AgentCallStepExecutor.buildRequest 가 ToolFilterContext 변환→SubagentRequest 전파 ②SubagentRunner 가 config 필터+AgentType 필터 병합(교집합) ③getToolDefs(filter)→CLI --allowedTools 자동 반영(CR-104 불변식). **동기**: 전역 builtin_* 제거는 타 워크플로우/SDK agent 영향 → 워크플로우별 필터가 정답(extract_facts 만 필요 도구 노출, builtin_* 혼란 그 WF 한정 제거). **CR-117 과 별개**(노이즈는 Read 폴백으로 동작은 함, 32MB 무관). **부품 존재(배선만)**: ToolFilterContext/getToolDefs(filter)/SubagentRequest.toolFilter 전파(CR-093) 완성, 막힌 곳=config→filter 변환+병합. 사용자: "원래 의도가 이 방식인데 안 되어있으면 별도 CR, 다른 세션 작업". origin=docs/origins/원본_요구사항_CR118_AGENT_CALL_워크플로우별_도구필터_20260627.md | 신규 | Medium | (미착수) | 📝 발번+origin 저장 완료, 다음 세션 작업 |
| CR-119 | 32MB API 거부의 timeout 둔갑 차단 (스트림 경로 CR-112 누락 보완) — **문제(운영 run 4185860e item[3] 실측)**: 35MB 도면(As_built_2, 81p)이 빈 응답으로 끝나고 다음 스텝 진행. 근본=32MB API 거부가 **timeout 으로 둔갑** → 무한 resume. CLI 가 `Request too large (max 32MB)` 를 `is_error:true(subtype=success)` result 로 둔갑 발행(CR-112 패턴). **비스트림**(`ClaudeCliWorker:594`)은 CR-112 로 is_error→예외승격 처리됨. **스트림**(`ClaudeCliRunnerClient.handleStreamEvent:482` result 분기)은 is_error 를 안 봐서 정상처리 → result 못 받고 stream 끊김 → `:167` "turn truncated…likely agent async timeout" 으로 **새 메시지 생성=32MB 정보 소실** → `ClaudeCliAdapter:111` `cli_runner_unreachable:…timeout` → AgentCallStepExecutor 가 `isTurnTimeoutFailure=true`/`isPayloadTooLargeFailure=false` 판정 → **resume**(같은 35MB 재전송) → 32MB 무한반복 → 빈 응답. **수정 3곳(BE만)**: ①`handleStreamEvent` result 분기에서 `is_error=true` 면 errText(evt.error/message/result) 보존해 `RuntimeException("CLI result reported is_error=true …")` 승격 — CR-112 를 스트림 경로에 이식 ②stream 루프 try 분리 — parse 예외만 catch→continue, 의도적 승격은 전파(★안 하면 ①이 기존 catch(Exception) 에 먹히는 함정) ③`AgentCallStepExecutor.buildRequest` resume 조건에 `!tooLarge` 추가 → 32MB 면 timeout 단어가 섞여도 resume 금지(fresh+힌트). resume 은 같은 파일 재전송이라 32MB 무한반복의 독. **CR-112 와 경계**: CR-112=비스트림 경로, CR-119=스트림 경로(누락분). **누적 32MB 근본해소는 CR-120**. 커밋 c9a23cf. origin=docs/origins/원본_요구사항_CR119_32MB둔갑_timeout오분류_스트림경로_20260627.md | 신규 | Medium | (운영 라이브) | ✅ 구현+배포+운영검증+커밋(2026-06-27) |
| CR-120 | 대용량 문서 전수 분석 엔진 (LARGE_INPUT 스텝) — **목적**: 자율주행(에이전트 자율 판단) 밖의 **결정론적** 파이프라인 — 청크 분해 → map(청크별 독립 처리) → 계층 Reduce → 누락 검증. 32MB 는 API 물리 한계라 **애초에 안 치게** 설계(CR-117/119 의 사후 대응과 층이 다름). 분석 동작(extract/summarize/verify)은 코드+DB 프롬프트로 무한 확장 가능한 부품. **★구현 중 착지 변경**: "동적 FOREACH 위임" **철회 → 엔진 자체 map 루프**. 실측 근거 = `LlmCallStepExecutor.callLlm`(L586-590)이 텍스트 메시지만 조립(`ofText` USER 1건)이라 이미지형 청크의 base64 페이지를 LLM_CALL body 에 못 넣음 → 멀티모달은 `UnifiedMessage.ofUserContent([ContentBlock.Image.ofBase64])` 직접 호출 필수. `ForeachStepExecutor.runParallel` 의 Semaphore+VT+TenantContext 재주입 패턴만 차용(private 라 호출 불가, 패턴 복제). **산출물**: enum `WorkflowStep.LARGE_INPUT` + `LargeInputStepExecutor` + `largeinput/`(LargeInputSource·Chunk, SourceLoader·PdfSourceLoader, DecomposerService, HierarchicalReduceService(BiFunction 콜백 주입으로 엔진과 분리), CoverageVerifier, Policy) + `workflow/analysis/`(DocumentAnalysis iface, Extract/Summarize/Verify impl, ActionRegistry) + `LargeInputJobEntity`/Repository + tenant 마이그 V67(테이블)·V68(프롬프트 6 seed) = 16 신설. FE: FOREACH body 중첩 비주얼 편집 + LARGE_INPUT 1급 노드. **성능(별건 3중첩, project_cr120_filepath_transport)**: 운영 bidding `opportunity-analysis` extract_facts 가 **83분→14분**. ①BE→사이드카 **전체 PDF base64 전송 제거**(file_path 운반, 8fd78aa) ②**사이드카 완전 탈피**(poppler 직접 exec — 판별/추출 pdftotext, 렌더 pdftoppm)로 38분 ③**extract reduce 코드 병합**(LLM 0회)으로 14분(219ff60) ④결과 이중 적재 제거 — output 은 프리뷰만, fact 본체는 structured_data(6fff128). **곁다리**: `execution_backend` 제거→connection_id 단일화, LARGE_INPUT 청크 진행 가시화("(본문 없음)" 해소), source_file 을 run 격리 작업장 기준 resolve, reduce 구조화 출력+custom_instruction 범용 통로. **단위 13 PASS**(ActionRegistry 5/CoverageVerifier 3/Policy 2/HierarchicalReduce 3). origin=docs/origins/설계_CR120_대용량문서_전수분석엔진_20260627.md | 신규 | High | (운영 라이브) | ✅ 구현+배포+성능검증(83분→14분)+커밋 |
| CR-121 | 원격 에이전트 도구가 `/mcp/sse` 에 노출되지 않던 구조적 누락 해소 — **발단**: bp-wes 소비앱이 등록한 원격 도구 8종(get_progress/get_bottleneck 등)을 CLI 러너가 못 씀. **근본원인(실측)**: `ServerMcpConfig` 가 도구 소스로 **Spring 빈 목록**(`List<ToolExecutor>`)을 쓰고 `ApplicationReadyEvent` 1회만 `addTool`. 그런데 원격 도구는 `RemoteToolDiscovery`(BIZ-080, 30초 주기)가 `new RemoteAgentToolExecutor(...)` 로 만들어 **`ToolRegistry` 에만 런타임 등록**하는 객체라 Spring 빈이 아니다 → **화이트리스트 추가로도 재기동으로도 절대 안 풀리는 구조**. **수정**: ①`ToolRegistry.isCliExposed()/getCliExposedTools()` 신설 — builtin 은 화이트리스트(`mcp.cli-exposed-tools`), 외부 MCP·원격 도구는 통과. `getToolDefs(filter)`(API 경로)가 쓰던 규칙을 공용 메서드로 승격해 양쪽 통일 ②`ServerMcpConfig` 소스를 ToolRegistry 로 교체 + 30초 주기 **양방향** 동기화(addTool/removeTool/notifyToolsListChanged) ③`ServerMcpToolDispatcher` 실행 게이트도 동일 판정 위임(★안 고치면 노출돼도 호출이 차단됨). **★기동 순서 함정(배포 1차에서 실측 발견)**: ServerMcpConfig 와 `ToolRegistry.registerBuiltins` 가 같은 ApplicationReadyEvent **동순위**라 순서가 비결정적 — 운영 로그상 노출이 ToolRegistry 초기화보다 **6ms 앞섬**. 그러면 `builtinNames` 가 비어 **모든 builtin 을 외부 도구로 오판 → 화이트리스트 우회**(parse_document 등 비노출 대상까지 실림). `isBuiltinsRegistered()` 플래그로 true 될 때까지 노출 연기(+`syncOnce()` 진입부 가드). **정정된 오해**: "자동 노출=통제권 포기"가 아니라, API 경로는 **이미** 원격 도구를 무조건 통과시키고 있었고 CLI 를 거기 맞춘 것 — CR-104/110 이 선언만 하고 원격 도구에선 깨져 있던 "CLI 집합=API 집합" 불변식을 실제로 성립시켰다. **검증**: 노출 **110개**(builtin 40 + rag-pipeline 42 + 외부MCP 20 + WES 원격 8), 내부전용 6종 유출 0, 동기화 에러 0. 커밋 227c524 | 신규 | Medium | (운영 라이브) | ✅ 구현+배포+운영검증+커밋(2026-07-21) |
| CR-122 | 테넌트 DataSource 기동 로드 실패 + 스케줄러 테넌트 컨텍스트 누락 해소 — **문제**: 온보딩 API 로 만든 테넌트(bp_wes/workmap)가 **재기동하면 DB 에 못 붙음**. **근본원인**: `TenantOnboardingService:167` 이 `db_password_encrypted` 에 **BCrypt 해시**를 저장(`passwordEncoder.encode`)하는데 `TenantDataSourceManager.createDataSource` 는 그 값을 **복호화 없이 그대로 접속 비밀번호로** 사용 → BCrypt 는 단방향이라 복원 불가 → `password authentication failed`. **"전엔 됐는데"의 정체**: 온보딩 직후엔 `addTenantDataSource(...)` 로 **평문이 인자 전달**돼 메모리에 정상 DataSource 가 올라가고, 재기동하면 사라져 다시는 안 붙는다. **연쇄 피해**: `catch` 가 루프 **바깥**이라 6번째 테넌트 실패가 이후 로드를 통째 중단 → bp_wes/workmap 누락 → `RemoteToolDiscovery` 가 해당 테넌트를 순회 못 해 원격 도구 동기화 정지(CR-121 효과가 안 보이던 원인). **수정**: ①접속 비밀번호 출처를 **`platform.default-db-password`** 설정값으로 통일 — 전 테넌트가 `db_username=platform` **한 계정 공유**이고 온보딩의 DB 생성/삭제/마이그레이션도 이미 이 값을 쓰고 있어(TenantOnboardingService 133·248) 일관성이 맞다. 호출자가 평문을 넘긴 경우(`db_password` 키)만 우선 ②try 를 루프 안으로 ③`AppDataSourceManager` 동일 적용. **같이 고친 스케줄러 2건**(테넌트 테이블인데 `TenantContext` 없이 조회 → master 로 라우팅 → 테이블 없어 실패): `AttachmentGcScheduler`(5분마다 `relation "chat_attachments" does not exist`, GC 전혀 미동작) / `ToolResultStorageService.expireOldResults`(동일 결함이나 catch 가 삼켜 **로그에도 안 남고** 매일 03:00 조용히 실패). ★순회 메서드에 `@Transactional` 금지 — 첫 테넌트 커넥션에 묶이면 라우팅이 어긋남, `TransactionTemplate` 으로 테넌트별 분리. 나머지 스케줄러 2개(MCPRagClient 헬스핑/LSPClientManager)는 DB 미사용. **검증**: 테넌트 **7/7** 로드(이전 5/7), bp_wes·workmap 최초 정상화, WES 원격 도구 8종 등록+노출, 스케줄러 예외 0. 커밋 e8b7264 | 신규 | High | (운영 라이브) | ✅ 구현+배포+운영검증+커밋(2026-07-21) |
| CR-123 | 커넥션 config 변경이 어댑터 캐시에 반영되지 않던 문제 해소 — **근본원인**: `ConnectionAdapterFactory` 가 **connectionId 만으로** LLMAdapter 를 캐싱(`cache.computeIfAbsent`). 어댑터는 생성 시점 config(tool_mode/api_key/model…)를 필드로 굳히므로 캐시가 살아있는 한 DB 변경이 영영 반영 안 됨. `evict()` 는 **`ConnectionController.update()`(API 경로)에서만** 호출 → **DB 직접 UPDATE 는 무효화 경로가 아예 없었다**. **운영 실측**: `connections.config.tool_mode` 를 NATIVE→AIMBASE 로 바꿨는데 캐시된 NATIVE 어댑터가 계속 쓰여 러너가 `toolMode=NATIVE` 로 spawn → CLI 가 MCP 미연결이라 도구를 못 찾고 ToolSearch/Bash 로 우회하다 실패. **수정(정본)**: 캐시 키를 **`tenantId::connectionId::updatedAt`** 으로 구성 — DB 가 바뀌면 키가 달라져 자동으로 새 어댑터 생성, 호출자가 evict 를 기억할 필요 없음. 낡은 버전 엔트리는 접두사 매칭 제거(무한 증식 방지). 테넌트 접두사는 Database-per-Tenant 교차 오염 차단. `evict()` 도 접두사 매칭으로 변경(이제 필수가 아닌 최적화). **★진단 중 오판 2건(재발 방지 기록)**: ①"커넥션 캐시 없음" — `@Cacheable`/`@CacheEvict` 만 grep 해서 놓쳤다, 실제로는 **수동 ConcurrentHashMap** 캐시(캐시 유무는 `Map<.*>`+`computeIfAbsent` 도 확인할 것) ②"러너 재기동이 답, BE 재기동은 무의미" — **반대**였다, 워커 수명이 아니라 BE 캐시가 원인(러너 재기동 후에도 NATIVE 로 뜬 로그가 반증). **곁다리**: `docker-compose.prod.yml` — `aimbase-agent-wes` 의 MCP 인증키가 bidding_system 키를 공유해 `AGENT_TENANT_ID`(bp_wes)와 테넌트가 어긋나 있어 bp_wes 전용 키로 분리(★axopm/workmap 러너도 동일 구조 — 후속 점검 필요). **검증**: `ConnectionAdapterCacheTest` 4 PASS(변경시 재생성/무변경시 재사용/낡은버전 제거/테넌트 격리), 배포 후 `toolMode=AIMBASE` 전파 확인. 커밋 7171881 | 신규 | Medium | (운영 라이브) | ✅ 구현+배포+운영검증+커밋(2026-07-21) |
| CR-124 | **MCP SSE → Streamable HTTP 전환** (프로토콜 `2025-11-25` 협상 실패 해소) — **발단**: CR-121/122/123 을 전부 해소했는데도 CLI 가 도구를 못 받는 증상이 남아 추적. **근본원인(운영 로그 실측)**: `McpAsyncServer : Client requested unsupported protocol version: 2025-11-25, so the server will suggest the 2024-11-05 version instead`. CLI 2.1.197 이 `2025-11-25` 를 요구하는데 서버가 못 줌 → 핸드셰이크 미완료 → `status=pending` 고정, 도구 0개(5회 전부 재현되는 결정적 실패). **★발번 시 전제가 틀렸음(정정)**: "SDK 0.17.0→2.0.0 업그레이드"로 풀린다고 봤으나 **2.0.0 만으로는 해결되지 않는다**. 바이트코드 실측 — `HttpServletSseServerTransportProvider.protocolVersions()` 는 **2.0.0 에서도 `["2024-11-05"]` 단일 하드코딩**이고, `HttpServletStreamableServerTransportProvider` 만 오버라이드 없이 기본값 4종(`2025-11-25` 포함)을 상속한다. SDK 만 올리고 SSE 를 유지하면 증상이 100% 재현되며 오히려 협상폭이 3종→1종으로 **축소**된다. 즉 조건은 SDK 버전이 아니라 **transport 교체**. 부수적으로 `mcp-spring-webmvc` 는 deprecated 가 아니라 **2.0.0 에 아티팩트 자체가 없다**(0.18.3 에서 중단) → SSE 유지가 물리적으로 불가. **CLI 실측 프로브**: `"type":"http"` 로 붙이면 `2025-11-25` 협상 성공 + 도구 인식, `"type":"sse"` + 서버 `2024-11-05` 강제는 도구 0개로 **운영 증상 그대로 재현** → 원인 확정과 해결책 검증을 동시 완료. CLI zod 유니온 `["stdio","sse","sse-ide","http","ws","sdk"]` 에서 `http` 가 Streamable. **변경**: mcp-bom 2.0.0 + `mcp-spring-webmvc` 제거 / WebMvcSse→`HttpServletStreamable`(ServerMcpConfig·AimbaseAdminMcpConfig·AgentMcpServer) / RouterFunction 빈→`ServletRegistrationBean`(2.0.0 provider 가 `HttpServlet` 상속체) / `McpJsonMapper.createDefault()`→`McpJsonDefaults.getMapper()` / 도구 핸들러 `(exchange, Map)`→`(exchange, CallToolRequest)` + `request.arguments()` / `Tool.inputSchema`·`CallToolResult` 를 Map·builder 로 이관 / 러너 config `"type":"sse"`→`"http"`, `/mcp/sse`→`/mcp`. **★구현에서만 드러난 추가 변경**: `McpStreamableServerTransportProvider` 가 `McpServerTransportProvider` 와 **별개 인터페이스**라 `McpServer.sync()` 오버로드가 갈려 stdio/Streamable 공용 헬퍼가 불가 → `AgentMcpServer.buildMcpServer` 를 "도구 스펙 목록"과 "서버 조립"으로 분해. **★연쇄 수정(조사 단계에서 못 본 것)**: ①`RemoteAgentToolExecutor` — BE→agent 호출이 `/mcp/sse` 하드코딩이라 agent 를 Streamable 로 바꾸는 순간 **단절** → `"streamable"`+`/mcp` ②`MCPServerClient` — SSE 전용이라 streamable 분기 + `HttpClientStreamableHttpTransport` + `/sse` 미부착 `splitBaseAndPath()` 추가. **Python 사이드카(rag/safety/evaluation)는 SSE 유지** — 기존 동작 보존 테스트 포함 ③`McpTenantSessionFilter` — `/admin-mcp/sse`·`/message` 두 하위경로 문자열에만 반응해 Streamable 단일 엔드포인트에서 **테넌트 전파가 예외 없이 조용히 끊김** → `/admin-mcp` 전체 대상으로 변경 + 요청 종료 시 `TenantContext` 정리(스레드 재사용 누수 차단). **Spring Security 는 변경 불필요** — `"/mcp/**"` 매처가 하위경로 없는 `"/mcp"` 를 이미 매칭(실측 후 테스트로 고정). **검증**: e2e — 임베디드 Tomcat 기동 + MCP 클라이언트 핸드셰이크로 `2025-11-25` 협상·tools/list·tools/call 성공(`Cr124E2eHandshakeTest`), "SDK 만 올리면 안 됨"을 테스트로 고정(`Cr124ProtocolNegotiationTest`). 회귀 platform-core 922 / tool-sdk-mcp 32 / cli-runner 45 = **0 fail** (aimbase-agent 6 fail 은 손대지 않은 HEAD 에서도 동일 재현되는 사전 결함 — Mockito 스텁 NPE). **운영 검증**: `initialize`→`{"protocolVersion":"2025-11-25"}`, `tools/list`→**110개**(get_progress/get_bottleneck 포함), `unsupported protocol` 경고 소멸. **★배포 함정 2건**: ①`deploy.sh agent` 는 러너 4개 중 **1개만** 재기동(axopm/workmap/wes 누락) → `agents`/`mcp` 타겟 신설, MCP 변경은 `./deploy.sh mcp`(BE+러너 전체 연속) 필수 — 프로토콜 비호환이라 단계 배포 불가 ②배포 직후 tools/list 는 **102개**로 나온다. 원격 도구 8종은 agent 재등록(~+45s) → CR-121 30초 주기 동기화를 거쳐 **~90초 뒤 110개** — 성급히 재면 누락으로 오판. 커밋 99c6d82(코드) + 8a9a2d3(deploy.sh). origin=docs/origins/원본_요구사항_CR124_MCP_SDK_2_0_0_프로토콜_20260721.md | 신규 | High | (운영 라이브) | ✅ 구현+배포+운영검증+커밋(2026-07-22) |
| CR-125 | admin MCP 테넌트 컨텍스트가 전역 단일 필드라 동시 사용 시 교차 오염 — **발견 경위**: CR-124(SSE→Streamable 전환) 중 `McpTenantSessionFilter` 를 고치다 확인된 **기존 결함**(CR-124 가 만든 것이 아님). **문제**: `McpTenantSessionFilter.currentMcpTenant` 가 **`private static volatile String`** 단일 필드다. admin MCP 는 URL 쿼리(`?tenant_id=X`)로 테넌트를 받아 이 필드에 저장하고, 이후 도구 호출이 `getCurrentMcpTenant()` 로 읽어 `TenantContext` 에 심는다. 따라서 **서로 다른 테넌트가 admin MCP 를 동시에 사용하면 나중 연결이 앞선 값을 덮어써**, 앞 사용자의 도구 호출이 **다른 테넌트 DB 로 라우팅될 수 있다**(Database-per-Tenant 격리 위반, BIZ-003). 단일 사용자 운용에서는 드러나지 않아 지금까지 표면화되지 않았다. **방향(안)**: MCP 세션 단위 보관으로 전환 — Streamable 은 `Mcp-Session-Id` 헤더로 세션이 식별되므로 `sessionId→tenantId` 맵으로 바꾸고 요청마다 해당 세션의 테넌트를 사용. 세션 종료 시 엔트리 제거. 대안으로 `X-Tenant-Id` 헤더 기반 요청 단위 결정(쿼리 파라미터 의존 제거)도 검토. **참고**: CR-124 에서 요청 종료 시 `TenantContext` 정리는 이미 넣어 **요청 간 누수**는 차단됨 — 남은 것은 **연결 간 덮어쓰기**. **영향**: `platform-core/src/main/java/com/platform/mcp/admin/McpTenantSessionFilter.java`, `AimbaseAdminMcpConfig`(도구 핸들러의 tenant 조회부) | 신규 | Medium | (미착수) | 📝 발번 — CR-124 중 발견, 별도 처리 |
...
### CR-095 | PDF Native 비전 파싱 (openclaude 1:1 포팅 — 텍스트 추출 대신 모델 비전)
- **대상 기능 ID**: BIZ-061(위젯 PDF 첨부), CR-094(ParseDocumentTool), BIZ-105~107(CR-092 OCR)
- **변경 타입**: 신규
- **변경 내용**:
  - PDF를 사이드카 텍스트 추출(unstructured/OCR, 건당 ~38초 직렬)이 아니라 **LLM API 안에서 모델 비전으로 파싱**한다. openclaude FileReadTool 로직 1:1 포팅.
  - **크기 게이트**: ≤3MB & PDF 지원 어댑터 → base64 `document` block 통째 (모델이 PDF 직접 봄). >3MB or 미지원 → poppler(pdf2image, CR-092에서 이미 설치)로 페이지→JPEG 이미지화 → `image` block 배열.
  - **상수**: `PDF_EXTRACT_SIZE_THRESHOLD`=3MB / `PDF_MAX_PAGES_PER_READ`=20 / `PDF_AT_MENTION_INLINE_THRESHOLD`=10 / `PDF_TARGET_RAW_SIZE`=20MB → `application.yml` (`aimbase.pdf.*`) 외부화.
  - **사이드카**: `pdf_to_images(base64, pages?)` MCP 도구 신설 — `pdf2image.convert_from_bytes` → `{images:[base64...], page_count, dpi}`. `pages` 파라미터(예 "1-5") 지원(`parsePDFPageRange` 포팅).
  - **도구 결과 멀티모달 주입(핵심 구조 확장)**: openclaude 의 `newMessages`(isMeta:true user 메시지로 document/image block 을 컨텍스트 스트림에 별도 주입) 대응. `ToolResult` 에 `newMessages` 필드 추가 + `ToolCallHandler` 도구 루프가 이를 `mutableMessages` 에 주입 + `UnifiedMessage.ofUserContent(blocks)` 팩토리 신설. (현재 `ContentBlock.ToolResult.content` 는 String 만 → tool_result 자체엔 메타 텍스트만, 실제 PDF/이미지는 별도 user 메시지로.)
  - **두 진입점 분기**: ① `ChatController.resolveDocumentBlock` 3MB 게이트 추가 ② `ParseDocumentTool` PDF 면 사이드카 텍스트 대신 비전 경로 + newMessages 반환.
  - **비전 주체**: Anthropic 기본(이미 작동) + 로컬 Ollama 비전 옵션(`OllamaAdapter` IMAGE_ONLY + `images` 경로 구조 준비됨). **전제조건**: 로컬 비전 경로는 Ollama 에 비전 모델(llava/qwen2-vl 등) pull 필요 — 현재 텍스트 모델 6개뿐.
- **변경 사유**: 사용자 지시 — "parse_document(OCR/추출) 쓰지 말고 base64 document block 으로 통째로 모델에 전달. 큰 PDF 는 페이지 이미지화 분기." parse_document 직렬 OCR/추출은 느리고(~38초) 표·레이아웃·도표를 뭉갬. 모델 비전은 OCR 단계 없이 페이지를 그림으로 직접 이해 → 복잡한 문서 품질↑. openclaude 가 이미 이 모델(전부 LLM 비전)이라 1:1 포팅.
- **영향 모듈**: 사이드카(rag_pipeline/MCP 도구), AnthropicAdapter, OllamaAdapter, ToolResult(SDK core), ToolCallHandler, UnifiedMessage, ChatController, ParseDocumentTool, MCPRagClient, PdfTextExtractor(폴백 유지)
- **영향도**: High (도구 루프 멀티모달 주입 = 오케스트레이터 핵심 구조 확장)
- **영향 범위**: CR-061(위젯 PDF 첨부), CR-094(ParseDocumentTool), CR-092(OCR — 미지원 모델 폴백 시 공존), BIZ-001(도구 루프 5회)
- **영향 설계서**: T3-* (도구/어댑터 계열) — 후속 보정 대상
- **원본 요구사항**: `docs/origins/원본_요구사항_PDF_비전파싱_20260604.md`
- **요청자**: 사용자 | **승인자**: 사용자 | **적용 버전**: v8.18.0
- **변경 일자**: 2026-06-04 (발번) / 2026-06-04 (1차 구현·배포) / 2026-06-05 (페이지 가드 후속)
- **구현 상태**: ✅ 운영 라이브 (1차 비전 게이트 + 페이지 분할 가드 후속)
- **마이그레이션**: 불필요 — 상수는 application.yml, ToolResult.newMessages 는 코드 필드 (DB 스키마 무변경)
- **설계 결정 (확정)**:
  - 미지원 어댑터(supportsPdf=false) PDF: **페이지 이미지화 강제** (supportsImage 도 false 면 PdfTextExtractor 텍스트 폴백)
  - 로컬 Ollama 비전 모델 선택(한국어 문서 품질) — 별도 결정 (보류)
- **후속 (2026-06-05, 페이지 분할 가드)**: 1차 배포 후 13MB/29p PDF AGENT_CALL(extract_facts) **300초 turn timeout** 발견. 근본원인=openclaude `getPDFPageCount > PDF_AT_MENTION_INLINE_THRESHOLD(10)` 가드 포팅 누락 — 3MB 크기 게이트만 가져오고 옆의 10p 분할 가드가 빠져, >3MB PDF 첫 20p 를 한 턴에 통째 이미지화→거대 입력 timeout. 해결(openclaude 1:1): ①사이드카 `pdf_page_count` MCP 도구 신설(pdfplumber, 렌더 없이 페이지 수) + `pdf_to_images` `total_pages` 반환 ②`MCPRagClient.pdfPageCount` ③`PdfVisionResolver.resolve(..., pages, allowSplitGuard)` — pages 지정 시 가드 우회+범위 렌더, 미지정+가드 시 페이지 수 확인→임계(`aimbase.pdf.inline-page-threshold:10`) 초과면 **NEEDS_SPLIT**(이미지화 안 하고 "pages 로 나눠 호출하라" 안내, 모델 자율 분할) ④`ParseDocumentTool` `pages` 파라미터 스키마 + NEEDS_SPLIT→`too_many_pages` 반환. 첨부 경로(ChatController)는 1회성이라 `allowSplitGuard=false`(가드 미적용, 첫 20p+truncated note). 테스트: 사이드카 pdf_images 20 + ocr 26 회귀 + PdfVisionResolver 14(가드 5 추가) + ParseDocumentTool/mcp.server 회귀 GREEN
