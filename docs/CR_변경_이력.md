...
| CR-090 | 워크플로우 실행 가시성 — `workflow_run_events` 테이블 + 조회 API. 현재 BE 로그는 운영자가 `docker logs` SSH 접근 + DEBUG 토글 없이는 "어떤 도구를 어떤 input 으로 불렀고 무엇을 받았다" 흐름을 볼 수 없음 (INFO 레벨엔 막힌 케이스만, 정상 흐름은 DEBUG). `traces`/`subagent_runs`/`tool_result_storage` 에 raw 는 있으나 (1) TOOL_CALL 스텝의 직접 도구 호출은 어디에도 기록 없음 (2) 시간순 흐름 매끄럽지 않음 (3) AGENT_CALL 안 도구 iteration 메타 없음. **추가**: V64 마이그레이션으로 `workflow_run_events` (id BIGSERIAL, run_id UUID, step_id, iteration INT nullable, event_type[STEP_START/TOOL_USE/TOOL_RESULT/LLM_RESPONSE/STEP_END/STEP_FAILED], tool_name, duration_ms, payload JSONB[메타만], trace_id, subagent_run_id, created_at) + 인덱스 `(run_id, created_at)` + 부분인덱스 `(trace_id WHERE NOT NULL)`. **얇은 버전 원칙**: prompt/response 원본은 저장 안 함, `trace_id`/`subagent_run_id` 참조 키만 → 깊이 분석 필요 시 join. payload 는 메타만(input_keys, preview ≤100자, finish_reason 등). **hook in 5곳**: `WorkflowEngine` STEP_START/END/FAILED, `ToolCallStepExecutor` TOOL_USE+RESULT, `ToolCallHandler` (AGENT_CALL 도구 루프) iteration 별 TOOL_USE+RESULT+LLM_RESPONSE, `LlmCallStepExecutor` LLM_RESPONSE. **모두 async fire-and-forget** — 워크플로우 hot path 지연 없음. **API**: `GET /api/v1/workflows/runs/{runId}/events` (시간순). 소비앱이 화면에 Claude Code 처럼 한 줄 흐름 렌더링 가능. **소비앱 동기**: bidding-agency 가 opportunity-analysis 한글화 진행 중 "무슨 일이 일어나고 있는지 보고 싶다" → 사후 퀄리티 분석/디버깅에 사용. **약 1MD 추정** | 신규 | Medium | TBD | 📝 발번 |
