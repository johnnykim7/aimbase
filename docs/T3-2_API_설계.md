...
> 설계 버전: 2.4 | 최종 수정: 2026-03-29 | 관련 CR: CR-009, CR-010, CR-011, CR-012
...
| 메서드 | 경로 | 설명 | 인증 |
|--------|------|------|------|
| GET | `/memory` | 메모리 조회 (sessionId, layer 필터) | 🔒 |
| POST | `/memory` | 메모리 항목 추가 | 🔒 |
| DELETE | `/memory/{id}` | 메모리 항목 삭제 | 🔒 |
| GET | `/memory/user/{userId}` | 사용자 프로필 메모리 조회 | 🔒 |

### 메모리 조회

**GET /api/v1/memory?sessionId=xxx&layer=SHORT_TERM**
