...
> 설계 버전: 4.2 | 최종 수정: 2026-04-08 | 관련 CR: CR-009, CR-010, CR-011, CR-012, CR-031, CR-032, CR-033, CR-036, CR-038
...
## Sprint 46: Tool/Policy 확장성·자동화 (BE + Python + FE) [CR-035]

> **목표**: Cron 스케줄 자동화, 스킬 경량 실행, 도구 탐색, Firecrawl 웹 크롤링, 도메인 필터링 정책을 추가하여 에이전트의 자동화·확장성·보안 강화
> **Flyway**: V20 (Master DB), V44 (Tenant DB)
> **Phase 수**: 6

### 46-1. Cron 스케줄 엔진 + 도구 3종 (PRD-234, PRD-235) — Phase 1

| # | 작업 | 산출물 |
|---|------|--------|
| 1 | Flyway V20 마이그레이션 — scheduled_jobs 테이블 (Master DB) | `V20__cr035_scheduled_jobs.sql` |
| 2 | ScheduledJobEntity JPA 엔티티 생성 | `domain/ScheduledJobEntity.java` |
| 3 | ScheduledJobRepository 생성 | `repository/master/ScheduledJobRepository.java` |
| 4 | CronScheduleManager 구현 (Spring TaskScheduler 래핑, 서버 기동 시 active job 로드, TenantContext 내 실행) | `workflow/CronScheduleManager.java` |
| 5 | ScheduleCronTool 구현 (EnhancedToolExecutor) — Cron 작업 생성 | `tool/builtin/ScheduleCronTool.java` |
| 6 | CronListTool 구현 (EnhancedToolExecutor) — Cron 작업 목록 조회 | `tool/builtin/CronListTool.java` |
| 7 | CronDeleteTool 구현 (EnhancedToolExecutor) — Cron 작업 삭제 | `tool/builtin/CronDeleteTool.java` |
...
CREATE TABLE scheduled_jobs (
    ...
    target_type     VARCHAR(50) NOT NULL,  -- WORKFLOW | TOOL
...
### 46-2. ToolSearchTool (PRD-236) — Phase 2

| # | 작업 | 산출물 |
|---|------|--------|
| 1 | ToolRegistry에 searchTools(keyword, tags, capabilities) 메서드 추가 | `tool/ToolRegistry.java` 수정 |
| 2 | ToolSearchTool 구현 (EnhancedToolExecutor) — ToolContractMeta의 tags/capabilities/description 키워드 검색 | `tool/builtin/ToolSearchTool.java` |

### 46-3. SkillInvokeTool + DB (PRD-237) — Phase 3

| # | 작업 | 산출물 |
|---|------|--------|
| 1 | Flyway V44 마이그레이션 — skills 테이블 (Tenant DB) | `V44__cr035_skills.sql` |
| 2 | SkillEntity JPA 엔티티 생성 | `domain/SkillEntity.java` |
| 3 | SkillRepository 생성 | `repository/SkillRepository.java` |
| 4 | SkillService — 스킬 CRUD + 단일 LLM 호출 실행 | `tool/SkillService.java` |
| 5 | SkillInvokeTool 구현 (EnhancedToolExecutor) — 프롬프트+도구 조합 경량 실행 | `tool/builtin/SkillInvokeTool.java` |
| 6 | SkillController — 스킬 REST API | `api/SkillController.java` |
...
### 46-4. Python 사이드카 Firecrawl 어댑터 (PRD-238) — Phase 4

| # | 작업 | 산출물 |
|---|------|--------|
| 1 | firecrawl-py 의존성 추가 | `python-sidecar/requirements.txt` 수정 |
| 2 | scraper.py에 firecrawl 모드 추가 (Self-hosted 지원, API Key 미설정 시 js_render 폴백) | `python-sidecar/src/rag_pipeline/scraper.py` 수정 |
| 3 | Firecrawl 연동 단위 테스트 | `python-sidecar/tests/test_scraper_firecrawl.py` |

### 46-5. BE 지식소스 Firecrawl 모드 + DOMAIN_FILTER 정책 (PRD-239, PRD-240) — Phase 5

| # | 작업 | 산출물 |
|---|------|--------|
| 1 | KnowledgeSourceEntity에 crawl_mode 필드 추가 | `domain/KnowledgeSourceEntity.java` 수정 |
| 2 | IngestionPipeline에 Firecrawl 모드 분기 추가 | `rag/IngestionPipeline.java` 수정 |
| 3 | PolicyEngine에 DOMAIN_FILTER 규칙 타입 추가 (allowed_domains/blocked_domains, ALLOWLIST/BLOCKLIST 모드, 와일드카드 서브도메인) | `policy/PolicyEngine.java` 수정 |
| 4 | DOMAIN_FILTER 정책 평가 단위 테스트 | `PolicyEngineTest.java` 수정 |

### 46-6. FE 관리 UI (FE-017) — Phase 6

| # | 작업 | 산출물 |
|---|------|--------|
| 1 | API 클라이언트 함수 추가 (scheduledJobs, skills CRUD) | `frontend/src/api/scheduledJobs.ts`, `frontend/src/api/skills.ts` |
| 2 | useScheduledJobs, useSkills 훅 생성 | `frontend/src/hooks/useScheduledJobs.ts`, `frontend/src/hooks/useSkills.ts` |
| 3 | ScheduledJobs 관리 페이지 (모니터링 탭 포함) | `frontend/src/pages/ScheduledJobs.tsx` |
| 4 | Skills 관리 페이지 (CRUD + 실행 테스트) | `frontend/src/pages/Skills.tsx` |
| 5 | 정책 편집에 DOMAIN_FILTER UI 추가 (도메인 목록 입력, 모드 선택) | `frontend/src/pages/policies/PolicyEditor.tsx` 수정 |
| 6 | App.tsx에 라우트 등록 + Sidebar에 메뉴 추가 | `frontend/src/App.tsx`, `frontend/src/components/layout/Sidebar.tsx` 수정 |

### 46-검증 체크리스트

- [ ] CronScheduleManager가 서버 기동 시 active job을 로드하는가?
- [ ] ScheduleCronTool로 생성한 Cron 작업이 지정 시간에 실행되는가?
- [ ] 테넌트당 Cron 작업 50개 제한이 동작하는가? (BIZ-057)
- [ ] 최소 1분 간격 제한이 동작하는가? (BIZ-058)
- [ ] 3회 연속 실패 시 자동 비활성화되는가? (BIZ-059)
- [ ] CronListTool이 테넌트 범위 내 작업 목록을 반환하는가?
- [ ] CronDeleteTool이 작업을 삭제하고 스케줄러에서 제거하는가?
- [ ] ToolSearchTool이 키워드/태그로 도구를 검색하는가?
- [ ] SkillInvokeTool이 프롬프트+도구 조합으로 단일 LLM 호출을 실행하는가?
- [ ] Firecrawl 모드가 Python 사이드카에서 정상 동작하는가?
- [ ] API Key 미설정 시 js_render 폴백이 동작하는가? (BIZ-063)
- [ ] DOMAIN_FILTER 정책의 ALLOWLIST/BLOCKLIST 모드가 정상 동작하는가?
- [ ] 와일드카드 서브도메인 매칭이 동작하는가? (BIZ-065)
- [ ] FE ScheduledJobs/Skills 페이지가 렌더링되는가?
- [ ] 기존 정책/도구/워크플로우 동작이 깨지지 않는가?

---

## Sprint 47: 핵심 도구 네이티브화 (BE) [CR-037]

> **목표**: ClaudeCodeTool 경유 없이 직접 실행 가능한 핵심 도구 4종을 네이티브 구현하여 프로세스 오버헤드 제거, 정책 엔진 적용, lineage 추적 가능
> **Flyway**: 없음 (DB 변경 없음)
> **Phase 수**: 4

### 47-1. BashTool (PRD-241) — Phase 1

| # | 작업 | 산출물 |
|---|------|--------|
| 1 | BashTool 구현 (EnhancedToolExecutor) — ProcessBuilder 기반 셸 명령 실행 | `tool/builtin/BashTool.java` |
| 2 | 위험 명령 차단 목록 정의 (rm -rf /, shutdown, mkfs 등) | BashTool 내부 BLOCKED_COMMANDS 상수 |
| 3 | WorkspaceResolver 연동 — 작업 디렉토리 제한 | BashTool 내부 |
| 4 | 타임아웃 120초 + stdout/stderr 분리 캡처 | BashTool 내부 |
| 5 | BashTool 단위 테스트 (정상 실행, 차단 명령, 타임아웃) | `BashToolTest.java` |

### 47-2. FileWriteTool (PRD-242) — Phase 2

| # | 작업 | 산출물 |
|---|------|--------|
| 1 | FileWriteTool 구현 (EnhancedToolExecutor) — 신규 파일 생성/덮어쓰기 | `tool/builtin/FileWriteTool.java` |
| 2 | 부모 디렉토리 자동 생성 로직 | FileWriteTool 내부 |
| 3 | 기존 파일 덮어쓰기 시 경고 메시지 반환 | FileWriteTool 내부 |
| 4 | WorkspacePolicyEngine 연동 — 경로 검증 (허용 디렉토리 밖 쓰기 차단) | FileWriteTool 내부 |
| 5 | FileWriteTool 단위 테스트 (생성, 덮어쓰기, 경로 차단) | `FileWriteToolTest.java` |

### 47-3. WebSearchTool (PRD-243) — Phase 3

| # | 작업 | 산출물 |
|---|------|--------|
| 1 | WebSearchTool 구현 (EnhancedToolExecutor) — Tavily API 우선 | `tool/builtin/WebSearchTool.java` |
| 2 | DuckDuckGo HTML 폴백 구현 (Tavily Key 미설정 시) | WebSearchTool 내부 |
| 3 | Connection 테이블에서 Tavily API Key 조회 | WebSearchTool 내부 |
| 4 | 검색 결과 구조화 반환 (title/url/snippet) | WebSearchTool 내부 |
| 5 | WebSearchTool 단위 테스트 (Tavily 성공, 폴백, 결과 파싱) | `WebSearchToolTest.java` |

### 47-4. SuggestBackgroundPRTool (PRD-244) — Phase 4

| # | 작업 | 산출물 |
|---|------|--------|
| 1 | SuggestBackgroundPRTool 구현 (EnhancedToolExecutor) — Git 커밋 + GitHub PR 자동 생성 | `tool/builtin/SuggestBackgroundPRTool.java` |
| 2 | ProcessBuilder 기반 git 명령 실행 (add, commit, push, branch) | SuggestBackgroundPRTool 내부 |
| 3 | GitHub REST API 연동 — PR 생성 (Connection에서 토큰 조회) | SuggestBackgroundPRTool 내부 |
| 4 | 브랜치 네이밍 규칙 (aimbase/auto/{timestamp}-{description}) | SuggestBackgroundPRTool 내부 |
| 5 | SuggestBackgroundPRTool 단위 테스트 (PR 생성 플로우, 토큰 누락) | `SuggestBackgroundPRToolTest.java` |

### 47-검증 체크리스트

- [ ] BashTool이 셸 명령을 실행하고 stdout/stderr를 분리 반환하는가?
- [ ] BashTool에서 위험 명령(rm -rf / 등)이 차단되는가?
- [ ] BashTool 타임아웃 120초가 동작하는가?
- [ ] BashTool이 WorkspaceResolver 범위 밖 실행을 차단하는가?
- [ ] FileWriteTool이 신규 파일을 생성하고 부모 디렉토리를 자동 생성하는가?
- [ ] FileWriteTool이 기존 파일 덮어쓰기 시 경고를 반환하는가?
- [ ] FileWriteTool이 WorkspacePolicyEngine 범위 밖 경로를 차단하는가?
- [ ] WebSearchTool이 Tavily API로 검색 결과를 반환하는가?
- [ ] Tavily Key 미설정 시 DuckDuckGo 폴백이 동작하는가?
- [ ] WebSearchTool 결과가 title/url/snippet 구조로 반환되는가?
- [ ] SuggestBackgroundPRTool이 브랜치 생성 + 커밋 + PR을 생성하는가?
- [ ] GitHub 토큰 미설정 시 적절한 에러 메시지가 반환되는가?
- [ ] 4종 도구 모두 ToolRegistry에 등록되어 FE 도구 목록에 노출되는가?
- [ ] 4종 도구 모두 PolicyEngine 평가를 거치는가?
- [ ] 기존 도구 동작이 깨지지 않는가?

---

## Sprint 48: 에이전트 자율성 강화 (BE + FE) [CR-038]

> **목표**: 에이전트가 사람 개입 없이 MCP 리소스 탐색·읽기, 이벤트 기반 즉시 트리거, 세션 브리핑을 자율 수행하는 도구 4종 + FE 패널
> **Flyway**: V45 (Tenant DB)
> **Phase 수**: 4

### 48-1. MCPServerClient 리소스 메서드 + ListMcpResourcesTool + ReadMcpResourceTool (PRD-245, PRD-246) — Phase 1

| # | 작업 | 산출물 |
|---|------|--------|
| 1 | MCPServerClient에 listResources() 메서드 추가 | `mcp/MCPServerClient.java` 수정 |
| 2 | MCPServerClient에 readResource(uri) 메서드 추가 | `mcp/MCPServerClient.java` 수정 |
| 3 | MCPServerManager에 getClient(serverId) 접근자 추가 | `mcp/MCPServerManager.java` 수정 |
| 4 | ListMcpResourcesTool 구현 (EnhancedToolExecutor) | `tool/builtin/ListMcpResourcesTool.java` |
| 5 | ReadMcpResourceTool 구현 (EnhancedToolExecutor) | `tool/builtin/ReadMcpResourceTool.java` |

### 48-2. RemoteTriggerTool (PRD-247) — Phase 2

| # | 작업 | 산출물 |
|---|------|--------|
| 1 | RemoteTriggerTool 구현 (EnhancedToolExecutor) | `tool/builtin/RemoteTriggerTool.java` |
| 2 | Redis 기반 rate limiter 로직 (분당 10회) | RemoteTriggerTool 내부 |

### 48-3. BriefTool + SessionBriefService + DB (PRD-248) — Phase 3

| # | 작업 | 산출물 |
|---|------|--------|
| 1 | Flyway V45 마이그레이션 — session_briefs 테이블 | `V45__cr038_session_briefs.sql` |
| 2 | SessionBriefEntity JPA 엔티티 생성 | `domain/SessionBriefEntity.java` |
| 3 | SessionBriefRepository 생성 | `repository/SessionBriefRepository.java` |
| 4 | SessionBriefService — 브리핑 생성/캐시/조회 | `session/SessionBriefService.java` |
| 5 | BriefTool 구현 (EnhancedToolExecutor) | `tool/builtin/BriefTool.java` |
| 6 | SessionController에 brief REST API 추가 | `api/SessionController.java` 수정 |
...
### 48-4. FE 세션 브리핑 패널 (FE-019) — Phase 4

| # | 작업 | 산출물 |
|---|------|--------|
| 1 | API 클라이언트 함수 추가 (getBrief, createBrief) | `frontend/src/api/sessions.ts` 수정 |
| 2 | BriefPanel 컴포넌트 | `frontend/src/components/session/BriefPanel.tsx` |
| 3 | SessionDetail에 Brief 탭 추가 | `frontend/src/pages/sessions/SessionDetail.tsx` 수정 |

### 48-검증 체크리스트

- [ ] ListMcpResourcesTool이 연결된 MCP 서버의 리소스 목록을 반환하는가?
- [ ] ReadMcpResourceTool이 리소스 URI로 콘텐츠를 읽는가?
- [ ] ReadMcpResourceTool에서 32KB 초과 텍스트가 트렁케이션되는가? (BIZ-066)
- [ ] RemoteTriggerTool이 워크플로우/도구를 즉시 실행하는가?
- [ ] RemoteTriggerTool의 분당 10회 제한이 동작하는가? (BIZ-067)
- [ ] BriefTool이 세션 메시지 기반 브리핑을 생성하는가?
- [ ] BriefTool 캐시 TTL 1시간이 동작하는가? (BIZ-068)
- [ ] BriefTool이 최근 50개 메시지만 사용하는가? (BIZ-069)
- [ ] FE 세션 상세에서 Brief 탭이 렌더링되는가?
- [ ] 기존 MCP 도구/세션 동작이 깨지지 않는가?

---

## Sprint 49: CR-036 프롬프트 외부화 + OpenClaude 프롬프트 포팅 (10 Phase)

> **적용 버전**: v6.5.0 | **PRD**: PRD-249 ~ PRD-260, FE-020 ~ FE-021

### 49-1. 인프라 구축 — Phase 1 (PRD-249, PRD-250, PRD-251)

| # | 작업 | 산출물 |
|---|------|--------|
| 1 | Tenant DB 마이그레이션 — prompt_templates 테이블 | `backend/platform-core/src/main/resources/db/migration/tenant/V46__cr036_prompt_templates.sql` |
| 2 | PromptTemplateEntity + PromptTemplateEntityId (복합키 key+version) | `backend/platform-core/src/main/java/com/platform/domain/PromptTemplateEntity.java` |
| 3 | PromptTemplateRepository (JPA) | `backend/platform-core/src/main/java/com/platform/repository/PromptTemplateRepository.java` |
| 4 | PromptTemplateService (Caffeine 캐시 + 파일 폴백 + 렌더링) | `backend/platform-core/src/main/java/com/platform/prompt/PromptTemplateService.java` |
| 5 | PromptTemplateController (CRUD + render + bulk + cache 무효화) | `backend/platform-core/src/main/java/com/platform/api/PromptTemplateController.java` |

### 49-2. 기존 하드코딩 외부화 + 영문 전환 (25개) — Phase 2 (PRD-252, PRD-253)

| # | 작업 | 산출물 |
|---|------|--------|
| 1 | resources/prompts/ 디렉토리에 25개 기본값 txt 파일 생성 | `backend/platform-core/src/main/resources/prompts/*.txt` |
| 2 | V47 시드 마이그레이션 (25개 영문 INSERT, is_system=true) | `backend/platform-core/src/main/resources/db/migration/tenant/V47__cr036_seed_prompts.sql` |
| 3 | Java 8개 파일 수정: 하드코딩 → promptTemplateService.render() 호출 | 기존 Java 파일 수정 (Orchestrator, Agent, Policy 등) |

### 49-3. Python 사이드카 연동 — Phase 3 (PRD-254, PRD-255)

| # | 작업 | 산출물 |
|---|------|--------|
| 1 | PromptTemplateClient (httpx, 시작 시 bulk 로드 + 메모리 캐시) | `python-sidecar/src/rag_pipeline/prompt_client.py` |
| 2 | Python 3개 파일 수정: 하드코딩 → prompt_client 호출 | 기존 Python 파일 수정 (RAG, Safety, Evaluation) |

### 49-4. OpenClaude Core 시스템 프롬프트 포팅 — Phase 4 (PRD-256)

| # | 작업 | 산출물 |
|---|------|--------|
| 1 | OpenClaude 소스 분석 (~/Downloads/openclaude-main/src/) | 프롬프트 매핑 문서 |
| 2 | Core 카테고리 프롬프트 10+개 txt 파일 생성 | `backend/platform-core/src/main/resources/prompts/core.*.txt` |
| 3 | V47 시드에 Core 프롬프트 추가 INSERT | V47 마이그레이션 업데이트 |

### 49-5. OpenClaude Tool 프롬프트 Part 1 — Phase 5 (PRD-257)

| # | 작업 | 산출물 |
|---|------|--------|
| 1 | 주요 도구 8개 프롬프트 txt 파일 생성 (Read, Write, Edit, Bash, Grep, Glob 등) | `backend/platform-core/src/main/resources/prompts/tool.*.txt` |
| 2 | DB 시드 INSERT 추가 | V47 마이그레이션 업데이트 |

### 49-6. OpenClaude Tool 프롬프트 Part 2 — Phase 6 (PRD-258)

| # | 작업 | 산출물 |
|---|------|--------|
| 1 | 나머지 도구 28개 프롬프트 txt 파일 생성 (MCP, Agent, Notebook, Web 등) | `backend/platform-core/src/main/resources/prompts/tool.*.txt` |
| 2 | DB 시드 INSERT 추가 | V47 마이그레이션 업데이트 |

### 49-7. OpenClaude 서비스 프롬프트 포팅 — Phase 7 (PRD-259)

| # | 작업 | 산출물 |
|---|------|--------|
| 1 | 서비스 프롬프트 6개 txt 파일 생성 (에러 처리, 권한 요청, 압축, 메모리 등) | `backend/platform-core/src/main/resources/prompts/service.*.txt` |
| 2 | DB 시드 INSERT 추가 | V47 마이그레이션 업데이트 |

### 49-8. 동적 프롬프트 조립 엔진 — Phase 8 (PRD-260)

| # | 작업 | 산출물 |
|---|------|--------|
| 1 | PromptAssembler (에이전트 타입 + 도구 + 정책 기반 프롬프트 조립) | `backend/platform-core/src/main/java/com/platform/prompt/PromptAssembler.java` |
| 2 | 토큰 예산 관리 (우선순위 낮은 섹션 자동 제거) | PromptAssembler 내부 |
| 3 | Orchestrator에서 PromptAssembler 호출로 전환 | 기존 Orchestrator 수정 |

### 49-9. FE 관리 화면 — Phase 9 (FE-020, FE-021)

| # | 작업 | 산출물 |
|---|------|--------|
| 1 | API 클라이언트 함수 추가 | `frontend/src/api/promptTemplates.ts` |
| 2 | usePromptTemplates 훅 (React Query) | `frontend/src/hooks/usePromptTemplates.ts` |
| 3 | PromptTemplates 목록/상세/편집 페이지 | `frontend/src/pages/PromptTemplates.tsx` |
| 4 | PromptTestPanel 컴포넌트 (렌더링 테스트) | `frontend/src/components/prompt/PromptTestPanel.tsx` |
| 5 | App.tsx 라우트 등록 + Sidebar 메뉴 추가 | 기존 파일 수정 |

### 49-10. 캐시 최적화 + 가이드 + 검증 — Phase 10

| # | 작업 | 산출물 |
|---|------|--------|
| 1 | Caffeine 캐시 설정 튜닝 (TTL, 최대 크기) | application.yml 수정 |
| 2 | API 가이드 업데이트 (prompt-templates 엔드포인트) | `docs/guides/aimbase-api-guide.md` |
| 3 | 운용 가이드 업데이트 (프롬프트 관리 시나리오) | `docs/guides/aimbase-ops-guide.md` |
| 4 | 통합 검증 | 수동 테스트 |

### 49-검증 체크리스트

- [ ] prompt_templates 테이블이 Tenant DB에 정상 생성되는가?
- [ ] PromptTemplateController CRUD가 정상 동작하는가?
- [ ] is_system=true 프롬프트 삭제 시 409 반환되는가?
- [ ] render API가 {{변수}} 치환 + 토큰 추정을 반환하는가?
- [ ] bulk API가 카테고리 필터링된 프롬프트 목록을 반환하는가?
- [ ] 캐시 무효화 API가 Caffeine 캐시를 초기화하는가?
- [ ] Java 하드코딩 18개가 PromptTemplateService 호출로 교체되었는가?
- [ ] Python 하드코딩 7개가 PromptTemplateClient 호출로 교체되었는가?
- [ ] V47 시드 마이그레이션이 25개 + OpenClaude 프롬프트를 정상 INSERT하는가?
- [ ] 파일 폴백이 DB 프롬프트 없을 때 resources/prompts/*.txt를 로드하는가?
- [ ] PromptAssembler가 에이전트 타입별 프롬프트를 동적 조립하는가?
- [ ] 토큰 예산 초과 시 우선순위 낮은 섹션이 자동 제거되는가?
- [ ] FE 프롬프트 관리 화면이 렌더링되고 CRUD가 동작하는가?
- [ ] FE 테스트 패널에서 변수 입력 → 렌더링 결과가 표시되는가?
- [ ] 기존 Orchestrator/Agent/RAG 동작이 깨지지 않는가?

---

## Sprint 50: 런타임 설정 관리 (v6.7.0, CR-040)

> DB 기반 런타임 설정 관리 + 관리자 UI. 서버 재기동 없이 미세조정 가능.

### 50-1. BE — PlatformSettingsService + Controller + V13 seed — Phase 1

| # | 작업 | 산출물 |
|---|------|--------|
| 1 | V13__cr040_seed_platform_settings.sql 마이그레이션 작성 (~12개 기본값 INSERT) | `db/migration/master/V13__cr040_seed_platform_settings.sql` |
| 2 | PlatformSettingsService 구현 (Caffeine 캐시 + 카테고리 그룹핑 + 변경 감사 로그) | `config/PlatformSettingsService.java` |
| 3 | PlatformSettingsController 구현 (GET/PUT /api/v1/platform/settings) | `api/PlatformSettingsController.java` |

### 50-2. BE — 하드코딩 제거 — Phase 2

| # | 작업 | 산출물 |
|---|------|--------|
| 1 | ToolCallHandler: 80KB/50KB 임계값 → PlatformSettingsService 조회 | `tool/ToolCallHandler.java` 수정 |
| 2 | SessionStore: 24h TTL → PlatformSettingsService 조회 | `session/SessionStore.java` 수정 |
| 3 | SendMessageTool: 500/32KB → PlatformSettingsService 조회 | `tool/builtin/SendMessageTool.java` 수정 |
| 4 | ReadMcpResourceTool: 32KB → PlatformSettingsService 조회 | `tool/builtin/ReadMcpResourceTool.java` 수정 |

### 50-3. FE — 설정 관리 페이지 — Phase 3

| # | 작업 | 산출물 |
|---|------|--------|
| 1 | platformApi에 settings API 함수 추가 | `api/platform.ts` 수정 |
| 2 | usePlatformSettings 훅 (React Query) | `hooks/usePlatformSettings.ts` |
| 3 | PlatformSettings 페이지 컴포넌트 (카테고리 카드 + 인라인 편집) | `pages/platform/PlatformSettings.tsx` |
| 4 | App.tsx 라우트 등록 + Sidebar 메뉴 추가 | `App.tsx`, `components/layout/Sidebar.tsx` 수정 |

### Sprint 50 검증 체크리스트

- [ ] GET /api/v1/platform/settings 가 카테고리별 설정을 반환하는가?
- [ ] PUT /api/v1/platform/settings 로 설정 변경 후 즉시 반영되는가?
- [ ] 설정 변경 시 감사 로그가 기록되는가?
- [ ] 하드코딩 7곳이 PlatformSettingsService 조회로 교체되었는가?
- [ ] DB에 값이 없을 때 기본값으로 폴백하는가?
- [ ] FE 설정 페이지가 렌더링되고 수정이 동작하는가?
- [ ] 기존 오케스트레이터/세션/압축 동작이 깨지지 않는가?

---

## Sprint 52: 런타임 성능 최적화 (BE only) [CR-047]

**배경**: OpenClaude 전수 대조 결과 런타임 성능 레이어 4가지 누락/부분 구현. 기능 변경 없이 체감 지연(TTFT)·턴 총 시간·토큰 비용을 직접 개선.

**범위**: BE only. FE·DB 스키마·외부 API 변경 없음. 설정값 3개 추가(`aimbase.tool.parallel-max`, `aimbase.cache.ttl.system`, `aimbase.cache.ttl.tools`).

### Phase 0: KPI Baseline 측정

구현 전 현재 수치 기록. Phase 5에서 재측정하여 개선폭 보고.

| 지표 | 측정 방법 |
|---|---|
| TTFT (첫 토큰 지연) | Chat API 요청부터 첫 SSE `delta` 이벤트까지 |
| 병렬 도구 처리 시간 | safeCalls 3개(Read/Grep/Glob) 포함 턴 시작→전체 tool_result까지 |
| Cache hit 비율 | Anthropic 응답 `cache_read_input_tokens` / (`input_tokens` + `cache_read_input_tokens`) |
| 턴 총 지연 | 요청부터 `done` 이벤트까지 |

### Phase 1: PRD-298 병렬 도구 실행

**대상**: `ToolCallHandler.executeLoop():264-285`, `ToolCallHandler.executeLoopStream()`

| # | 작업 | 산출물 |
|---|------|--------|
| 1 | `aimbase.tool.parallel-max` 설정 추가(기본 10) | `application.yml`, `ToolCallHandlerProperties.java` |
| 2 | safeCalls 병렬 디스패치 구현 — `CompletableFuture.allOf` + `Executors.newVirtualThreadPerTaskExecutor()` | `ToolCallHandler.java` |
| 3 | 동시 실행 상한 Semaphore(parallelMax) 적용 | `ToolCallHandler.java` |
| 4 | 예외 격리(`exceptionally` 체인) — 한 도구 실패가 다른 도구 차단 금지 | `ToolCallHandler.java` |
| 5 | 결과 취합 시 원래 tool_use 순서대로 재정렬 | `ToolCallHandler.java` |
| 6 | unsafeCalls(Bash/Write/Edit 등) 순차 유지 확인 | `isConcurrencySafe` 플래그 재검토 |
| 7 | CancellationToken 전파 — 병렬 실행 중 abort 시 모든 future cancel | CR-046 연계 |
| 8 | 테스트 — 3개 safeCalls 병렬 시 max(t1,t2,t3) 수준으로 수렴 | `ToolCallHandlerParallelTest` |

### Phase 2: PRD-299 Memory/Skill Prefetch

**대상**: `OrchestratorEngine.chat():208`, `OrchestratorEngine.chatStream():430-525`, `ContextAssemblyEngine.assemble():177-192`

| # | 작업 | 산출물 |
|---|------|--------|
| 1 | OrchestratorEngine 진입 직후 memory/skill prefetch kick-off | `OrchestratorEngine.java` |
| 2 | `CompletableFuture<MemoryContext> memoryFuture` + `CompletableFuture<List<Skill>> skillFuture` | — |
| 3 | Virtual Thread executor 사용 + TenantContext 전파 (CR-045 SSE 패턴 참조) | `TenantAwareVirtualExecutor` 헬퍼 |
| 4 | ContextAssembly 단계에서 `.get(timeout)` 합류 — 타임아웃 시 빈 컨텍스트 폴백 | `ContextAssemblyEngine.java` |
| 5 | Prefetch 실패는 로그만 — LLM 호출 차단 금지 | `ContextAssemblyEngine.java` |
| 6 | 스트리밍 경로(chatStream)도 동일 적용 | `OrchestratorEngine.java` |
| 7 | 테스트 — prefetch 실패 시 LLM 호출 정상 진행 + 빈 컨텍스트 검증 | `OrchestratorPrefetchTest` |

### Phase 3: PRD-300 Prompt Cache TTL 소스별 분기

**대상**: `AnthropicAdapter.java:66-67,117-126,179-185,238-252`

| # | 작업 | 산출물 |
|---|------|--------|
| 1 | `CacheControlStrategy` 인터페이스 신설 | `llm/adapter/CacheControlStrategy.java` |
| 2 | 구현체 3종 — SystemPromptCacheStrategy(1h), ToolSchemaCacheStrategy(5m), MessageCacheStrategy(none) | `llm/adapter/cache/` |
| 3 | `aimbase.cache.ttl.system=1h`, `aimbase.cache.ttl.tools=5m` 설정 추가 | `application.yml` |
| 4 | AnthropicAdapter가 각 cache_control 포인트에서 해당 Strategy 주입 | `AnthropicAdapter.java` |
| 5 | Anthropic 응답의 `cache_creation_input_tokens` / `cache_read_input_tokens` 파싱 | `AnthropicAdapter.java` |
| 6 | UsageLog에 cache hit 필드 추가(기존 JSONB meta 활용 권장, 스키마 변경 회피) | `UsageLogEntity.java` |
| 7 | 시스템 프롬프트 수정 시 수동 invalidation API — `POST /api/v1/platform/cache/invalidate` 또는 버전 해시 기반 키 | `PlatformCacheController`(신규) |
| 8 | 테스트 — 동일 system 프롬프트 2회 호출 시 2회차 cache_read_input_tokens > 0 | `AnthropicCacheTest` |

### Phase 4: PRD-301 Hook 백그라운드 실행

**대상**: `HookDispatcher.java:75-100`, `HookEvent` enum

| # | 작업 | 산출물 |
|---|------|--------|
| 1 | `HookEvent` enum에 `synchronous: boolean` 속성 추가 | `HookEvent.java` |
| 2 | 26종 hook을 게이팅/로그성으로 분류 — 리뷰 필수 | 분류 테이블 문서화 |
| 3 | `HookDispatcher.executeAndAggregate()` 분기 — 동기 path와 `CompletableFuture.runAsync(..., virtualExecutor)` 비동기 path | `HookDispatcher.java` |
| 4 | 비동기 hook 실패 로그만 기록 — 메인 플로우 차단 금지 | `HookDispatcher.java` |
| 5 | 비동기 hook 순서 의존성 검토 — 순서 의존 시 동기 유지 또는 전용 순차 executor | 리뷰 산출물 |
| 6 | TenantContext 전파 (Virtual Thread) | `TenantAwareVirtualExecutor` 재사용 |
| 7 | 테스트 — 로그성 hook 예외 발생 시 turn 정상 완료 확인 | `HookDispatcherAsyncTest` |

### Phase 5: KPI 재측정 + 회귀 검증

| # | 작업 |
|---|------|
| 1 | Phase 0 동일 시나리오 재측정 |
| 2 | 개선폭 리포트 작성 (TTFT, 병렬 처리, cache hit, 턴 총 지연) |
| 3 | 기존 회귀 — CR-045 SSE 이벤트 순서, CR-046 중지·Soft Delete, CR-043 다중계정 재시도 정상 동작 확인 |
| 4 | 부하 테스트 — 동시 10 세션 기준 CPU/메모리 추이 |

### Sprint 52 검증 체크리스트

- [ ] safeCalls 3개 병렬 실행 시 총 시간이 max(t1,t2,t3) 수준인가?
- [ ] unsafeCalls(Bash/Write/Edit)는 순차 유지되는가?
- [ ] `aimbase.tool.parallel-max` 설정 변경이 런타임에 반영되는가?
- [ ] Memory/Skill prefetch가 LLM 호출과 병렬로 수행되는가?
- [ ] Prefetch 실패 시 빈 컨텍스트로 LLM 호출이 정상 진행되는가?
- [ ] System prompt 2회차 호출 시 `cache_read_input_tokens > 0` (1h TTL 동작)?
- [ ] Tool schema 변경 후 5분 경과 시 cache invalidation 동작하는가?
- [ ] 게이팅 hook(PRE_TOOL_USE 등)은 여전히 동기 실행되는가?
- [ ] 로그성 hook(POST_TOOL_USE 등) 예외가 메인 플로우를 차단하지 않는가?
- [ ] TenantContext가 Virtual Thread prefetch/async hook에서 정상 전파되는가?
- [ ] CR-046 abort 요청이 병렬 실행 중인 도구에도 전달되는가?
- [ ] KPI baseline 대비 TTFT/턴 총 지연/cache hit 개선 보고되는가?

---

## Sprint 53: 컨텍스트·토큰 효율 [CR-048]

**배경**: OpenClaude 전수 대조 결과 토큰·컨텍스트 최적화 3종 누락. MCP 도구 수백 개 보유 테넌트에서 매 턴 5k~10k 토큰 낭비, 대형 tool result 인라인 truncate로 복구 불가, Extended Thinking budget이 정적이라 복잡도 반영 부재.

**범위**: BE 중심. FE 변경 없음. 신규 테이블 1개(`tool_result_storage`), 신규 도구 1개(`ReadToolResult`), 수정 모듈 4곳(ToolSearchTool, ToolCallHandler, ContextAssemblyEngine, AnthropicAdapter).

**결정사항 (CR-048 § 4 승인, 2026-04-16)**:
- 기본 활성 도구 세트: Read/Edit/Grep/Bash/TodoWrite/ToolSearch 6종
- Storage 백엔드: Postgres
- Storage TTL: 24h (세션 TTL과 일치)
- Adaptive 공식: base 4000 × 1.5/2.0/1.3, cap 32000
- 비활성 도구 노출: 이름+1줄 설명을 system prompt 후미에 텍스트 삽입

### Phase 0: KPI Baseline 측정

| 지표 | 측정 방법 |
|---|---|
| 턴당 평균 input token | 도구 100개 보유 테스트 테넌트 기준 |
| Cache hit 비율 | `cache_read_input_tokens` / 전체 input (CR-047 Phase 3와 공유) |
| 대형 tool_result 발생률 | 임계치(81920B) 초과 tool_use 수 / 전체 |
| Adaptive thinking budget 분포 | session_metadata 집계 |

### Phase 1: PRD-300 Deferred Tool 스키마 런타임 주입

**대상**: `ToolSearchTool.java:56-69`, `ContextAssemblyEngine`, 신규 `SessionToolRegistry`

| # | 작업 | 산출물 |
|---|------|--------|
| 1 | `SessionToolRegistry` 신설 (sessionId별 활성 도구 집합 관리, Redis 백업 선택) | `tool/registry/SessionToolRegistry.java` |
| 2 | 기본 활성 세트 6종 시딩 — 세션 최초 접근 시 자동 등록 | `SessionToolRegistry.initDefaults()` |
| 3 | `ToolSearchTool.execute()` 수정 — 매치된 도구를 `registry.activate()` | `ToolSearchTool.java` |
| 4 | 응답 포맷 변경 — `activated` 배열 + 안내 문구 | `ToolSearchTool.java` |
| 5 | `ContextAssemblyEngine.assemble()` 수정 — 활성 도구만 full schema 주입 | `ContextAssemblyEngine.java` |
| 6 | 비활성 도구 이름+description만 system prompt 후미 텍스트 블록에 삽입 | `ContextAssemblyEngine.java` |
| 7 | Anthropic cache_control prefix 고정 — 활성 도구 변경은 prompt 후반부에만 반영 | 캐시 경계 테스트 |
| 8 | 테스트 — 초기 활성 6종만 tool defs 포함, ToolSearch 후 스키마 주입 확인 | `SessionToolRegistryTest`, `ToolSearchIntegrationTest` |

### Phase 2: PRD-301 Tool Result Storage 치환

**대상**: `ToolCallHandler.java:287-309`, `ToolResultCompactorRegistry`, 신규 `ToolResultStorageService` + `ReadToolResultTool`

| # | 작업 | 산출물 |
|---|------|--------|
| 1 | Flyway 마이그레이션 — `tool_result_storage` 테이블 + 인덱스 2개 | `V__tool_result_storage.sql` |
| 2 | `ToolResultStorageEntity` + `Repository` | `domain/ToolResultStorageEntity.java`, `repository/ToolResultStorageRepository.java` |
| 3 | `ToolResultStorageService` — `store()` / `read()` (session_id 검증 포함) | `tool/storage/ToolResultStorageService.java` |
| 4 | `ToolCallHandler` 수정 — 임계치 초과 시 storage 저장, 체인에 ref stub 주입 | `ToolCallHandler.java:287-309` |
| 5 | `ReadToolResultTool` 신규 — session_id 일치 검증 + audit_logs 기록 | `tool/builtin/ReadToolResultTool.java` |
| 6 | 기본 활성 도구 세트에 `ReadToolResult` 포함 (모델 자율 복구) | `SessionToolRegistry.initDefaults()` |
| 7 | 만료 스케줄러 — `@Scheduled` 일일 1회 `expires_at < NOW()` 삭제 | `tool/storage/ToolResultExpirer.java` |
| 8 | 테이블 크기 모니터링 — Micrometer gauge | `ToolResultStorageMetrics.java` |
| 9 | 테스트 — 85KB 결과 → stub 주입, 복구 성공, 타 세션 403, 만료 410 | `ToolResultStorageServiceTest`, `ReadToolResultToolTest` |

### Phase 3: PRD-302 Adaptive Thinking 동적 조정

**대상**: `AnthropicAdapter.java:130-160`, 신규 `AdaptiveThinkingPolicy`

| # | 작업 | 산출물 |
|---|------|--------|
| 1 | `AdaptiveThinkingPolicy` 인터페이스 + `DefaultAdaptiveThinkingPolicy` 구현 | `llm/thinking/AdaptiveThinkingPolicy.java` |
| 2 | `AdaptiveThinkingInputs` record — 직전 턴 tool_use 수/에러/질문 길이 | 동일 패키지 |
| 3 | 공식 구현 — base × 1.5/2.0/1.3, cap 32000 | `DefaultAdaptiveThinkingPolicy.calculate()` |
| 4 | 설정 키 5개 — CR-040 PlatformSettingsService seed | `PlatformSettings.seed()` |
| 5 | `AnthropicAdapter.resolveThinkingMode()` — ADAPTIVE 분기에서 policy 호출 | `AnthropicAdapter.java:130-160` |
| 6 | session_metadata에 budget + 결과 지표 기록 (A/B 로깅) | `SessionMetadataService.recordThinkingBudget()` |
| 7 | 테스트 — 시나리오별 budget 산정 검증 (tool 3+error+long → 15600) | `AdaptiveThinkingPolicyTest` |

### Phase 4: KPI 재측정 + 회귀 검증

| # | 작업 |
|---|------|
| 1 | Phase 0 동일 시나리오 재측정 — 턴당 input token 감소율, cache hit 변화 |
| 2 | tool_result_storage 일일 저장량/만료 삭제량 리포트 |
| 3 | Adaptive budget 분포 + 재시도/실패 상관 리포트 |
| 4 | 기존 회귀 — CR-047/046/045 정상 동작 확인 |
| 5 | cache_control prefix 경계 테스트 — 활성 도구 변경 전후 cache_read_input_tokens 유지 |

### Sprint 53 검증 체크리스트

- [ ] 세션 초기 상태에서 tool defs에 활성 6종만 포함되는가?
- [ ] ToolSearch 호출 후 다음 턴부터 매치 도구 스키마가 주입되는가?
- [ ] 비활성 도구는 system prompt 후미에 이름+설명만 노출되는가?
- [ ] Anthropic cache_read_input_tokens가 활성 도구 변경 후에도 유지되는가? (prefix 불변)
- [ ] 대형 tool result(>81920B) 발생 시 storage 저장 + 체인에 ref stub만 들어가는가?
- [ ] ReadToolResult가 본인 세션만 반환하고 타 세션은 403인가?
- [ ] 만료 result_id 요청 시 410 + 감사 로깅이 동작하는가?
- [ ] 일일 스케줄러가 expires_at 초과 레코드를 삭제하는가?
- [ ] Adaptive budget 공식이 입력 지표에 따라 정확히 계산되는가?
- [ ] session_metadata에 산정 budget + 결과 지표가 기록되는가?
- [ ] KPI baseline 대비 턴당 input token 감소율이 측정/보고되었는가?
- [ ] CR-047/046/045 회귀가 없는가?

### 흐름도 요약

**F-1. Deferred Tool 스키마 런타임 주입**
```
[Client request] → OrchestratorEngine
   → ContextAssemblyEngine
       ↳ registry.getActive(sessionId) → [Read, Edit, Grep, Bash, TodoWrite, ToolSearch]
       ↳ activeDefs = toolRegistry.schemasOf(active)
       ↳ inactiveList = toolRegistry.namesOf(all - active) → system prompt 후미 텍스트
   → AnthropicAdapter (tools=activeDefs, cache prefix 고정)
   → LLM 응답에 tool_use=ToolSearch
   → ToolSearchTool.execute()
       ↳ registry.activate(sessionId, [WebFetch, TaskCreate])
       ↳ response.activated = [...]
   → [다음 턴] ContextAssemblyEngine이 확장된 active 기준으로 tool defs 재조립
```

**F-2. Tool Result Storage 치환**
```
Tool 실행 완료 → result.content.bytes > 81920
   → ToolResultCompactorRegistry.compact() → summary
   → ToolResultStorageService.store(sessionId, toolName, full, summary) → resultId
   → 체인에 {type:"tool_result_ref", id:resultId, summary} 주입
   → 다음 LLM 호출은 요약만 포함 (토큰 절감)
(필요 시)
   → LLM tool_use=ReadToolResult(result_id)
   → session_id 일치 검증 + audit_log
   → full_content 반환
```

**F-3. Adaptive Thinking 동적 조정**
```
AnthropicAdapter.resolveThinkingMode() (mode=ADAPTIVE)
   → AdaptiveThinkingInputs {
       lastTurnToolCalls, lastTurnHadErrors, userQuestionLength
     }
   → policy.calculate(inputs)
       base 4000
       × (tool≥3 ? 1.5 : 1)
       × (error ? 2.0 : 1)
       × (q>500 ? 1.3 : 1)
   ...
   → session_metadata.record(budget, outcome)
```

---

## Sprint 54: 세션 복원·지침 체계 [CR-049]

### 목표
장기 세션 무손실 재개(Compact Boundary + Resume), Stop Hook 재진입 강제, 테넌트/프로젝트 커스텀 시스템 지침(cascade 병합).

### Phase 분할 (6 Phase)

**Phase 0: 결정 포인트 7개 사용자 최종 승인 + KPI baseline + prod 자동 마이그레이션 Initializer**
- 결정 7건 docs/CR_변경_이력.md CR-049 카드와 일치 확인
- baseline: 현재 평균 system prompt 길이, Resume 사용 가능 세션 비율(=COMPACT_BOUNDARY 보유 비율 — 신규 도입 전이라 0)
- **`ProdTenantMigrationInitializer`(@Profile("prod"), ApplicationRunner, @Order(1000)) 신규**: 부팅 시 TenantDataSourceManager.getAllCachedDataSources() 를 순회하며 FlywayMultiTenantConfig.migrateTenantDatabase() 를 호출해 모든 활성 테넌트에 마이그레이션을 자동 적용한다. 한 테넌트 실패 시 로그만 남기고 다음으로 진행. 재배포 시 반복되던 수동 V??.sql 적용 작업을 구조적으로 해소.

**Phase 1: PRD-305 BE 코어 (테넌트/프로젝트 지침)**
- Flyway V48 (tenant DB): `prompt_templates`에 `scope VARCHAR(20) NOT NULL DEFAULT 'GLOBAL'`, `project_id VARCHAR(100) NULL`, CHECK(scope='PROJECT' → project_id NOT NULL), `idx_pt_scope` 추가
- `PromptTemplateEntity` 필드 추가, JPA 매핑
- `PromptTemplateService.getTemplate(key, tenantId, projectId)` 신규 시그니처 — PROJECT → TENANT → GLOBAL 순서로 SELECT, cascade append 병합 (replace 아님)
- 합산 길이 8KB 초과 시 경고 로그 + Spring event 발행
- 단위 테스트: scope별 폴백, append 순서, 8KB 경고

**Phase 2: PRD-305 API + FE-034**
- `TenantInstructionsController`: GET/PUT `/api/v1/prompt-templates`, GET `/api/v1/prompt-templates/preview`
- `ProjectInstructionsController`: GET/PUT `/api/v1/projects/{id}/instructions`
- 권한 가드: 슈퍼어드민=GLOBAL, 테넌트 관리자=TENANT/PROJECT (`@PreAuthorize`)
- secret 패턴 감지 → audit_log + Response 헤더 `X-Secret-Warning`
- FE: TenantSettings "시스템 지침" 탭, ProjectDetail "프로젝트 지침" 탭, MonacoEditorLazy, SystemPromptPreview 3-pane, SecretWarningInline
- 훅: `usePromptTemplates`, `useUpsertPromptTemplate`, `useProjectInstructions`, `usePromptPreview`
- ContextAssemblyEngine.assembleSystemPrompt() 수정: GLOBAL 로드 → TENANT append → PROJECT append (BIZ-098)

**Phase 3: PRD-303 Compact Boundary 메시지 마커 + Resume API (BE)**
- Flyway V49 (tenant DB): `conversation_messages.message_type VARCHAR(30) NOT NULL DEFAULT 'TEXT'`, `boundary_meta JSONB NULL`, 부분 인덱스 `idx_conv_messages_session_boundary`
- `ConversationMessageEntity.messageType`, `boundaryMeta` 필드 추가
- `ContextWindowManager.trimWithState()` 수정: 압축 수행 시 압축된 그룹 뒤에 `message_type='COMPACT_BOUNDARY'` INSERT, `boundary_meta = {summary, compacted_count, tokens_saved}`
- LLM 컨텍스트 직렬화 시 COMPACT_BOUNDARY는 본문 X, 메타데이터로만 전달
- `SessionResumeController`: POST `/api/v1/sessions/{sessionId}/resume` — 가장 최근 boundary 이후 메시지 + preserved_context 반환, 24h TTL/소유자/streaming 검증

**Phase 4: PRD-303 FE Resume UI**
- ConversationItem에 ResumeButton 추가 (24h 이내 + COMPACT_BOUNDARY 보유 세션만 활성)
- `useSessionResume(sessionId)` 훅 + `resumeSession()` API
- CompactBoundaryDivider 컴포넌트: 접기/펼치기, summary, "N개 / Xk 토큰 절약" 표시
- MessageList가 COMPACT_BOUNDARY 메시지 만나면 Divider 렌더링 (위쪽 메시지 기본 접힘)

**Phase 5: PRD-304 Stop Hook 재진입 강제**
- `HookEvent.STOP` 결과 BLOCK 처리 강화: BLOCK reason을 새 user 메시지로 주입(`role=user, content=BLOCK 사유`)
- `ToolCallHandler.executeLoop()` 턴 종료 직전 STOP hook 호출, BLOCK이면 루프 재진입(max_iterations 상한 내)
- BIZ-097: 동일 reason 키 3회 초과 누적 시 강제 종료 + 사용자 알림 메시지 INSERT
- 단위 테스트: BLOCK→재진입, 3회 초과→강제 종료, max_iterations 우선

**Phase 6: 통합 검증 + 가이드 갱신**
- aimbase-api-guide.md, aimbase-ops-guide.md 갱신
- KPI 측정: Resume 사용률, Stop Hook BLOCK→PASS 전환율, 활성 테넌트 지침 수, cascade 합산 길이 분포
- 회귀: CR-046(Soft Delete), CR-048(Deferred Tool), CR-036(프롬프트 외부화) 정상 동작

### Sprint 54 검증 체크리스트

- [ ] V48 마이그레이션 후 기존 prompt_templates 레코드가 모두 scope='GLOBAL'로 backfill되는가?
- [ ] PROJECT scope 레코드 INSERT 시 project_id 누락이면 CHECK 위반으로 거부되는가?
- [ ] PromptTemplateService.getTemplate가 PROJECT → TENANT → GLOBAL 순서로 cascade append하는가?
- [ ] 합산 길이 8KB 초과 시 경고 발행 + 저장은 진행되는가?
- [ ] 슈퍼어드민이 아닌 사용자가 GLOBAL 편집 시 403인가?
- [ ] secret 패턴(API_KEY=, sk-, ghp_) 입력 시 audit_log + 경고 헤더가 동작하는가?
- [ ] V49 마이그레이션 후 conversation_messages.message_type 기본값이 'TEXT'인가?
- [ ] 압축 수행 시 COMPACT_BOUNDARY 메시지가 정확한 위치에 INSERT되는가?
- [ ] LLM 컨텍스트에 COMPACT_BOUNDARY 본문이 포함되지 않는가?
- [ ] Resume API가 24h 초과 세션에 대해 404를 반환하는가?
- [ ] Resume API가 streaming 중 세션에 대해 409를 반환하는가?
- [ ] Resume 응답에 boundary 정보 + preserved_context가 포함되는가?
- [ ] FE에서 ResumeButton이 24h 이내 + boundary 보유 세션에만 활성되는가?
- [ ] CompactBoundaryDivider가 접기/펼치기 동작하는가?
- [ ] STOP hook BLOCK 시 새 user 메시지가 주입되고 루프 재진입되는가?
- [ ] 동일 BLOCK reason 3회 초과 시 강제 종료 + 알림 메시지가 INSERT되는가?
- [ ] CR-046/CR-048/CR-036 회귀가 없는가?

### 흐름도 요약

**F-1. Cascade 시스템 지침 병합**
```
ContextAssemblyEngine.assembleSystemPrompt(tenantId, projectId)
   → PromptTemplateService.getTemplate("core.system.prefix", tenantId, projectId)
       ↳ SELECT * FROM prompt_templates WHERE key=? AND scope='GLOBAL' AND is_active
       ↳ SELECT * FROM prompt_templates WHERE key=? AND scope='TENANT' AND is_active
       ↳ SELECT * FROM prompt_templates WHERE key=? AND scope='PROJECT' AND project_id=? AND is_active
   → merged = global + "\n\n" + tenant + "\n\n" + project
   → if (length > 8192) eventPublisher.publish(SystemPromptOverflowEvent)
   → return merged
```

**F-2. Compact Boundary + Resume**
```
[자동 압축 트리거]
ContextWindowManager.trimWithState()
   → 토큰 임계 초과 감지
   → 압축 대상 메시지 그룹 선정
   → ConversationCompactor.summarize(group) → summary
   → INSERT conversation_messages(role='system', message_type='COMPACT_BOUNDARY',
                                    content=summary, boundary_meta={...})
   → 압축 대상 메시지는 deleted_at SET (soft delete) 또는 컨텍스트에서 제외
...
POST /sessions/{id}/resume
   → SessionResumeController
       ...
       ↳ SELECT * FROM conversation_messages
            WHERE session_id=? AND created_at >= (가장 최근 COMPACT_BOUNDARY)
       ↳ preserved_context = SessionStore.loadContext(sessionId)
   → 200 OK { boundary, messages, preserved_context }
```

**F-3. Stop Hook 재진입**
```
ToolCallHandler.executeLoop()
   → 턴 종료 조건 충족 (finishReason=end_turn)
   → HookDispatcher.dispatch(STOP, context)
       ↳ 게이팅 hook (BIZ-096) → 동기 실행
   → result.decision == BLOCK?
       Y → blockReasonHistory.add(reason)
           if (blockReasonHistory.countOf(reason) > 3) [BIZ-097]
               → INSERT system message "동일 검증 실패 3회 초과로 종료"
               → break
           else
               → INSERT user message (content=reason)
               → continue (재진입)
       N → return
```
