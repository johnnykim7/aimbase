...
| AGENT_CALL | 서브에이전트 호출 [v4.x, CR-033] |
| EVALUATOR_LOOP | 생성-평가-재생성 루프 (한 노드 내부에서 반복, max_iterations ≤ 10) [v7.10, CR-055] |
...
#### global_config seed 데이터 [CR-040]

V13 마이그레이션으로 아래 기본 설정값을 INSERT한다 (ON CONFLICT DO NOTHING).

| config_key | config_value | 카테고리 | 설명 |
|------------|-------------|---------|------|
| `orchestrator.max-tool-iterations` | `30` | orchestrator | 도구 루프 최대 횟수 |
| `orchestrator.default-max-tokens` | `16000` | orchestrator | Anthropic 기본 응답 최대 토큰 |
| `orchestrator.tool-result-budget-bytes` | `51200` | orchestrator | 도구 결과 축약 기준 (50KB) |
| `orchestrator.tool-result-compaction-threshold` | `81920` | orchestrator | 축약 트리거 임계값 (80KB) |
| `session.session-ttl-hours` | `24` | session | 세션 유지 시간 |
| `session.max-messages-per-session` | `500` | session | 세션당 메시지 수 제한 |
| `session.message-body-max-bytes` | `32768` | session | 메시지 본문 크기 제한 (32KB) |
| `compaction.snip-percent` | `70` | compaction | SNIP 압축 시작 % |
| `compaction.micro-compact-percent` | `85` | compaction | MICRO_COMPACT 시작 % |
| `compaction.session-memory-percent` | `91` | compaction | 세션 메모리 추출 % |
| `compaction.auto-compact-percent` | `93` | compaction | AUTO_COMPACT 시작 % |
| `compaction.blocking-limit-percent` | `98` | compaction | 블로킹 제한 % |
...
| deleted_at | TIMESTAMPTZ | ❌ | - | Soft delete 시점. NULL이면 활성 [v4.4, CR-046] |
| created_at | TIMESTAMPTZ | ✅ | NOW() | 생성일시 |
| updated_at | TIMESTAMPTZ | ✅ | NOW() | 수정일시 |
...
- 인덱스: `idx_conv_sessions_deleted` (deleted_at) WHERE deleted_at IS NULL [v4.4, CR-046]
- **Soft Delete 규칙 [CR-046]**: 모든 목록/조회 쿼리는 `WHERE deleted_at IS NULL` 필수. `DELETE /conversations/{id}`는 hard delete가 아닌 `UPDATE SET deleted_at = NOW()` 수행. 본인(user_id 일치)만 삭제 가능. 휴지통/복구 UI는 본 CR 범위 밖.

### conversation_messages (대화 메시지) [v3.0, CR-010 / v4.5, CR-049]
...
| message_type | VARCHAR(30) | ✅ | 'TEXT' | 메시지 타입: TEXT/TOOL_USE/TOOL_RESULT/COMPACT_BOUNDARY [v4.5, CR-049] |
| content | TEXT | ✅ | - | 메시지 내용 (COMPACT_BOUNDARY는 summary JSON) |
| tokens | INT | ✅ | 0 | 토큰 수 |
| model | VARCHAR(100) | ❌ | - | 모델명 |
| boundary_meta | JSONB | ❌ | - | COMPACT_BOUNDARY 메타: {summary, compacted_count, tokens_saved} [v4.5, CR-049] |
| deleted_at | TIMESTAMPTZ | ❌ | - | Soft delete 시점. 부모 세션 삭제 시 함께 마킹 [v4.4, CR-046] |
| created_at | TIMESTAMPTZ | ✅ | NOW() | 생성일시 |

- 인덱스: `idx_conv_messages_session` (session_id), `idx_conv_messages_session_boundary` (session_id, message_type) WHERE message_type='COMPACT_BOUNDARY' [v4.5, CR-049]
- **Append-only 규칙 [CR-046/CR-047]**: `SessionStore.persistToDb()`는 INSERT만 수행(기존 메시지 UPDATE/DELETE 금지). 중지(abort) 시 partial 메시지는 `[중단됨]` 마커 prefix로 저장.
- **Compact Boundary 규칙 [CR-049]**: `ContextWindowManager.trimWithState()`가 압축 수행 시 압축된 메시지 그룹 뒤에 `message_type='COMPACT_BOUNDARY'` 메시지를 INSERT한다. boundary_meta JSONB에 `{summary, compacted_count, tokens_saved}` 저장. LLM 컨텍스트 전송 시 본문 X, 메타데이터로만 전달. Resume API는 가장 최근 COMPACT_BOUNDARY 이후 메시지 + 보존 context를 반환한다.
...
### prompt_templates (Tenant DB) [v6.5, CR-036 / v7.7, CR-049]

| 컬럼 | 타입 | NULL | 기본값 | 설명 |
|------|------|------|--------|------|
| key | VARCHAR(100) | PK (복합) | - | 프롬프트 키 (category.subcategory.identifier) |
| version | INTEGER | PK (복합) | 1 | 버전 번호 |
| scope | VARCHAR(20) | NOT NULL | 'GLOBAL' | 적용 범위: GLOBAL/TENANT/PROJECT [v7.7, CR-049] |
| project_id | VARCHAR(100) | NULL | - | scope=PROJECT일 때 필수 [v7.7, CR-049] |
...
**복합키**: (key, version)
**인덱스**: `idx_pt_category` (category), `idx_pt_active` (is_active), `idx_pt_scope` (scope, project_id) [v7.7, CR-049]
**제약**: scope='PROJECT'이면 project_id NOT NULL (CHECK)
**스코프 우선순위 [BIZ-098, CR-049]**: PromptTemplateService.getTemplate(key, tenantId, projectId)는 GLOBAL을 base로 로드 후 TENANT scope 레코드를 append, PROJECT scope 레코드를 append하여 cascade 병합한다(replace 아닌 누적). tenant_id는 tenant DB라 자동 스코프됨. 합산 길이 8KB 초과 시 경고. 권한: 슈퍼어드민=GLOBAL 편집, 테넌트 관리자=TENANT/PROJECT 편집. 버저닝은 version 컬럼 재사용.
...
### tool_result_storage (Tenant DB) [v7.6, CR-048]

대형 Tool Result를 외부 저장소에 보관하고 체인에는 ref stub만 주입하기 위한 테이블. 임계치(81920B) 초과한 도구 결과의 원본을 저장하고, `ReadToolResult` 도구로 온디맨드 복구 가능.

| 필드 | 타입 | 필수 | 기본값 | 설명 |
|------|------|------|--------|------|
| result_id | VARCHAR(64) | ✅ | - | PK (예: `res_<uuid-short>`) |
| session_id | VARCHAR(100) | ✅ | - | 소유 세션 ID (권한 검증 기준) |
| tool_name | VARCHAR(100) | ✅ | - | 원본을 생성한 도구명 |
| full_content | TEXT | ✅ | - | 원본 Tool Result 본문 |
| summary | TEXT | ❌ | - | Compactor가 생성한 요약(체인 주입용) |
| size_bytes | INT | ✅ | - | 원본 바이트 수 |
| created_at | TIMESTAMPTZ | ✅ | NOW() | 저장 시각 |
| expires_at | TIMESTAMPTZ | ✅ | NOW() + INTERVAL '24 hours' | 만료 시각 (세션 TTL과 일치) |

- 인덱스: `idx_tool_result_storage_session` (session_id, created_at DESC), `idx_tool_result_storage_expires` (expires_at)
- 권한: `ReadToolResult` 호출 시 현재 세션의 result_id만 접근 가능 (session_id 일치 검증 + 감사 로깅)
- 만료 처리: 일일 스케줄러가 `expires_at < NOW()` 레코드 삭제. 테이블 크기 메트릭 모니터링.
- 관련 규칙: BIZ-002 (세션 TTL 24시간)
