...
| Layer | 역할 | 핵심 컴포넌트 |
|-------|------|--------------|
| **API Gateway** | 외부 진입점, 인증, 라우팅, 테넌트 식별 | Auth, Rate Limiter, Router, TenantResolver |
...
| **Multi-Tenancy** | 테넌트별 DB 완전 분리, 온보딩, 쿼터 | TenantRoutingDataSource, OnboardingService, QuotaService |
...
## 12. Multi-Tenancy — 테넌트별 완전 격리

### 12.1 격리 전략

본 플랫폼은 **테넌트별 DB 분리(Database-per-Tenant)** 방식을 채택한다. 각 테넌트는 완전히 독립된 PostgreSQL 인스턴스(또는 별도 데이터베이스)를 가지며, 테넌트 간 데이터가 물리적으로 격리된다.
...
│  │  요청 → tenant_id 추출 → DataSource 라우팅   │        │
│  └──────────────────┬──────────────────────────┘        │
│                     │                                   │
│  ┌──────────────────▼──────────────────────────┐        │
│  │           Master DB (1개)                    │        │
│  │                                             │        │
│  │  tenants: 테넌트 목록, DB 접속 정보          │        │
│  │  tenant_admins: 플랫폼 관리자 계정           │        │
│  │  subscriptions: 구독, 플랜, 쿼터             │        │
│  │  global_config: 플랫폼 전역 설정             │        │
│  └─────────────────────────────────────────────┘        │
│                     │                                   │
│       ┌─────────────┼─────────────┐                     │
│       │             │             │                     │
...
| DB | 역할 | 개수 |
|----|------|------|
| **Master DB** | 테넌트 목록, 구독/과금, 글로벌 설정, 플랫폼 관리자 | 1개 |
| **Tenant DB** | 해당 테넌트의 모든 비즈니스 데이터 | 테넌트당 1개 |

Master DB 테이블:

| 테이블 | 내용 |
|--------|------|
| tenants | id, name, slug, db_host, db_port, db_name, db_username, db_password(암호화), status(active/suspended/terminated), plan, created_at |
| tenant_admins | 플랫폼 슈퍼어드민 계정 (테넌트 생성/관리 권한) |
| subscriptions | tenant_id, plan(free/standard/enterprise), llm_monthly_quota, storage_quota_gb, max_knowledge_sources, max_workflows, expires_at |
| global_config | 플랫폼 전역 설정 (기본 LLM API 키 정책, 공지 등) |
| tenant_usage_summary | 테넌트별 월간 사용량 집계 (과금 용도) |

Tenant DB 테이블:

→ 기존 설계의 **모든 테이블 그대로** 포함. connections, mcp_servers, schemas, policies, prompts, routing_config, workflows, workflow_runs, action_logs, pending_approvals, audit_logs, roles, users, usage_logs, knowledge_sources, embeddings, ingestion_logs, retrieval_config. 스키마 변경 없음.

### 12.4 Tenant 식별 (Tenant Resolution)

API 요청에서 tenant_id를 추출하는 3가지 방법을 지원한다. 우선순위:

```
1순위: JWT 토큰 내 tenant_id 클레임 (인증된 요청)
2순위: HTTP 헤더 X-Tenant-Id (서버간 통신)
3순위: 서브도메인 (acme.platform.com → tenant_id = "acme")
```

추출된 tenant_id로 Master DB에서 해당 테넌트의 DB 접속 정보를 조회하고, 요청 스코프에서 해당 DataSource로 전환한다.

### 12.5 DataSource 라우팅

Spring의 `AbstractRoutingDataSource`를 활용한다.
...
TenantFilter (Servlet Filter)
  │ JWT / 헤더 / 서브도메인에서 tenant_id 추출
  │ ThreadLocal (또는 ScopedValue)에 tenant_id 설정
  │
  ▼
TenantRoutingDataSource (extends AbstractRoutingDataSource)
  │ ThreadLocal에서 tenant_id 읽음
  │ Master DB에서 해당 테넌트의 DataSource 조회 (캐싱됨)
  │ 해당 DataSource 반환
  │
  ▼
JPA Repository
  │ 테넌트를 모름. 그냥 쿼리 실행.
  │ DataSource가 이미 테넌트 DB를 가리키고 있으므로 자동으로 격리됨
  │
  ▼
응답 반환
  │
...
핵심: **Service, Repository, Controller 코드는 테넌트를 전혀 모른다.** DataSource 레이어에서 투명하게 처리된다.

### 12.6 테넌트 온보딩 자동화

새 테넌트 등록 시 자동으로 실행되는 프로세스:

```
1. Master DB에 tenant 레코드 생성
2. 새 PostgreSQL 데이터베이스 생성 (CREATE DATABASE)
3. pgvector 확장 설치 (CREATE EXTENSION vector)
4. Flyway 마이그레이션 실행 (테넌트 스키마 전체 적용)
5. 시드 데이터 삽입 (기본 역할, 기본 정책, 기본 라우팅 설정)
6. 테넌트 Admin 계정 생성
7. DataSource 캐시에 새 테넌트 추가
8. 완료 알림 (이메일 또는 웹훅)
```

이 전체 과정은 Admin API(`POST /api/v1/platform/tenants`)로 트리거되며, 자동화된다.

### 12.7 스키마 마이그레이션 전략

스키마 변경 시 모든 테넌트 DB에 순차 적용해야 한다.
...
│   └── ...
└── tenant/                    ← 모든 Tenant DB에 적용
    ├── V1__create_connections.sql
    ├── V2__create_policies.sql
    ├── V3__create_workflows.sql
    ├── V4__create_rag_tables.sql
    └── ...

배포 시:
1. Master DB에 master/ 마이그레이션 적용
2. 모든 활성 테넌트 목록 조회
3. 각 테넌트 DB에 tenant/ 마이그레이션 순차 적용
4. 실패한 테넌트가 있으면 로깅 + 알림 (나머지는 계속 진행)
```
...
| 리소스 | 격리 방식 | 설명 |
|--------|----------|------|
| **DB 데이터** | 물리적 격리 | 테넌트별 별도 DB |
| **RAG 벡터** | 물리적 격리 | 테넌트 DB 내 pgvector |
| **Redis 세션/캐시** | 논리적 격리 | key prefix `tenant:{id}:*` |
| **파일 스토리지** | 논리적 격리 | 경로 prefix `/{tenant_id}/` |
| **LLM API 키** | 선택적 | 테넌트 자체 키 또는 플랫폼 공유 키 (subscription에서 설정) |
| **MCP 서버** | 논리적 격리 | 테넌트 DB에 등록된 MCP만 사용 |

### 12.9 Admin 권한 구분

| 역할 | 범위 | 할 수 있는 것 |
|------|------|--------------|
| **Super Admin** | 플랫폼 전체 | 테넌트 생성/삭제/정지, 구독 관리, 전역 설정, 전체 모니터링 |
| **Tenant Admin** | 자기 테넌트만 | 연결/정책/워크플로우/RAG/프롬프트 관리, 사용자 관리, 모니터링 |
| **Tenant User** | 자기 테넌트 내 역할에 따라 | 기존 RBAC(roles 테이블)에 따른 권한 |

Super Admin은 Master DB의 tenant_admins로 관리되고, Tenant Admin/User는 각 테넌트 DB의 users/roles로 관리된다.

### 12.10 과금 및 쿼터

subscriptions 테이블에서 테넌트별 제한을 관리한다.

| 쿼터 항목 | 체크 시점 |
|----------|----------|
| 월간 LLM 토큰 사용량 | LLM 호출 전 (OrchestratorEngine) |
| 스토리지 용량 | RAG Ingestion 전, 파일 Write 전 |
| Knowledge Source 개수 | 소스 추가 시 |
| Workflow 개수 | 워크플로우 추가 시 |
| 동시 접속 / API 호출 Rate | API Gateway에서 |

쿼터 초과 시 요청을 거부하고 적절한 에러 응답을 반환한다.

### 12.11 코어 로직에 미치는 영향

**영향 없음.** 이것이 가장 중요한 포인트다.

Action Layer, Policy Engine, Workflow Engine, RAG Pipeline, Orchestrator, Schema Registry — 이 모든 코어 비즈니스 로직은 테넌트를 모른다. DataSource 라우팅이 인프라 레이어에서 투명하게 처리하므로, 코어 코드는 1줄도 변경되지 않는다.

변경되는 것은 인프라 레이어(DataSource 라우팅, 테넌트 관리 API, 온보딩 자동화, 마이그레이션 전략)뿐이다.

---

## 13. Domain Extension Layer

### 13.1 확장 포인트(Extension Points)
...
### Phase 1 — Core Foundation + Multi-Tenancy (5~7주)

**목표**: 멀티 테넌트 인프라 위에서 LLM 멀티 모델 호출 + 기본 액션 실행

```
[x] Multi-Tenancy 인프라: Master DB + Tenant DB 이중 구조
[x] TenantResolver + TenantRoutingDataSource (DataSource 라우팅)
[x] Tenant 온보딩 자동화 (DB 생성 → 마이그레이션 → 시드 → Admin 계정)
[x] 플랫폼 Admin API (테넌트 CRUD, 구독 관리)
[x] API Gateway (Spring Boot)
[x] LLM Adapter: OpenAI, Claude, Ollama(로컬)
[x] Unified Message Format
[x] 기본 Router (fixed model)
[x] Session Store (Redis, tenant prefix)
...
**결과물**: 테넌트별 완전 격리된 챗 API + DB 저장 + 실시간 알림 + 슈퍼어드민 테넌트 관리
...
| 멀티 테넌시 | 테넌트별 DB 분리 | 물리적 완전 격리. 코어 로직 변경 없이 인프라 레이어에서 처리 |
| 테넌트 라우팅 | AbstractRoutingDataSource | Service/Repository가 테넌트를 몰라도 자동 격리 |
...
| **Tenant** | 플랫폼을 사용하는 독립된 조직 단위. 각 테넌트는 자체 DB, 데이터, 설정을 가짐 |
| **Master DB** | 테넌트 목록, 구독, 과금 등 플랫폼 전역 정보를 관리하는 단일 DB |
| **Tenant DB** | 각 테넌트의 모든 비즈니스 데이터를 저장하는 독립 DB |
| **Tenant Resolution** | API 요청에서 tenant_id를 추출하여 해당 테넌트 DB로 라우팅하는 과정 |
| **Super Admin** | 플랫폼 전체를 관리하는 관리자 (테넌트 생성/삭제/정지) |
| **Tenant Admin** | 자기 테넌트 내의 모든 설정을 관리하는 관리자 |
