...
| **Multi-Tenancy** | 테넌트별 DB 완전 분리. Master DB(테넌트 관리) + Tenant DB(비즈니스 데이터). DataSource 라우팅으로 코어 로직 변경 없이 격리 |
| **Config-driven 동작** | 모든 비즈니스 로직이 DB 설정으로 동작. YAML은 시드/import/export용 |
| **Admin UI** | 비개발자가 연결, 정책, 워크플로우, Knowledge, 프롬프트, 모니터링을 관리하는 화면. 슈퍼어드민/테넌트어드민 구분 |
...
| `tenant` | 멀티 테넌시 인프라 | TenantResolver(JWT/헤더/서브도메인에서 tenant_id 추출), TenantRoutingDataSource(AbstractRoutingDataSource 구현), TenantDataSourceManager(테넌트별 DataSource 캐시/생성/제거), TenantContext(ThreadLocal/ScopedValue) |
| `tenant.onboarding` | 테넌트 온보딩 | TenantOnboardingService(DB 생성→마이그레이션→시드→Admin 계정 생성 자동화) |
| `tenant.quota` | 쿼터 관리 | QuotaService(LLM 토큰, 스토리지, Knowledge Source 수, Workflow 수 제한 체크) |
| `event` | 이벤트 버스 | EventBus, EventStore |
| `session` | 세션 | SessionStore(Redis, key에 tenant prefix), ContextWindowManager(토큰 관리, 히스토리 압축) |
| `api` | REST Controllers | ChatController, ModelController, ToolController, ActionController, WorkflowController, SchemaController, PolicyController, ConnectionController, MCPController, KnowledgeController, AdminController, **PlatformController(슈퍼어드민 전용 — 테넌트 CRUD, 구독 관리)** |
| `domain.master` | Master DB JPA Entities | TenantEntity, TenantAdminEntity, SubscriptionEntity, GlobalConfigEntity, TenantUsageSummaryEntity |
| `domain.tenant` | Tenant DB JPA Entities | 기존 모든 Entity (ConnectionEntity, PolicyEntity, WorkflowEntity 등) |
| `repository.master` | Master DB Repositories | TenantRepository, SubscriptionRepository 등 |
| `repository.tenant` | Tenant DB Repositories | 기존 모든 Repository |
| `config` | 설정 | SecurityConfig, RedisConfig, WebSocketConfig, SchedulerConfig, **TenantDataSourceConfig, FlywayMultiTenantConfig** 등 |
...
| `src/pages/platform/Tenants.tsx` | **슈퍼어드민 전용** — 테넌트 목록, 생성, 상태 관리(active/suspended) |
| `src/pages/platform/Subscriptions.tsx` | **슈퍼어드민 전용** — 테넌트별 구독/플랜/쿼터 관리 |
| `src/pages/platform/PlatformMonitoring.tsx` | **슈퍼어드민 전용** — 전체 테넌트 사용량 총괄, 과금 현황 |

Admin UI는 로그인한 사용자의 역할에 따라 보이는 메뉴가 다르다:
- **Super Admin**: platform/* 페이지 포함 전체 접근. 테넌트 전환 가능.
- **Tenant Admin**: 자기 테넌트 내 모든 페이지 접근. platform/* 페이지 안 보임.
- **Tenant User**: roles 테이블 권한에 따라 제한적 접근.
...
DB는 **Master DB** (1개)와 **Tenant DB** (테넌트당 1개)로 분리된다. 모든 테이블은 JPA Entity + Flyway 마이그레이션으로 구현한다.

Flyway 마이그레이션 파일도 두 경로로 분리:
- `migrations/master/` — Master DB에만 적용
- `migrations/tenant/` — 모든 Tenant DB에 적용
...
| 필드 | 타입 | 설명 |
|------|------|------|
| id | PK, varchar(50) | 테넌트 식별자 (slug, 서브도메인에도 사용) |
| name | varchar(200), not null | 회사/조직명 |
| db_host | varchar(200), not null | 테넌트 DB 호스트 |
| db_port | integer, default 5432 | |
| db_name | varchar(100), not null | 테넌트 DB 이름 |
| db_username | varchar(100), not null | |
| db_password | varchar(500), not null | 암호화 저장 |
| status | varchar(20), default 'active' | active, suspended, terminated |
| plan | varchar(30), default 'standard' | free, standard, enterprise |
| created_at, updated_at | timestamptz | |

### [Master DB] tenant_admins

플랫폼 슈퍼어드민 계정. 테넌트 생성/삭제/정지 권한.
...
|------|------|------|
| id | PK, UUID | |
| tenant_id | FK → tenants, unique | 테넌트당 1개 |
| plan | varchar(30), not null | free, standard, enterprise |
| llm_monthly_token_quota | bigint | 월간 LLM 토큰 한도 (null이면 무제한) |
| storage_quota_gb | integer | 스토리지 한도 GB |
| max_knowledge_sources | integer | Knowledge Source 최대 개수 |
| max_workflows | integer | Workflow 최대 개수 |
| max_users | integer | 사용자 최대 수 |
| llm_api_key_policy | varchar(20), default 'platform' | platform(공유 키), tenant(자체 키) |
| expires_at | timestamptz, nullable | 만료일 (null이면 무기한) |
| created_at, updated_at | timestamptz | |

### [Master DB] tenant_usage_summary

과금을 위한 테넌트별 월간 사용량 집계.

| 필드 | 타입 | 설명 |
|------|------|------|
| id | PK, UUID | |
| tenant_id | FK → tenants | |
| year_month | varchar(7), not null | "2025-02" 형식 |
...
tenant_id + year_month에 unique 인덱스.

### [Master DB] global_config
...
**이하 모든 테이블은 Tenant DB에 존재한다.** 스키마는 모든 테넌트에서 동일하고, 데이터만 다르다.
...
### TenantResolver / TenantRoutingDataSource (멀티 테넌시 인프라)

**TenantResolver**: Servlet Filter로 구현. 모든 요청의 JWT 토큰, X-Tenant-Id 헤더, 또는 서브도메인에서 tenant_id를 추출한다. 추출된 tenant_id를 ThreadLocal(또는 Java 21 ScopedValue)에 설정한다. 응답 후 ThreadLocal을 정리한다. 슈퍼어드민 API(`/api/v1/platform/*`)는 Master DB를 사용하므로 테넌트 라우팅 대상이 아니다.

**TenantRoutingDataSource**: Spring의 AbstractRoutingDataSource를 상속. ThreadLocal에서 tenant_id를 읽고, 해당 테넌트의 DataSource를 반환한다. tenant_id가 없으면(슈퍼어드민 요청) Master DB DataSource를 반환한다.

**TenantDataSourceManager**: 테넌트별 DataSource를 캐싱하고 관리한다. Master DB의 tenants 테이블에서 접속 정보를 읽어 HikariCP DataSource를 생성한다. 테넌트 추가/삭제/정지 시 캐시를 갱신한다. 정지된 테넌트(status=suspended)의 요청은 거부한다.

**핵심: Service, Repository, Controller 코드는 테넌트를 전혀 모른다.** DataSource 라우팅이 인프라 레이어에서 투명하게 처리하므로 코어 비즈니스 로직은 변경 없이 그대로 동작한다.

### TenantOnboardingService

새 테넌트 등록 시 전체 프로비저닝을 자동화한다. 순서: Master DB에 tenant 레코드 생성 → 새 PostgreSQL 데이터베이스 생성 → pgvector 확장 설치 → Flyway 마이그레이션(tenant/ 경로) 실행 → 시드 데이터 삽입(기본 역할, 기본 정책, 기본 라우팅) → 테넌트 Admin 계정 생성 → DataSource 캐시에 추가 → 완료 알림. 중간에 실패하면 생성된 리소스를 롤백한다.

### QuotaService

LLM 호출 전, RAG Ingestion 전, 설정 추가 시 해당 테넌트의 쿼터를 체크한다. Master DB의 subscriptions에서 한도를, tenant_usage_summary에서 현재 사용량을 조회하여 비교한다. 쿼터 초과 시 적절한 에러(429 또는 커스텀)를 반환한다.
...
### 플랫폼 관리 (Super Admin 전용 — Master DB 대상)

이 API들은 TenantResolver를 타지 않고 Master DB에 직접 접근한다.

- GET /api/v1/platform/tenants — 전체 테넌트 목록
- POST /api/v1/platform/tenants — 테넌트 생성 (온보딩 자동화 트리거)
- GET /api/v1/platform/tenants/{id} — 테넌트 상세 (상태, DB 정보, 사용량)
- PUT /api/v1/platform/tenants/{id} — 테넌트 정보 수정
- POST /api/v1/platform/tenants/{id}/suspend — 테넌트 정지
- POST /api/v1/platform/tenants/{id}/activate — 테넌트 활성화
- DELETE /api/v1/platform/tenants/{id} — 테넌트 삭제 (DB 포함, 주의: 복구 불가)
- GET /api/v1/platform/subscriptions — 전체 구독 현황
- PUT /api/v1/platform/subscriptions/{tenant_id} — 구독/쿼터 변경
- GET /api/v1/platform/usage — 전체 테넌트 사용량 총괄
- GET /api/v1/platform/usage/{tenant_id} — 특정 테넌트 사용량 상세
...
| 변경 사항 | 설정 위치 | 효과 |
|----------|----------|------|
| 새 테넌트 추가 | 슈퍼어드민 API → Master DB tenants | DB 자동 생성, 즉시 사용 가능 |
...
| 테넌트 쿼터 변경 | subscriptions | 즉시 반영 |
| 테넌트 정지/활성화 | tenants의 status | 즉시 반영 (정지 시 API 접근 차단) |
...
### Phase 1 — Core Foundation + Tenant 인프라 (5~7주)

**목표**: 멀티 테넌트 인프라 위에서, 하나의 모델로 채팅 → DB에 저장 → WebSocket으로 알림, 이 단일 흐름이 동작.

- 프로젝트 세팅: 멀티 모듈 Gradle, Docker Compose (Master PostgreSQL, 샘플 Tenant PostgreSQL, Redis, Ollama)
- **멀티 테넌시 인프라**: Master DB 스키마 + Tenant DB 스키마, Flyway 이중 마이그레이션 구조, TenantResolver(Filter), TenantRoutingDataSource(AbstractRoutingDataSource), TenantDataSourceManager
- **테넌트 온보딩**: TenantOnboardingService (DB 생성→마이그레이션→시드→Admin 계정 자동화)
- **플랫폼 Admin API**: POST /api/v1/platform/tenants (테넌트 생성), GET/PUT/DELETE, suspend/activate
- LLM Adapter 3종: Anthropic, OpenAI, Ollama
- Action Adapter: PostgreSQL Write + WebSocket Notify
- Chat API (POST /api/v1/chat/completions) + Session Store (Redis, tenant prefix)
- 기본 Policy (인증, 감사 로깅)
- Schema Validator (JSON Schema 검증)
- Admin UI: 대시보드 + 연결관리 + 슈퍼어드민 테넌트 관리 화면

**완료 기준**: 슈퍼어드민이 테넌트를 생성 → 테넌트 Admin으로 로그인 → 연결 등록 → API로 채팅 → Claude 응답 + DB 저장 + WebSocket 이벤트 → 다른 테넌트의 데이터와 완전히 격리됨을 확인
...
- **QuotaService**: 테넌트별 LLM 토큰/스토리지/리소스 수 제한 체크
- **구독 관리 API**: 플랜별 쿼터 설정, 사용량 집계, 초과 시 알림
- Admin UI: 정책 규칙 빌더 + 시뮬레이션 + 슈퍼어드민 구독/쿼터 관리
...
| 시점 | 체크포인트 |
|------|-----------|
| Week 1 | 프로젝트 세팅 완료. Master DB + 샘플 Tenant DB Docker Compose 기동 |
| Week 2 | 멀티 테넌시 인프라 동작: TenantResolver → DataSource 라우팅 → 테넌트별 격리 확인 |
| Week 3 | 테넌트 온보딩 자동화: API로 테넌트 생성 → DB 프로비저닝 → Admin 로그인 |
| Week 4 | 인터페이스(LLMAdapter, WriteAdapter, NotifyAdapter, PolicyEngine) 확정 |
| Week 5 | Phase 1 중간 데모 — 테넌트 A/B 격리 + LLM 호출 + DB Write + WebSocket Notify |
| Week 7 | Phase 1 완료 — 멀티 테넌트 채팅 흐름 + Admin 대시보드 + 슈퍼어드민 화면 |
| Week 11 | Phase 2+3 완료 — MCP Tool + RAG 검색 |
| Week 15 | Phase 4 완료 — 워크플로우 DAG 실행 |
| Week 19 | Phase 5 완료 — Policy + 승인 + 쿼터 관리 |
| Week 25 | Phase 6 완료 — v1.0 릴리즈 후보 |
...
- 비밀번호/API 키 등 민감한 config 필드는 저장 시 암호화한다. 특히 tenants 테이블의 db_password.
...
- **멀티 테넌시**: 코어 비즈니스 로직(Service, Repository)에 tenant_id를 직접 사용하지 않는다. DataSource 라우팅으로 투명하게 처리한다. 이 원칙을 절대 깨지 말 것.
- **Flyway 마이그레이션**: master/와 tenant/ 두 경로를 분리한다. 새 마이그레이션 추가 시 tenant/ 경로 파일은 모든 활성 테넌트에 적용되므로 하위 호환성에 주의한다.
- **Redis key**: 모든 Redis key에 `tenant:{tenant_id}:` prefix를 포함한다. 테넌트 삭제 시 해당 prefix의 키를 전부 정리한다.
- **Docker Compose (로컬 개발)**: Master PostgreSQL(5432), 샘플 Tenant PostgreSQL(5433), Redis(6379), Ollama(11434)를 포함한다. 또는 하나의 PostgreSQL 인스턴스에서 별도 database로 분리해도 된다.
